...
ES_FILE *sft_fopen(const char *file, const char *mode)
{
    ...
    g_memset(&sft_header, 0x00, sizeof(sft_header));
    ...
    if (0 == g_strncmp("COMPV1.0", sft_header.head, 8))
    {
        ...
        g_memset(sft_fp->pmax_compress_block_buf, 0x00, sft_fp->sft_c_header.max_compress_block_size);
        g_memset(sft_fp->puncompress_block_buf, 0x00, sft_fp->sft_c_header.block_size);
    }
    else
    {
        sft_fp->type = 0;
