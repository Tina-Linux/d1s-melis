...
static FT_Error ft_gzip_check_header(FT_Stream  stream)
{
    FT_Error  error;
    FT_Byte   head[4];
...
static void ft_gzip_file_done(FT_GZipFile  zip)
{
    z_stream  *zstream = &zip->zstream;
    inflateEnd(zstream);
    /* clear the rest */
...
static FT_Error ft_gzip_file_reset(FT_GZipFile  zip)
{
...
static FT_Error ft_gzip_file_fill_input(FT_GZipFile  zip)
{
    z_stream  *zstream = &zip->zstream;
    FT_Stream  stream  = zip->source;
    FT_ULong   size;
...
static FT_Error ft_gzip_file_skip_output(FT_GZipFile  zip,
        FT_ULong     count)
{
    FT_Error  error = Gzip_Err_Ok;
    FT_ULong  delta;
...
static FT_ULong ft_gzip_file_io(FT_GZipFile  zip,
                                FT_ULong     pos,
                                FT_Byte     *buffer,
                                FT_ULong     count)
{
...
static FT_ULong ft_gzip_stream_io(FT_Stream  stream,
                                  FT_ULong   pos,
                                  FT_Byte   *buffer,
                                  FT_ULong   count)
{
...
static FT_ULong ft_gzip_get_uncompressed_size(FT_Stream  stream)
{
    FT_Error  error;
    FT_ULong  old_pos;
    FT_ULong  result = 0;
