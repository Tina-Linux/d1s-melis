...
static U16 _GetCharCode(const char GUI_UNI_PTR *s)
{
    ...
    if (esCHS_Char2Uni(EPDK_CHARSET_ENM_EUC_KR, (__u8 *)s, 8, &r) <= 0)
    {
        r = 1;
    }
...
static int _CalcSizeOfChar(U16 Char)
{
    ...
    if ((Char >= 0x2e00 && Char <= 0x9fff)
        || (Char >= 0xac00 && Char <= 0xd7af))  /* Single byte (ASCII)  */
    {
        r = 2;
    }
...
{
    int r;
    r = esCHS_Uni2Char(EPDK_CHARSET_ENM_EUC_KR, Char, (__u8 *)s, 3);
...
void GUI_UC_SetEncodeEUCKR(void)
{
    ...
    GUI_Context.charset_enum = EPDK_CHARSET_ENM_EUC_KR;
    GUI_UNLOCK();
}
