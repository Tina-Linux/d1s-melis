 ...
 * shall return 0.
 */
__attribute__((weak)) unsigned int __rt_ffs(rt_uint32_t value)
{
    if (value == 0)
    {
        return 0;
...
static void dump_memory(rt_uint32_t *buf, int32_t len)
{
    int i;
    rt_kprintf("\n\rdump stack memory:");
    for (i = 0; i < len; i ++)
...
void rt_assert_handler(const char *ex_string, const char *func, rt_size_t line)
{
    ...
    if (rt_assert_hook == RT_NULL)
    {
...
#endif
        {
            ...
            backtrace(RT_NULL, RT_NULL, 0, 0, rt_kprintf);
#endif
            dump_memory((rt_uint32_t *)&sp, 128);
            while (dummy == 0);
        }
    }
    else
...
int *__errno(void) __attribute__((alias("_rt_errno")));
