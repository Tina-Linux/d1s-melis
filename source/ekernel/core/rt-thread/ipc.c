 ...
 * @return the error code
 */
rt_err_t rt_sem_take(rt_sem_t sem, rt_base_t time)
{
...
rt_err_t rt_sem_control(rt_sem_t sem, int cmd, void *arg)
{
    ...
    else if (cmd == RT_IPC_CMD_GET_STATE)
    {
        level = rt_hw_interrupt_disable();
        *(rt_uint16_t *)arg = sem->value;
        rt_hw_interrupt_enable(level);
 ...
 * @return the error code
 */
rt_err_t rt_mutex_take(rt_mutex_t mutex, rt_base_t time)
{
    register rt_base_t temp;
    struct rt_thread *thread;
 ...
 */
rt_event_t rt_event_create(const char *name, rt_uint32_t init_val, rt_uint8_t flag)
{
    rt_event_t event;
 ...
 * @return the error code
 */
rt_err_t rt_mb_recv(rt_mailbox_t mb, rt_ubase_t *value, rt_base_t timeout)
{
    struct rt_thread *thread;
    register rt_ubase_t temp;
    rt_uint32_t tick_delta;
