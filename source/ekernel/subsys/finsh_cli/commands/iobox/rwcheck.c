...
#define DEFAULT_TEST_SIZE (128 * MB)
...
static long long g_default_buf_size = (4 * MB);
...
static int init_orgin(struct rwtask *task)
{
    ...
    if (!buf)
    {
        printf("malloc memory failed - (%lld)bytes\n", g_default_buf_size);
        close(fd);
        return -1;
    }
    ...
    while (size)
    {
        ...
        len = min(size, g_default_buf_size);
        randbuf(buf, len);
...
static int copy_file(const char *in, const char *out, unsigned long long size)
{
    ...
    while (size > 0)
    {
        unsigned long long sz = min(size, (unsigned long long)g_default_buf_size);
...
static int get_sum_by_file(const char *file, int len, sum_t *sum)
{
    ...
    *sum = 0;
    while (len)
    {
        int rlen = min(len, g_default_buf_size);
...
static int begin(struct rwtask *task)
{
    ...
    for (i = 0; i < multi_thread; i++)
    {
        ...
        pthread_create(&g_task[i].pth, NULL, rwcheck_thread, &g_task[i]);
...
static int rwcheck_main(int argc, char **argv)
{
    ...
    task = &g_task[0];
    optind = 0;
    while ((opts = getopt(argc, argv, ":hd:t:s:p:b:m:")) != EOF)
    {
        switch (opts)
        {
            case 'h':
    ...
    {
        printf("invalid sys info: ddr %llu/%llu KB, flash %llu KB, min free %llu KB\n",
