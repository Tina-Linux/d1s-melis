...
static int msh_readline(FILE* fp, char *line_buf, int size)
{
    char ch;
    int index = 0;
...
int msh_exec_script(const char *cmd_line, int size)
{
    ...
    if (fp)
    {
        ...
        do
        {
            length = msh_readline(fp, line_buf, RT_CONSOLEBUF_SIZE);
            if (length > 0)
            {
                char ch = '\0';
                int index;
...
int cmd_cat(int argc, char **argv)
{
    ...
    for (index = 1; index < argc; index ++)
    {
    	...
    	if (strncmp("/proc/version", argv[index], strlen("/proc/version")) == 0) {
			unsigned char buf[1024] = {0};
			unsigned char bufffer[2048] = {0};
			sprintf(bufffer, "Melis %s-%s-%s\nCommitid :%s", melis_banner_string(),  GCC_VERSION, LNK_VERSION, SDK_GIT_VERSION);
#if defined(HAL_GIT_VERSION)
			sprintf(buf, "\nHalgitid :%s\n", HAL_GIT_VERSION);
			strcat(bufffer, buf);
#endif
	        rt_kprintf("%s", bufffer);
		} else {
			cat(argv[index]);
		}
        rt_kprintf("\n");
