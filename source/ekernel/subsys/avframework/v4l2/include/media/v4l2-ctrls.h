...
#include <melis/common/list_type.h>
#else
#include <aw_list.h>
#endif
...
#else
int v4l2_ctrl_handler_init_class(struct v4l2_ctrl_handler *hdl,
				 unsigned int nr_of_controls_hint, const char *name);
#endif
...
#else
#define v4l2_ctrl_handler_init(hdl, nr_of_controls_hint)		\
	v4l2_ctrl_handler_init_class(hdl, nr_of_controls_hint, NULL)
#endif
#endif
...
{
//	rt_mutex_take(ctrl->handler->lock, RT_WAITING_FOREVER);
}
...
{
//	rt_mutex_release(ctrl->handler->lock);
}
 ...
 *
 * @file: pointer to struct file
 * @wait: pointer to struct rt_pollreq
 */
unsigned int v4l2_ctrl_poll(struct file *file, struct rt_pollreq *wait);
