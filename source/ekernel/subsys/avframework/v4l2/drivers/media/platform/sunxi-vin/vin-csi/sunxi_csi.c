/*
 * ekernel/subsys/avframework/v4l2/platform/sunxi-vin/vin-csi/sunxi_csi.c
 *
...
static int __csi_pin_config(struct csi_dev *dev, int enable)
{
	...
	*/
	unsigned int csi1_pins[] = {
		GPIOE(0), GPIOE(2), GPIOE(3),
		GPIOE(4), GPIOE(5), GPIOE(6), GPIOE(7),
		GPIOE(8), GPIOE(9), GPIOE(10), GPIOE(11),
		GPIOE(12), GPIOE(13), GPIOE(14), GPIOE(15),
		GPIOE(18), GPIOE(19), GPIOE(20), GPIOE(21)
	};
	...
	for (i = 0; i < ARRAY_SIZE(csi1_pins); i ++) {
		hal_gpio_pinmux_set_function(csi1_pins[i], func);
		hal_gpio_set_driving_level(csi1_pins[i], 1);
		hal_gpio_set_pull(csi1_pins[i], 0);
	}

	usleep(100);
#else
#error "You should configure csi pins"
#endif
				    ...
				    struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->pad == CSI_PAD_SOURCE) {
		if (mf) {
			rt_mutex_take(&csi->subdev_lock, RT_WAITING_FOREVER);
			fmt->format = *mf;
			rt_mutex_release(&csi->subdev_lock);
		}
		return 0;
	}
...
static int __csi_init_subdev(struct csi_dev *csi)
{
	...
	rt_mutex_init(&csi->subdev_lock, "csisubmutex", RT_IPC_FLAG_FIFO);
	csi->arrange.row = 1;
	csi->arrange.column = 1;
	csi->bus_info.ch_total_num = 1;
...
static int csi_probe(int id)
{
	struct csi_dev *csi = NULL;
	int ret = 0;

	csi = rt_calloc(1, sizeof(struct csi_dev));
	if (!csi) {
		ret = -ENOMEM;
		goto ekzalloc;
	}
	...
	csi->id = id;
	csi->base = sunxi_vin_get_csi_base(id);
	if (!csi->base) {
		ret = -EIO;
		goto freedev;
	}
	...
	rt_mutex_init(&csi->reset_lock, "csirstmtx", RT_IPC_FLAG_FIFO);
...
int csi_remove(int id)
{
	struct csi_dev *csi = glb_parser[id];
	struct v4l2_subdev *sd = &csi->subdev;

	v4l2_set_subdevdata(sd, NULL);
	__csi_pin_release(csi);
	rt_mutex_delete(&csi->subdev_lock);
	rt_mutex_delete(&csi->reset_lock);
