...
static int saveRawPicture(SampleViCapS *pViCapCtx, VIDEO_FRAME_INFO_S *pFrameInfo)
{
    ...
    snprintf(strFilePath, MAX_FILE_PATH_SIZE+64, "%s/pic[%d].%s", pContext->mConfigPara.mStoreDir, pViCapCtx->mRawStoreNum, strPixFmt);
...
//    }
    ...
    aloge("ViCap threadId[0x%lx] exit[%d], pValue[%p]", pContext->mViCapCtx.mThreadId, ret, pValue);
