...
//    snd_pcm_sw_params_alloca(&swparams);
    ...
    err = snd_pcm_hw_params_set_rate(pcmCfg->handle, params, pcmCfg->sampleRate, 0);
    if (err < 0) {
    ...
    pcmCfg->significantBitsPerSample = snd_pcm_format_size(pcmCfg->format, pcmCfg->sampleRate);
...
int alsaMixerSetVolume(AIO_MIXER_S *mixer, int playFlag, long value)
{
    ...
    if (playFlag) {
        ...
        printf("Now card: %s set PlaybackVolume to:%ld, err:%d",
        ...
        printf("Now card: %s set Capturegain to:%ld, err:%d",
                    mixer->card_name, realVol, err);
    }
