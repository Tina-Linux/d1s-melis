...
int cdx_sem_down_timedwait(cdx_sem_t* tsem, unsigned int timeout)
{
    ...
    if(tsem->semval == 0)
    {
        alogv("semdown wait:%p val:%d",tsem, tsem->semval);
        struct timespec ts;
        clock_gettime(CLOCK_REALTIME, &ts);    //CLOCK_REALTIME, CLOCK_MONOTONIC
        ...
        if(ETIMEDOUT == ret)
        {
            //alogd("pthread cond timeout[%d]ms np ret[%d]", timeout, ret);
        }
        else if(0 == ret)
        {
        }
...
void cdx_sem_timewait(cdx_sem_t* tsem, unsigned int msec)
{
    ...
    pthread_mutex_lock(&tsem->mutex);
    pthread_cond_timedwait(&tsem->condition, &tsem->mutex, &ts);
    pthread_mutex_unlock(&tsem->mutex);
}
...
{
    pthread_mutex_lock(&tsem->mutex);
    unsigned int semval = tsem->semval;
    pthread_mutex_unlock(&tsem->mutex);
    return semval;
}
