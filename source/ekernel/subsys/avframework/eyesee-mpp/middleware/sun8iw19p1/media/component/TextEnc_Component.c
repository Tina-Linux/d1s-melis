...
TryGetTextEncBuf:
            ...
            else if(ERR_TEXT_ENC_INPUT_UNDERFLOW == ret)
            {
                ...
                if (0 < cnt)
                {
                    alogw("(f:%s, l:%d) Low probability! TextEnc Component has input, cnt: [%d]", __FUNCTION__, __LINE__, cnt);
            ...
            { 
                ...
                if(0 < emptyFrameCnt)
                {
                    alogw("(f:%s, l:%d) Low probability! TextEnc Component has empty frames[%d]", __FUNCTION__, __LINE__, emptyFrameCnt);
                    pthread_mutex_unlock(&pTextEncData->mOutFrameListMutex);
                }
                else
                {
...
//    pTextEncData->sOutPortDef.format.audio.eEncoding = PT_AAC;
    ...
    alogd("create TextEnc comp threadId[0x%lx]", pTextEncData->thread_id);
    EXIT: return eError;
}
