...
int cdx_seek_fd_file(struct cdx_stream_info *stream, cdx_off_t offset, int whence)
{
	...
	if(whence == SEEK_SET) {
		...
		seek_result = lseek(stream->fd_desc.fd, stream->fd_desc.cur_offset, SEEK_SET);
        if(-1 == seek_result)
        {
    ...
    if(stream->mIODirectionFlag)
    {
        ...
        if(absSeekPos > stream->mFileSize)
        {
            ...
            {
                seek_result = lseek(stream->fd_desc.fd, 0, SEEK_CUR);
            }
            else
            {
        ...
        {
            seek_result = lseek(stream->fd_desc.fd, offset, whence);
        }
    }
    else
    {
...
ssize_t cdx_read_fd_file(void *ptr, size_t size, size_t nmemb, struct cdx_stream_info *stream)
{
	...
	ssize_t result = lseek(stream->fd_desc.fd, stream->fd_desc.cur_offset, SEEK_SET);
	if(result == -1) {
    ...
    stream->fd_desc.cur_offset = lseek(stream->fd_desc.fd, 0, SEEK_CUR);
...
{
    alogv("(f:%s, l:%d)");
    off64_t result = lseek(stream->fd_desc.fd, stream->fd_desc.cur_offset, SEEK_SET);
	if(result == -1) {
        aloge("fatal error! seek fail!");
		return -1;
	}
    ...
    if(stream->mFtruncateFlag)
    {
        ...
        fileOffset = lseek(stream->fd_desc.fd, 0, SEEK_END);
        ...
        if(nAlignByteSize > 0)
        {
            ...
            fileOffset = stream->mFileEndOffset = stream->mFileSize = lseek(stream->fd_desc.fd, 0, SEEK_CUR);
            if(nAlignByteSize <= stream->mAlignBufSize)
            {
            }
            else
    ...
    if(fileOffset%DIRECTIO_UNIT_SIZE!=0)
    {
        ...
        off64_t seekRet = lseek(stream->fd_desc.fd, alignOffset, SEEK_SET);
        if(-1 == seekRet)
        {
            aloge("fatal error! seek fail!");
            return -1;
        ...
        if(result != DIRECTIO_UNIT_SIZE)
        {
            ...
            off64_t seekPos = lseek(stream->fd_desc.fd, alignOffset, SEEK_SET);
            if(-1 == seekPos)
            {
                aloge("fatal error! seek fail!");
                return -1;
        ...
        result = lseek(stream->fd_desc.fd, alignOffset, SEEK_SET);
        if(-1 == result)
        {
            aloge("fatal error! seek fail!");
            return -1;
        ...
        if(blockLeftSize >= nByteSize)
        {
            ...
            pCurPtr += nByteSize;
            result = lseek(stream->fd_desc.fd, fileOffset+nByteSize, SEEK_SET);
            if(-1 == result)
            {
                aloge("fatal error! seek fail!");
                return -1;
    ...
    if(nByteSize)
    {
        ...
        off64_t curOffset = lseek(stream->fd_desc.fd, 0, SEEK_CUR);   //must align.
        if(-1 == curOffset)
        {
            aloge("fatal error! seek fail!");
            return -1;
        ...
        result = lseek(stream->fd_desc.fd, curOffset, SEEK_SET);
        if(-1 == result)
        {
            aloge("fatal error! seek fail!");
            return -1;
        ...
        pCurPtr += nByteSize;
        nByteSize = 0;
        result = lseek(stream->fd_desc.fd, desOffset, SEEK_SET);
        if(-1 == result)
        {
            aloge("fatal error! seek fail!");
            return -1;
    ...
    else
    {
        ...
        fileOffset = lseek(stream->fd_desc.fd, 0, SEEK_CUR);
        if(-1 == fileOffset)
        {
            aloge("fatal error! seek fail!");
            return -1;
...
ssize_t cdx_write_fd_file(const void *ptr, size_t size, size_t nmemb, struct cdx_stream_info *stream)
{
	int result = lseek(stream->fd_desc.fd, stream->fd_desc.cur_offset, SEEK_SET);
	if(result == -1) {
        aloge("fatal error! seek fail!");
		return -1;
	}
	...
	cdx_off_t seekResult = lseek(stream->fd_desc.fd, 0, SEEK_CUR);
    if(-1 == seekResult)
    {
...
long long cdx_get_fd_size_file(struct cdx_stream_info *stream)
{
	...
	cdx_off_t fdSize = lseek(stream->fd_desc.fd, 0, SEEK_END);
    if(-1 == fdSize)
    {
        aloge("fatal error! seek fail!");
	}
    cdx_off_t size = fdSize - stream->fd_desc.offset;
	cdx_off_t seekResult = lseek(stream->fd_desc.fd, stream->fd_desc.cur_offset, SEEK_SET);
    if(-1 == seekResult)
    {
        aloge("fatal error! seek fail!");
	}
...
//#ifdef __OS_ANDROID
    ...
    if(CEDARX_SOURCE_FD == datasource_desc->source_type)
    {
        //alogd("fd[%d], offset[%lld], length[%lld]", datasource_desc->ext_fd_desc.fd, datasource_desc->ext_fd_desc.offset, datasource_desc->ext_fd_desc.length);
            ...
            stm_info->fd_desc.cur_offset = datasource_desc->ext_fd_desc.offset;
            stm_info->fd_desc.length = datasource_desc->ext_fd_desc.length;
            stm_info->mFileEndOffset = lseek(stm_info->fd_desc.fd, 0, SEEK_END);
            stm_info->mFileSize = stm_info->mFileEndOffset;
