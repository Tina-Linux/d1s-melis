...
static int CedarvVideoEncInit(VIDEOENCDATATYPE *pVideoEncData)
{
    ...
    if(pVideoEncData->pCedarV)
    {
        ...
        else if(PT_H264 == pVideoEncData->mEncChnAttr.VeAttr.Type)
        {
            ...
            VideoEncSetParameter(pVideoEncData->pCedarV, VENC_IndexParamProductCase, &pVideoEncData->mEncChnAttr.RcAttr.product_mode);
        ...
        PARAM_IN  COMP_BUFFERHEADERTYPE* pBuffer)
{
    ...
    if(pVideoEncData->is_compress_source == 0)
    {
        ...
        if(pVideoEncData->csi_first_frame)
        {
            ...
            alogd("first video frame pts[%lld]us,tm1[%lld]us, vSize[%dx%d], frmId[%d]",
                    pEncFrame->VFrame.mpts, tm1, pEncFrame->VFrame.mWidth, pEncFrame->VFrame.mHeight, pEncFrame->mId);
        }
