...
int hal_efuse_write(char *key_name, unsigned char *key_data, size_t key_bit_len)
{
	...
	key_map = hal_efuse_search_key_by_name(key_name);
	...
	ret = hal_efuse_acl_ck(key_map, 1);
	if(ret) {
	...
	while(k_d_lft >= 4) {
		...
		if(tmp_data)
		{
			if(hal_efuse_uni_burn_key(niddle, tmp_data))
			{
	...
	if(k_d_lft) {
		...
		if(mask)
		{
			if(hal_efuse_uni_burn_key(niddle,mask))
			{
				return  EFUSE_ERR_BURN_TIMING;
			}
		}
	...
	if ((key_map->burned_flg_offset >= 0) &&
	   (key_map->burned_flg_offset <= EFUSE_BRUN_RD_OFFSET_MASK)) {
		hal_efuse_set_cfg_flg(EFUSE_WRITE_PROTECT,key_map->burned_flg_offset);
	}
	...
	if ((key_map->rd_fbd_offset >= 0) &&
	   (key_map->rd_fbd_offset <= EFUSE_BRUN_RD_OFFSET_MASK)) {
		hal_efuse_set_cfg_flg(EFUSE_READ_PROTECT,key_map->rd_fbd_offset);
	}
...
int hal_efuse_read(char *key_name, unsigned char *key_data, size_t key_bit_len)
{
	...
	ret = hal_efuse_acl_ck(key_map, 0);
	if(ret) {
	...
	for(i = 0;i<k_u32_l;i++)
	{
		tmp = hal_efuse_sram_read_key(offset);
