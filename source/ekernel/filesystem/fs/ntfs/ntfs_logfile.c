...
static BOOL ntfs_check_restart_page_header(RESTART_PAGE_HEADER *rp, s64 pos)
{
    ...
    {
        fs_log_error("$LogFile uses unsupported page size.\n");
        return FALSE;
    }
    /*
    ...
    {
        fs_log_error("Found restart area in incorrect "
                     "position in $LogFile.\n");
        return FALSE;
    }
    ...
    {
        fs_log_error("$LogFile version %i.%i is not "
    ...
    {
        fs_log_error("$LogFile restart page specifies "
...
skip_usa_checks:
    ...
    {
        fs_log_error("$LogFile restart page is not modified "
...
static BOOL ntfs_check_restart_area(RESTART_PAGE_HEADER *rp)
{
    ...
    {
        fs_log_error("$LogFile restart area specifies "
    ...
    {
        fs_log_error("$LogFile restart area is out of bounds "
...
check_list:
    ...
    return TRUE;
err_out:
    fs_log_error("$LogFile log client array is corrupt.\n");
    return FALSE;
}
 ...
 * and fs_err otherwise.  The restart page may have been modified by chkdsk in
        ...
        LSN *lsn)
{
    ...
    {
        err = fs_err;
        fs_log_error("Failed to read whole restart page into the "
                     "buffer.\n");
        if (err != ENOMEM)
        {
            err = EIO;
                                    ...
                                    le32_to_cpu(rp->system_page_size)))
    {
        ...
        {
            fs_log_error("Multi sector transfer error "
...
BOOL ntfs_check_logfile(ntfs_attr *log_na, RESTART_PAGE_HEADER **rp)
{
    ...
    {
        fs_log_error("$LogFile is too small.\n");
        return FALSE;
    }
    ...
    for (pos = 0; pos < size; pos <<= 1)
    {
        ...
        if (ntfs_attr_pread(log_na, pos, NTFS_BLOCK_SIZE, kaddr) !=
            NTFS_BLOCK_SIZE)
        {
            fs_log_error("Failed to read first NTFS_BLOCK_SIZE "
...
is_empty:
        fs_log_trace01("Done.  ($LogFile is empty.)\n");
        return TRUE;
    }
    if (!rstr1_ph)
    {
        if (rstr2_ph)
        {
            fs_log_error("BUG: rstr2_ph isn't NULL!\n");
        }
        fs_log_error("Did not find any restart pages in "
    ...
    if (rstr2_ph)
    {
        ...
        {
            fs_log_debug("Using second restart page as it is more "
                         "recent.\n");
            free(rstr1_ph);
            rstr1_ph = rstr2_ph;
            /* rstr1_lsn = rstr2_lsn; */
        ...
        {
            fs_log_debug("Using first restart page as it is more "
                         "recent.\n");
            free(rstr2_ph);
        }
        rstr2_ph = NULL;
...
BOOL ntfs_is_logfile_clean(ntfs_attr *log_na, RESTART_PAGE_HEADER *rp)
{
    ...
    {
        fs_log_trace01("$LogFile is empty\n");
        return TRUE;
    }
    if (!rp)
    {
        fs_log_error("Restart page header is NULL\n");
        return FALSE;
    }
    if (!ntfs_is_rstr_record(rp->magic) &&
        !ntfs_is_chkd_record(rp->magic))
    {
        fs_log_error("Restart page buffer is invalid\n");
        return FALSE;
    }
    ...
    {
        fs_log_error("The disk contains an unclean file system (%d, "
                     "%d).\n", le16_to_cpu(ra->client_in_use_list),
                     le16_to_cpu(ra->flags));
        return FALSE;
    }
    /* $LogFile indicates a clean shutdown. */
    fs_log_trace01("$LogFile indicates a clean shutdown\n");
    return TRUE;
}
 ...
 */
#define NTFS_LOG_BUF_SIZE (128 * 1024)
int ntfs_empty_logfile(ntfs_attr *na)
{
    s64 pos, count;
    char buf[NTFS_BUF_SIZE] = {'\0'};
    char *buf_write = NULL;
    char *buf_heap = NULL;
    int size = NTFS_LOG_BUF_SIZE;

    buf_heap = malloc(NTFS_LOG_BUF_SIZE);
    buf_write = buf_heap;
    if (buf_write == NULL)
    {
        size = NTFS_BUF_SIZE;
    ...
    {
        fs_err = EIO;
        fs_log_perror("Resident $LogFile $DATA attribute");
        if (buf_heap)
        {
            free(buf_heap);
        }
    ...
    while ((count = na->data_size - pos) > 0)
    {
        ...
        count = ntfs_attr_pwrite(na, pos, count, buf_write);
        if (count <= 0)
        {
            fs_log_perror("Failed to reset $LogFile");
            if (count != -1)
            {
                fs_err = EIO;
            }
