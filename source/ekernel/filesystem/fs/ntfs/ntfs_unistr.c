                       ...
                       const u32 upcase_len)
{
    ...
    {
        fs_log_debug("ntfs_names_collate received NULL pointer!\n");
        exit(1);
    }
#endif
...
int ntfs_ucsncmp(const ntfschar *s1, const ntfschar *s2, size_t n)
{
    ...
    if (!s1 || !s2)
    {
        fs_log_debug("ntfs_wcsncmp() received NULL pointer!\n");
        exit(1);
    }
#endif
    for (i = 0; i < n; ++i)
                     ...
                     const ntfschar *upcase, const u32 upcase_size)
{
    ...
    if (!s1 || !s2 || !upcase)
    {
        fs_log_debug("ntfs_wcsncasecmp() received NULL pointer!\n");
        exit(1);
    }
#endif
    for (i = 0; i < n; ++i)
 ...
 * on failure with fs_err set to the error code.
 */
 ...
 *
 * On error, -1 is returned, and fs_err is set to the error code. The following
                  ...
                  int outs_len)
{
    ...
    if (!mbs)
    {
        mbs_len = (ins_len + 1) * __MB_CUR_MAX;
    ...
    for (i = o = 0; i < ins_len; i++)
    {
        ...
        wc = (__wchar_t)le16_to_cpu(ins[i]);
        if (!wc)
        {
            break;
        }
        /* Convert the CPU endian wide character to multibyte. */
        cnt = nls->uni2char(wc, mbs + o, __MB_CUR_MAX);
        if (cnt == -1)
        {
            goto err_out;
        }
...
int ntfs_mbstoucs(const char *ins, ntfschar **outs, int outs_len)
{
    ...
    for (i = o = cnt = 0; i < ins_size; i += cnt, o++)
    {
        ...
        cnt = nls->char2uni(ins + i, ins_size - i, &wc);
        if (!cnt)
        {
            break;
        }
 ...
 * anymore.
 *
 * On error NULL is returned and fs_err is set to the error code.
 */
ntfschar *ntfs_str2ucs(const char *s, int *len)
{
    ...
    {
        fs_log_perror("Couldn't convert '%s' to Unicode", s);
        return NULL;
    }
