...
static BOOL ntfs_rl_are_mergeable(runlist_element *dst, runlist_element *src)
{
    if (!dst || !src)
    {
        fs_log_debug("Eeek. ntfs_rl_are_mergeable() invoked with NULL "
                     "pointer!\n");
        return FALSE;
    }
 ...
 *
 * On error, return NULL, with fs_err set to the error code. Both runlists are
 * left unmodified.
 */
                                       ...
                                       runlist_element *src, int ssize, int loc)
{
    ...
    if (!dst || !src)
    {
        fs_log_debug("Eeek. ntfs_rl_append() invoked with NULL "
                     "pointer!\n");
        fs_err = EINVAL;
        return NULL;
    }
                                       ...
                                       runlist_element *src, int ssize, int loc)
{
    ...
    if (!dst || !src)
    {
        fs_log_debug("Eeek. ntfs_rl_insert() invoked with NULL "
                     "pointer!\n");
        fs_err = EINVAL;
        return NULL;
    }
                                        ...
                                        int loc)
{
    ...
    if (!dst || !src)
    {
        fs_log_debug("Eeek. ntfs_rl_replace() invoked with NULL "
                     "pointer!\n");
        fs_err = EINVAL;
        return NULL;
    }
                                      ...
                                      runlist_element *src, int ssize, int loc)
{
    if (!dst || !src)
    {
        fs_log_debug("Eeek. ntfs_rl_split() invoked with NULL pointer!\n");
        fs_err = EINVAL;
        return NULL;
    }
        ...
        runlist_element *srl)
{
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("%s: unmapped source runlist", __FUNCTION__);
        return NULL;
    }
    ...
    {
        fs_err = ERANGE;
        fs_log_perror("Run lists overlap. Cannot merge");
        return NULL;
    }
...

    {
        ...
        fs_log_debug("dfinal = %i, dend = %i\n", dfinal, dend);
        fs_log_debug("sstart = %i, sfinal = %i, send = %i\n", sstart, sfinal, send);
        fs_log_debug("start = %i, finish = %i\n", start, finish);
        fs_log_debug("ds = %i, ss = %i, dins = %i\n", ds, ss, dins);
        ...
        if (!drl)
        {
            fs_log_perror("Merge failed");
            return drl;
        }
        free(srl);
        if (marker)
        {
            fs_log_debug("Triggering marker code.\n");
            ...
            if (drl[ds].vcn <= marker_vcn)
            {
                ...
                if (drl[ds].vcn == marker_vcn)
                {
                    fs_log_debug("Old marker = %lli, replacing with "
                                 "LCN_ENOENT.\n",
...
finished:
    ...
    fs_log_debug("Merged runlist:\n");
    ntfs_debug_runlist_dump(drl);
    return drl;

critical_error:
    /* Critical error! We cannot afford to fail here. */
    fs_log_perror("libntfs: Critical error");
    fs_log_debug("Forcing segmentation fault!\n");
 ...
 *
 * On error, return NULL with fs_err set to the error code. @old_rl is left
 ...
 * runlist if overlap present before returning NULL, with fs_err = ERANGE).
 */
        ...
        const ATTR_RECORD *attr, runlist_element *old_rl)
{
    ...
    fs_log_trace01("Entering for attr 0x%x.\n",
    ...
    {
        fs_log_debug("Corrupt attribute.\n");
        fs_err = EIO;
        return NULL;
    }
    ...
    while (buf < attr_end && *buf)
    {
        ...
        {
            fs_log_debug("Missing length entry in mapping pairs "
                         "array.\n");
            deltaxcn = (s64) - 1;
        }
        /*
        ...
        if (deltaxcn < 0)
        {
            fs_log_debug("Invalid length in mapping pairs array.\n");
            goto err_out;
        }
        /*
        ...
        else
        {
            ...
            if (lcn < (LCN) - 1)
            {
                fs_log_debug("Invalid LCN < -1 in mapping pairs "
                             "array.\n");
                goto err_out;
            }
    ...
    {
mpa_err:
        fs_log_debug("Corrupt mapping pairs array in non-resident "
                     "attribute.\n");
        goto err_out;
    }
    ...
    if (!attr->lowest_vcn)
    {
        ...
        if (deltaxcn)
        {
            ...
            {
                fs_log_debug("More extents to follow; deltaxcn = "
            ...
            {
                fs_log_debug("Corrupt attribute. deltaxcn = "
                             "0x%llx, max_cluster = 0x%llx\n",
                             (long long)deltaxcn,
                             (long long)max_cluster);
                goto mpa_err;
    ...
    if (!old_rl)
    {
        fs_log_debug("Mapping pairs array successfully decompressed:\n");
    ...
    err = fs_err;
    free(rl);
    fs_log_debug("Failed to merge runlists.\n");
    fs_err = err;
    return NULL;
io_error:
    fs_log_debug("Corrupt attribute.\n");
                  ...
                  const s64 pos, s64 count, void *b)
{
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("Failed to read runlist [vol: %p rl: %p "
    ...
    for (total = 0LL; count; rl++, ofs = 0)
    {
        ...
        to_read = min(count, (rl->length << vol->cluster_size_bits) -
                      ofs);
retry:
        bytes_read = ntfs_pread(vol->_part, (rl->lcn <<
        ...
        if (bytes_read == (s64) - 1 && fs_err == EINTR)
        {
            goto retry;
        }
        if (bytes_read == (s64) - 1)
                   ...
                   const s64 pos, s64 count, void *b)
{
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("Failed to write runlist [vol: %p rl: %p "
                      "pos: %lld count: %lld]", vol, rl,
                      (long long)pos, (long long)count);
        goto fs_err_set;
    }
...
retry:
        if (!NVolReadOnly(vol))
            written = ntfs_pwrite(vol->_part, (rl->lcn <<
                                               vol->cluster_size_bits) + ofs,
                                  to_write, b);
        else
        {
 ...
 * fs_err set to the error code.  The following error codes are defined:
                                    ...
                                    const runlist_element *rl, const VCN start_vcn)
{
    ...
    if (start_vcn < 0)
    {
        fs_log_trace01("start_vcn %lld (should be >= 0)\n",
                       (long long) start_vcn);
        fs_err = EINVAL;
        goto fs_err_set;
    }
    if (!rl)
    {
        if (start_vcn)
        {
            fs_log_trace01("rl NULL, start_vcn %lld (should be > 0)\n",
                           (long long) start_vcn);
            fs_err = EINVAL;
            goto fs_err_set;
        }
 ...
 * destination buffer @dst is too small, return -1 with fs_err set ENOSPC.
 */
 ...
 *
 * Return 0 on success.  On error, return -1 with fs_err set to the error code.
 * The following error codes are defined:
 *  EINVAL  - Run list contains unmapped elements. Make sure to only pass
 *        fully mapped runlists to this function.
 *      - @start_vcn is invalid.
...
int ntfs_rl_truncate(runlist **arl, const VCN start_vcn)
{
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("rl_truncate error: arl: %p *arl: %p", arl, *arl);
        return -1;
    }
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("Start_vcn lies outside front of runlist");
        return -1;
    }
    ...
    {
        fs_err = EIO;
        fs_log_trace01("Truncating already truncated runlist?\n");
        return -1;
    }
 ...
 * @rl:     runlist to check
 *
 * Return 1 if have, 0 if not, -1 on error with fs_err set to the error code.
 */
int ntfs_rl_sparse(runlist *rl)
{
    ...
    for (rlc = rl; rlc->length; rlc++)
        if (rlc->lcn < 0)
        {
            ...
            {
                fs_err = EINVAL;
                fs_log_perror("%s: bad runlist", __FUNCTION__);
                return -1;
            }
            return 1;
        }
 ...
 *
 * Return compressed size or -1 on error with fs_err set to the error code.
 */
...
static void test_rl_pure_test(int test, BOOL contig, BOOL multi, int vcn, int len, runlist_element *file, int size)
{
    ...
    if (!src || !dst)
    {
        __log("Test %2d ---------- FAILED! (no free memory?)", test);
        return;
    }
...
static void test_rl_pure(char *contig, char *multi)
{
    ...
    {
        __log("rl pure [contig|noncontig] [single|multi]");
        return;
    }
...
static void test_rl_frag(char *test)
{
    ...
    {
        test_rl_frag_combine(&vol, attr1, attr2, attr3);
    }
    else if (strcmp(test, "132") == 0)
    {
        test_rl_frag_combine(&vol, attr1, attr3, attr2);
    }
    else if (strcmp(test, "213") == 0)
    {
        test_rl_frag_combine(&vol, attr2, attr1, attr3);
    }
    else if (strcmp(test, "231") == 0)
    {
        test_rl_frag_combine(&vol, attr2, attr3, attr1);
    }
    else if (strcmp(test, "312") == 0)
    {
        test_rl_frag_combine(&vol, attr3, attr1, attr2);
    }
    else if (strcmp(test, "321") == 0)
    {
        test_rl_frag_combine(&vol, attr3, attr2, attr1);
    }
    else
    {
        __log("Frag: No such test '%s'", test);
    }
...
int test_rl_main(int argc, char *argv[])
{
    ...
    else if ((argc == 3) && (strcmp(argv[1], "frag") == 0))
    {
        test_rl_frag(argv[2]);
    }
    else if ((argc == 4) && (strcmp(argv[1], "pure") == 0))
    {
        test_rl_pure(argv[2], argv[3]);
    }
    else
    {
        __log("rl [zero|frag|pure] {args}");
    }
...
void ntfs_debug_runlist_dump(const runlist_element *rl)
{
    ...
    fs_log_debug("NTFS-fs DEBUG: Dumping runlist (values in hex):\n");
    if (!rl)
    {
        fs_log_debug("Run list not present.\n");
        return;
    }
    fs_log_debug("VCN              LCN               Run length\n");
    do
    {
        LCN lcn = (rl + i)->lcn;
