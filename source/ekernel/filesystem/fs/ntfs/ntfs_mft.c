 ...
 * @b. Return 0 on success or -1 on error, with fs_err set to the error
 * code.
 *
                          ...
                          const s64 count, MFT_RECORD *b)
{
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("%s: b=%p  count=%lld  mft=%llu", __FUNCTION__,
    ...
    {
        fs_err = ESPIPE;
        fs_log_perror("Trying to read non-allocated mft records "
    ...
    if (br != count)
    {
        ...
        fs_log_perror("Failed to read of MFT, mft=%llu count=%lld "
                          ...
                          MFT_RECORD *m)
{
    ...
    {
        fs_log_error("Record %llu has no FILE magic (0x%x)\n",
    ...
    {
        fs_log_error("Record %llu has corrupt allocation size "
                     "(%u <> %u)\n", (unsigned long long)MREF(mref),
                     vol->mft_record_size,
    ...
    {
        fs_log_error("Record %llu is corrupt\n",
                     (unsigned long long)MREF(mref));
        goto err_out;
    }
 ...
 * If either of these fails, -1 is returned and fs_err is set to EIO. If you get
                          ...
                          MFT_RECORD **mrec, ATTR_RECORD **attr)
{
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("%s: mrec=%p", __FUNCTION__, mrec);
        return -1;
    }
    ...
    {
        fs_log_error("Record %llu has wrong SeqNo (%d <> %d)\n",
                           ...
                           MFT_RECORD *mrec)
{
    ...
    else
    {
        ...
        {
            fs_err = ERANGE;
            fs_log_perror("Mft reference exceeds 32 bits");
            return -1;
        }
    ...
    else
    {
        ...
        fs_log_error("Sector size is bigger than MFT record size.  "
                     "Setting usa_count to 1. \n");
    }
 ...
 * @vol. Return 0 on success or -1 on error, with fs_err set to the error code.
 *
 * If any of the records exceed the initialized size of the $MFT/$DATA
 * attribute, i.e. they cannot possibly be allocated mft records, assume this
 * is a bug and return error code ESPIPE.
                           ...
                           const s64 count, MFT_RECORD *b)
{
    ...
    {
        fs_err = ESPIPE;
        fs_log_perror("Trying to write non-allocated mft records "
                      "(%lld > %lld)", (long long)m + count,
                      (long long)vol->mft_na->initialized_size >>
                      vol->mft_record_size_bits);
        return -1;
    ...
    if (bw != count)
    {
        ...
        if (bw >= 0)
            fs_log_debug("Error: partial write while writing $Mft "
                         "record(s)!\n");
        else
        {
            fs_log_perror("Error writing $Mft record(s)");
        }
        res = fs_err;
    }
    if (bmirr && bw > 0)
    {
        ...
        if (bw != cnt)
        {
            ...
            fs_log_debug("Error: failed to sync $MFTMirr! Run "
                         "chkdsk.\n");
            res = fs_err;
        }
    }
 ...
 * mft record @base_ni.
 *
 * Return the free mft record on success and -1 on error with fs_err set to the
...
static int ntfs_mft_bitmap_find_free_rec(ntfs_volume *vol, ntfs_inode *base_ni)
{
    ...
    fs_log_debug("Starting bitmap search: pass %u, pass_start 0x%llx, "
    ...
    for (; pass <= 2; size = PAGE_SIZE)
    {
        ...
        if (ll < 0)
        {
            fs_log_perror("Failed to read $MFT bitmap");
            free(buf);
            goto leave;
        }
        fs_log_debug("Read 0x%llx bytes.\n", (long long)ll);
        ...
        if (ll)
        {
            ...
            bit = data_pos & 7;
            data_pos &= ~7ull;
            fs_log_debug("Before inner for loop: size 0x%x, "
            ...
            fs_log_debug("After inner for loop: size 0x%x, "
                         "data_pos 0x%llx, bit 0x%llx, "
                         "*byte 0x%hhx, b %u.\n", size,
                         (long long)data_pos, (long long)bit,
                         byte ? *byte : -1, b);
        ...
        if (pass == 2)
        {
            ...
            fs_log_debug("pass %i, pass_start 0x%llx, pass_end "
...
static int ntfs_mft_attr_extend(ntfs_attr *na)
{
    ...
    if (!NInoAttrList(na->ni))
    {
        ...
        {
            fs_log_perror("%s: Can not add attrlist #3", __FUNCTION__);
            goto out;
        }
    ...
    {
        fs_log_perror("%s: MP update failed", __FUNCTION__);
        goto out;
    }
...
static int ntfs_mft_bitmap_extend_allocation_i(ntfs_volume *vol)
{
    ...
    {
        fs_log_error("Failed to determine last allocated "
    ...
    if (!rl2)
    {
        fs_log_error("Failed to allocate a cluster for "
                     "the mft bitmap.\n");
        return STATUS_ERROR;
    }
    ...
    {
        err = fs_err;
        fs_log_error("Failed to merge runlists for mft "
                     "bitmap.\n");
        if (ntfs_cluster_free_from_rl(vol, rl2))
            fs_log_error("Failed to deallocate "
                         "cluster.%s\n", es);
        free(rl2);
        fs_err = err;
        return STATUS_ERROR;
    ...
    mftbmp_na->rl = rl;
    fs_log_debug("Adding one run to mft bitmap.\n");
    ...
    {
        fs_log_error("Failed to find last attribute extent of "
    ...
    if (!rl2 || !rl2->length)
    {
        fs_log_error("Failed to determine previous last "
    ...
    if (mp_size <= 0)
    {
        fs_log_error("Get size for mapping pairs failed for "
    ...
    {
        fs_log_info("extending $MFT bitmap\n");
        ...
        if (ret == STATUS_ERROR)
        {
            fs_log_perror("%s: ntfs_mft_attr_extend failed", __FUNCTION__);
            update_mp = TRUE;
        }
        goto undo_alloc;
    }
    ...
    if (ntfs_mapping_pairs_build(vol, (u8 *)a +
                                 ...
                                 NULL))
    {
        fs_log_error("Failed to build mapping pairs array for "
                     "mft bitmap attribute.\n");
        fs_err = EIO;
        goto undo_alloc;
    }
    ...
    if (a->lowest_vcn)
    {
        ...
        {
            fs_log_error("Failed to find first attribute "
...
undo_alloc:
    ...
    {
        fs_log_error("Failed to free cluster.%s\n", es);
    }
    else
    {
        vol->free_clusters++;
    ...
    if (mp_rebuilt)
    {
        if (ntfs_mapping_pairs_build(vol, (u8 *)a +
                                     ...
                                     rl2, ll, NULL))
            fs_log_error("Failed to restore mapping "
                         "pairs array.%s\n", es);
        if (ntfs_attr_record_resize(m, a, old_alen))
            fs_log_error("Failed to restore attribute "
                         "record.%s\n", es);
        ntfs_inode_mark_dirty(ctx->ntfs_ino);
    }
    if (update_mp)
...
static int ntfs_mft_bitmap_extend_initialized(ntfs_volume *vol)
{
    ...
    if (ntfs_attr_lookup(mftbmp_na->type, mftbmp_na->name,
                         mftbmp_na->name_len, 0, 0, NULL, 0, ctx))
    {
        fs_log_error("Failed to find first attribute extent of "
                     "mft bitmap attribute.\n");
        err = fs_err;
        goto put_err_out;
    }
    ...
    if (ll == 8)
    {
        fs_log_debug("Wrote eight initialized bytes to mft bitmap.\n");
        vol->free_mft_records += (8 * 8);
        ret = 0;
        goto out;
    }
    fs_log_error("Failed to write to mft bitmap.\n");
    err = fs_err;
    if (ll >= 0)
    {
        err = EIO;
...
put_err_out:
    ...
    ntfs_inode_mark_dirty(ctx->ntfs_ino);
    ntfs_attr_put_search_ctx(ctx);
    fs_log_debug("Restored status of mftbmp: allocated_size 0x%llx, "
...
static int ntfs_mft_data_extend_allocation(ntfs_volume *vol)
{
    ...
    fs_log_trace01("Extending mft data allocation.\n");
    ...
    lcn = rl->lcn + rl->length;
    fs_log_debug("Last lcn of mft data attribute is 0x%llx.\n", (long long)lcn);
    ...
    do
    {
        ...
        if (fs_err != ENOSPC || nr == min_nr)
        {
            fs_log_perror("Failed to allocate (%lld) clusters "
         ...
         */
        nr = min_nr;
        fs_log_debug("Retrying mft data allocation with minimal cluster "
                     "count %lli.\n", (long long)nr);
    } while (1);

    fs_log_debug("Allocated %lld clusters.\n", (long long)nr);

    rl = ntfs_runlists_merge(mft_na->rl, rl2);
    if (!rl)
    {
        err = fs_err;
        fs_log_error("Failed to merge runlists for mft data "
                     "attribute.\n");
        if (ntfs_cluster_free_from_rl(vol, rl2))
            fs_log_error("Failed to deallocate clusters "
    ...
    if (ntfs_mapping_pairs_build(vol,
                                 ...
                                 rl2, ll, NULL))
    {
        fs_log_error("Failed to build mapping pairs array of "
                     "mft data attribute.\n");
        fs_err = EIO;
        goto undo_alloc;
    }
...
undo_alloc:
    ...
    if (ntfs_cluster_free(vol, mft_na, old_last_vcn, -1) < 0)
        fs_log_error("Failed to free clusters from mft data "
                     "attribute.%s\n", es);
    if (ntfs_rl_truncate(&mft_na->rl, old_last_vcn))
        fs_log_error("Failed to truncate mft data attribute "
                     "runlist.%s\n", es);
    if (mp_rebuilt)
    {
        if (ntfs_mapping_pairs_build(vol, (u8 *)a +
                                     ...
                                     old_alen - le16_to_cpu(a->mapping_pairs_offset),
                                     rl2, ll, NULL))
            fs_log_error("Failed to restore mapping pairs "
                         "array.%s\n", es);
        if (ntfs_attr_record_resize(m, a, old_alen))
            fs_log_error("Failed to restore attribute "
                         "record.%s\n", es);
...
static int ntfs_mft_record_init(ntfs_volume *vol, s64 size)
{
     ...
     */
    fs_log_debug("Status of mft data before extension: "
    ...
    while (size > mft_na->allocated_size)
    {
        ...
        fs_log_debug("Status of mft data after allocation extension: "
                     "allocated_size 0x%llx, data_size 0x%llx, "
                     "initialized_size 0x%llx.\n",
                     (long long)mft_na->allocated_size,
                     (long long)mft_na->data_size,
    ...
    while (size > mft_na->initialized_size)
    {
        ...
        fs_log_debug("Initializing mft record 0x%llx.\n", (long long)ll2);
        if (ntfs_mft_record_format(vol, ll2) < 0)
        {
            fs_log_perror("Failed to format mft record");
            goto undo_data_init;
        }
    }
    ...
    ntfs_inode_mark_dirty(ctx->ntfs_ino);
    ntfs_attr_put_search_ctx(ctx);
    fs_log_debug("Status of mft data after mft record initialization: "
                 "allocated_size 0x%llx, data_size 0x%llx, "
                 "initialized_size 0x%llx.\n",
                 (long long)mft_na->allocated_size,
                 (long long)mft_na->data_size,
    ...
    {
        fs_log_error("mft_na sanity checks failed");
    }
...
static int ntfs_mft_rec_init(ntfs_volume *vol, s64 size)
{
    ...
    {
        fs_err = EIO;
        fs_log_perror("%s: unexpected $MFT sizes, see below", __FUNCTION__);
        fs_log_error("$MFT: size=%lld  allocated_size=%lld  "
...
static ntfs_inode *ntfs_mft_rec_alloc(ntfs_volume *vol)
{
    ...
    fs_log_error("No free mft record for $MFT: %s\n", strerror(fs_err));
    goto err_out;

found_free_rec:
    if (ntfs_bitmap_set_bit(mftbmp_na, bit))
    {
        fs_log_error("Failed to allocate bit in mft bitmap #2\n");
        goto err_out;
    }
    ...
    {
        fs_log_error("Inode %lld is used but it wasn't marked in "
                     "$MFT bitmap. Fixed.\n", (long long)bit);
        free(m);
        goto undo_mftbmp_alloc;
    }
    ...
    {
        fs_log_error("Failed to re-format mft record.\n");
        free(m);
        goto undo_mftbmp_alloc;
    }
    ...
    if (!ni)
    {
        fs_log_error("Failed to allocate buffer for inode.\n");
        free(m);
        goto undo_mftbmp_alloc;
    }
    ni->mft_no = bit;
    ...
    fs_log_error("allocated %sinode %lld\n",
...
undo_mftbmp_alloc:
    ...
    {
        fs_log_error("Failed to clear bit in mft bitmap.%s\n", es);
    }
    fs_err = err;
err_out:
    if (!fs_err)
...
ntfs_inode *ntfs_mft_record_alloc(ntfs_volume *vol, ntfs_inode *base_ni)
{
    ...
    if (base_ni)
        fs_log_trace01("Entering (allocating an extent mft record for "
                       "base mft record %lld).\n",
                       (long long)base_ni->mft_no);
    else
    {
        fs_log_trace01("Entering (allocating a base mft record)\n");
    }
...
retry:
    ...
    if (bit >= 0)
    {
        fs_log_debug("found free record (#1) at %lld\n",
                     (long long)bit);
        goto found_free_rec;
    }
    if (fs_err != ENOSPC)
        ...
        mftbmp_na->initialized_size > RESERVED_MFT_RECORDS / 8)
    {
        ...
        fs_log_debug("found free record (#2) at %lld\n",
                     (long long)bit);
        goto found_free_rec;
    }
    /*
     ...
     */
    fs_log_debug("Status of mftbmp before extension: allocated_size 0x%llx, "
                 "data_size 0x%llx, initialized_size 0x%llx.\n",
                 (long long)mftbmp_na->allocated_size,
                 (long long)mftbmp_na->data_size,
                 (long long)mftbmp_na->initialized_size);
    if (mftbmp_na->initialized_size + 8 > mftbmp_na->allocated_size)
    {
        ...
        fs_log_debug("Status of mftbmp after allocation extension: "
                     "allocated_size 0x%llx, data_size 0x%llx, "
                     "initialized_size 0x%llx.\n",
                     (long long)mftbmp_na->allocated_size,
                     (long long)mftbmp_na->data_size,
    ...
    fs_log_debug("Status of mftbmp after initialized extension: "
                 "allocated_size 0x%llx, data_size 0x%llx, "
                 "initialized_size 0x%llx.\n",
                 (long long)mftbmp_na->allocated_size,
                 (long long)mftbmp_na->data_size,
                 (long long)mftbmp_na->initialized_size);
    fs_log_debug("found free record (#3) at %lld\n", (long long)bit);
found_free_rec:
    /* @bit is the found free mft record, allocate it in the mft bitmap. */
    if (ntfs_bitmap_set_bit(mftbmp_na, bit))
    {
        fs_log_error("Failed to allocate bit in mft bitmap.\n");
        goto err_out;
    }
    ...
    {
        fs_log_error("Failed to allocate buffer for inode 2.\n");
    }
    ...
    /* Return the opened, allocated inode of the allocated mft record. */
    fs_log_debug("allocated %sinode 0x%llx.\n",
                 base_ni ? "extent " : "", (long long)bit);
...
bitmap_rollback:
    if (ntfs_bitmap_set_bit(vol->mftbmp_na, mft_no))
        fs_log_debug("Eeek! Rollback failed in ntfs_mft_record_free().  "
 ...
 *
 * On success return 0 and on error return -1 with fs_err set.
 */
