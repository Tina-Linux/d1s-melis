 ...
 *
 * Return 0 on success and -1 on error with fs_err set to the error code from
 * ntfs_attr_lookup().
 *
 ...
 *  if (!ctx)
 *      // Error code is in fs_err. Handle this case.
 ...
 *  }
 *  if (err && fs_err != ENOENT)
 *      // Ooops. An error occurred! You should handle this case.
 *  // Now finished with all attributes in the inode.
 */
static inline int ntfs_attrs_walk(ntfs_attr_search_ctx *ctx)
{
 ...
 * If we return 0, check fs_err. If fs_err is 0 the actual length was 0,
 * otherwise fs_err describes the error.
 *
 * FIXME: Describe possible fs_errs.
 */
 ...
 *
 * Return number of bytes copied. If this is zero check fs_err. If fs_err is 0
 * then nothing was read due to a zero-length attribute value, otherwise
 * fs_err describes the error.
 */
