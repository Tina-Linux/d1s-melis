                                       ...
                                       s64 count, int value)
{
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("%s: Invalid argument (%p, %lld, %lld)",
    ...
    do
    {
        ...
        if (count > 0 && bit)
        {
            ...
            {
                // FIXME: Eeek! BUG!
                fs_log_error("Lastbyte is zero. Leaving "
            ...
            if (lastbyte_pos <= bufsize)
            {
                ...
                if (br != 1)
                {
                    ...
                    fs_log_perror("Reading of last byte "
        ...
        if (br != bufsize)
        {
            ...
            fs_log_perror("Failed to write buffer to bitmap "
        ...
        {
            // FIXME: Eeek! BUG!
            fs_log_error("Last buffer but count is not zero "
...
int ntfs_bitmap_set_run(ntfs_attr *na, s64 start_bit, s64 count)
{
    ...
    fs_log_trace01("Set from bit %lld, count %lld\n",
...
int ntfs_bitmap_clear_run(ntfs_attr *na, s64 start_bit, s64 count)
{
    ...
    fs_log_trace01("Clear from bit %lld, count %lld\n",
                   (long long)start_bit, (long long)count);
