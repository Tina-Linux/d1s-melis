                           ...
                           u8 *const cb_start, const u32 cb_size)
{
    ...
    fs_log_trace01("Entering, cb_size = 0x%x.\n", (unsigned)cb_size);
do_next_sb:
    fs_log_debug("Beginning sub-block at offset = %d in the cb.\n",
                 (int)(cb - cb_start));
    /*
    ...
    {
        fs_log_debug("Completed. Returning success (0).\n");
        return 0;
    }
    ...
    {
        fs_log_debug("Found uncompressed sub-block.\n");
    ...
    fs_log_debug("Found compressed sub-block.\n");
...
do_next_tag:
    if (cb == cb_sb_end)
    {
        ...
        if (dest < dest_sb_end)
        {
            ...
            fs_log_debug("Filling incomplete sub-block with zeroes.\n");
...
return_overflow:
    fs_err = EOVERFLOW;
    fs_log_perror("Failed to decompress file");
    return -1;
}
...
s64 ntfs_compressed_attr_pread(ntfs_attr *na, s64 pos, s64 count, void *b)
{
    ...
    fs_log_trace01("Entering for inode 0x%llx, attr 0x%x, pos 0x%llx, count 0x%llx.\n",
...
do_next_cb:
    ...
    if (rl->lcn == LCN_HOLE)
    {
        ...
        fs_log_debug("Found sparse compression block.\n");
    ...
    else if (!ntfs_is_cb_compressed(na, rl, vcn, cb_clusters))
    {
         ...
         * overlapping the cb.
         */
        fs_log_debug("Found uncompressed compression block.\n");
        /*
    ...
    else
    {
         ...
         */
        fs_log_debug("Found compressed compression block.\n");
        /*
        ...
        fs_log_debug("Successfully read the compression block.\n");
