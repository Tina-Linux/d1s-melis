...
static int ntfs_mft_load(ntfs_volume *vol)
{
    ...
    if (!vol->mft_ni || !mb)
    {
        fs_log_perror("Error allocating memory for $MFT");
        goto error_exit;
    }
    vol->mft_ni->mft_no = 0;
    vol->mft_ni->mrec = mb;
    /* Can't use any of the higher level functions yet! */
    l = ntfs_mst_pread(vol->_part, vol->mft_lcn << vol->cluster_size_bits, 1,
    ...
    if (l != 1)
    {
        ...
        fs_log_perror("Error reading $MFT");
        goto error_exit;
    }
                         ...
                         ctx))
    {
        if (fs_err != ENOENT)
        {
            fs_log_error("$MFT has corrupt attribute list.\n");
            goto io_error_exit;
        }
    ...
    if (l <= 0 || l > 0x40000)
    {
        fs_log_error("$MFT/$ATTR_LIST invalid length (%lld).\n",
                     (long long)l);
        goto io_error_exit;
    }
    ...
    if (!l)
    {
        fs_log_error("Failed to get value of $MFT/$ATTR_LIST.\n");
        goto io_error_exit;
    }
    if (l != vol->mft_ni->attr_list_size)
    {
        fs_log_error("Partial read of $MFT/$ATTR_LIST (%lld != "
                     "%u).\n", (long long)l,
...
mft_has_no_attr_list:
    ...
    if (!vol->mft_na)
    {
        fs_log_perror("Failed to open ntfs attribute");
        goto error_exit;
    }
                             ...
                             ctx))
    {
        ...
        if (!a->non_resident)
        {
            fs_log_error("$MFT must be non-resident.\n");
            goto io_error_exit;
        }
        ...
        {
            fs_log_error("$MFT must be uncompressed and "
                         "unencrypted.\n");
            goto io_error_exit;
        }
        /*
        ...
        if (!nrl)
        {
            fs_log_perror("ntfs_mapping_pairs_decompress() failed");
            goto error_exit;
        }
        vol->mft_na->rl = nrl;
    ...
    if (!a)
    {
        fs_log_error("$MFT/$DATA attribute not found.\n");
        goto io_error_exit;
    }
    if (highest_vcn && highest_vcn != last_vcn - 1)
    {
        fs_log_error("Failed to load runlist for $MFT/$DATA.\n");
        fs_log_error("highest_vcn = 0x%llx, last_vcn - 1 = 0x%llx\n",
    ...
    if (!vol->mftbmp_na)
    {
        fs_log_perror("Failed to open $MFT/$BITMAP");
        goto error_exit;
    }
    return 0;
io_error_exit:
...
static int ntfs_mftmirr_load(ntfs_volume *vol)
{
    ...
    if (!vol->mftmirr_ni)
    {
        fs_log_perror("Failed to open inode $MFTMirr");
        return -1;
    }
    ...
    if (!vol->mftmirr_na)
    {
        fs_log_perror("Failed to open $MFTMirr/$DATA");
        goto error_exit;
    }

    if (ntfs_attr_map_runlist(vol->mftmirr_na, 0) < 0)
    {
        fs_log_perror("Failed to map runlist of $MFTMirr/$DATA");
        goto error_exit;
    }
 ...
 */
ntfs_volume *ntfs_volume_startup(__hdle part, u32 flags)
{
    ...
    br = ntfs_pread(part, 0, sizeof(NTFS_BOOT_SECTOR), bs);
    if (br != sizeof(NTFS_BOOT_SECTOR))
    {
        ...
        if (!br)
        {
            fs_log_error("Failed to read bootsector (size=0)\n");
        }
        else
        {
            fs_log_perror("Error reading bootsector");
        }
        goto error_exit;
    }
    ...
    if (ntfs_device_block_size_set(part, vol->sector_size))
        fs_log_debug("Failed to set the device block size to the "
    ...
    fs_log_debug("mft_zone_pos = 0x%llx\n", (long long)vol->mft_zone_pos);
    ...
    fs_log_debug("mft_zone_start = 0x%llx\n", (long long)vol->mft_zone_start);
    ...
    fs_log_debug("mft_zone_end = 0x%llx\n", (long long)vol->mft_zone_end);
    ...
    fs_log_debug("data1_zone_pos = %lld\n", (long long)vol->data1_zone_pos);
    vol->data2_zone_pos = 0;
    fs_log_debug("data2_zone_pos = %lld\n", (long long)vol->data2_zone_pos);
    ...
    {
        fs_log_perror("Failed to load $MFT");
        goto error_exit;
    }
    ...
    {
        fs_log_perror("Failed to load $MFTMirr");
        goto error_exit;
    }
    return vol;
error_exit:
...
static int ntfs_read_root(struct inode *inode)
{
    ...
    ntfs_volume *vol = NTFS_SB(inode->i_sb);
    ntfs_attr *ia_na;
    ...
    if (!ia_na)
    {
        if (fs_err != ENOENT)
        {
            fs_log_perror("Failed to open index allocation "
                          "attribute. Directory inode 0x%x is "
                          "corrupt or bug", (unsigned int)
                          inode->i_ino);
            return -1;
        }
        inode->i_size = 0;
    ...
    {
        inode->i_size = ia_na->data_size;
    }
    ntfs_attr_close(ia_na);

    inode->i_blocks = ((inode->i_size + (vol->cluster_size - 1))
                       & ~((__s64)vol->cluster_size - 1)) >> 9;
 ...
 *
 * Return 0 on success. On error return -1 with fs_err set appropriately and
 * @vol->free_mft_records is not touched in this case.
 */
 ...
 * vol:     ntfs volume for which perform calculations.
 *
 * This function initializes @vol->free_clusters. @vol->lcnbmp_na should be
 * already opened upon call to this function.
 *
 * Return 0 on success. On error return -1 with fs_err set appropriately and
 * @vol->free_clusters is not touched in this case.
 */
 ...
 * fs_err set to the error code.
 */
__s32 ntfs_fill_super(struct super_block *sb, void *data, __s32 silent)
{
    s64 l;
    u32 flags = *(u32 *)data;
     ...
     */
    sctsize = dev_hardsect_size(sb->s_part);
    ...
    if (ntfs_device_block_size_set(sb->s_part, sctsize))
    {
        fs_log_error("Failed to set the device block size.");
        return EPDK_FALSE;
    }

    vol = ntfs_volume_startup(sb->s_part, flags);
    if (!vol)
    {
        return 0;
    }
    ...
    sb->s_op = &ntfs_sops;
    sb->s_blocksize_bits = vol->sector_size_bits;
    sb->s_blocksize = vol->sector_size;
    ...
    if (l != vol->mftmirr_size)
    {
        if (l == -1)
        {
            fs_log_perror("Failed to read $MFT");
        }
        else
        {
            fs_log_error("Failed to read $MFT, unexpected length "
                         "(%lld != %d).\n", (long long)l,
                         vol->mftmirr_size);
            fs_err = EIO;
        }
    ...
    if (l != vol->mftmirr_size)
    {
        if (l == -1)
        {
            fs_log_perror("Failed to read $MFTMirr");
            goto error_exit;
        }
        vol->mftmirr_size = l;
    }
    fs_log_debug("Comparing $MFTMirr to $MFT...\n");
    for (i = 0; i < vol->mftmirr_size; ++i)
    {
        ...
        if (mrec->flags & MFT_RECORD_IN_USE)
        {
            ...
            {
                fs_log_error("$MFT error: Incomplete multi "
            ...
            {
                fs_log_error("$MFT error: Invalid mft "
                             "record for '%s'.\n", s);
                goto io_error_exit;
            }
        }
        ...
        if (mrec2->flags & MFT_RECORD_IN_USE)
        {
            ...
            {
                fs_log_error("$MFTMirr error: Incomplete "
            ...
            {
                fs_log_error("$MFTMirr error: Invalid mft "
                             "record for '%s'.\n", s);
                goto io_error_exit;
            }
        }
        ...
        {
            fs_log_error("$MFTMirr does not match $MFT (record "
                         "%d).\n", i);
            goto io_error_exit;
        }
    }
    ...
    fs_log_debug("Loading $Bitmap...\n");
    vol->lcnbmp_ni = ntfs_inode_open(vol, FILE_Bitmap);
    if (!vol->lcnbmp_ni)
    {
        fs_log_perror("Failed to open inode FILE_Bitmap");
        goto error_exit;
    }
    ...
    {
        fs_log_error("Corrupt cluster map size (%lld > %lld)\n",
    ...
    fs_log_debug("Loading $UpCase...\n");
    ni = ntfs_inode_open(vol, FILE_UpCase);
    if (!ni)
    {
        fs_log_perror("Failed to open inode FILE_UpCase");
        goto error_exit;
    }
    ...
    {
        fs_log_error("Error: Upcase table is too big (max 32-bit "
                     "allowed).\n");
        fs_err = EINVAL;
        goto error_exit;
    }
    ...
    if (l != na->data_size)
    {
        fs_log_error("Failed to read $UpCase, unexpected length "
                     "(%lld != %lld).\n", (long long)l,
    ...
    {
        fs_log_perror("Failed to close $UpCase");
        goto error_exit;
    }
     ...
     */
    fs_log_debug("Loading $Volume...\n");
    vol->vol_ni = ntfs_inode_open(vol, FILE_Volume);
    if (!vol->vol_ni)
    {
        fs_log_perror("Failed to open inode FILE_Volume");
        goto error_exit;
    }
    ...
    if (ntfs_attr_lookup(AT_VOLUME_INFORMATION, AT_UNNAMED, 0, 0, 0, NULL,
                         0, ctx))
    {
        fs_log_perror("$VOLUME_INFORMATION attribute not found in "
                      "$Volume");
        goto error_exit;
    }
    a = ctx->attr;
    ...
    if (a->non_resident)
    {
        fs_log_error("Attribute $VOLUME_INFORMATION must be "
    ...
    {
        fs_log_error("$VOLUME_INFORMATION in $Volume is corrupt.\n");
        fs_err = EIO;
        goto error_exit;
    }
                         ...
                         ctx))
    {
        if (fs_err != ENOENT)
        {
            fs_log_perror("Failed to lookup of $VOLUME_NAME in "
                          "$Volume failed");
            goto error_exit;
        }
        /*
    ...
    else
    {
        ...
        if (a->non_resident)
        {
            fs_log_error("$VOLUME_NAME must be resident.\n");
            fs_err = EIO;
            goto error_exit;
        }
        /* Get a pointer to the value of the attribute. */
        ...
        {
            fs_log_perror("Volume name could not be converted "
                          "to current locale");
            fs_log_debug("Forcing name into ASCII by replacing "
    ...
    fs_log_debug("Loading $AttrDef...\n");
    ni = ntfs_inode_open(vol, FILE_AttrDef);
    if (!ni)
    {
        fs_log_perror("Failed to open $AttrDef");
        goto error_exit;
    }
    ...
    {
        fs_log_error("Attribute definition table is too big (max "
                     "32-bit allowed).\n");
        fs_err = EINVAL;
        goto error_exit;
    }
    ...
    if (l != na->data_size)
    {
        fs_log_error("Failed to read $AttrDef, unexpected length "
                     "(%lld != %lld).\n", (long long)l,
                     (long long)na->data_size);
        fs_err = EIO;
        goto error_exit;
    ...
    if (ntfs_inode_close(ni))
    {
        fs_log_perror("Failed to close $AttrDef");
        goto error_exit;
    }
    /* Initialize number of free clusters and MFT records. */
    //  if (ntfs_volume_get_nr_free_mft_records(vol)) {
    //      fs_log_perror("Failed to calculate number of free MFTs");
    //      goto error_exit;
    //  }
    //  if (ntfs_volume_get_nr_free_clusters(vol)) {
    //      fs_log_perror("Failed to calculate number of free clusters");
    //      goto error_exit;
    //  }
    /*
    ...
    if (!(flags & MS_RDONLY))
    {
        ...
        if (ntfs_volume_check_logfile(vol) < 0)
        {
            ...
            fs_log_info("The file system wasn't safely "
    ...
    /* from now on, ntfs_inode_open() will use vfs::new_inode() to alloc inode */
    vol->v_sb = sb;

    ni = ntfs_inode_open(vol, FILE_root);
    if (!ni)
    {
        fs_log_perror("Failed to open inode");
        goto error_exit;
    }
    ...
    fs_err = ntfs_read_root(root_inode);
    if (fs_err < 0)
    {
        goto error_exit;
    }
    ...
    if (!sb->s_root)
    {
        fs_log_error("NTFS: get root inode failed\n");
        goto error_exit;
    }

    if (vol->vol_name[0])
    {
        strncpy(sb->s_volname, vol->vol_name, MAX_VOLUME_NAME_LEN - 1);
    }
...
int ntfs_part_identify(__hdle part)
{
    s64 br;
    NTFS_BOOT_SECTOR *bs;
    int res;

    bs = (NTFS_BOOT_SECTOR *)palloc(sizeof(NTFS_BOOT_SECTOR));
    if (!bs)
    {
        return 0;
    }
    ...
    if (br != sizeof(NTFS_BOOT_SECTOR))
    {
        ...
        if (!br)
            fs_log_debug("Error: partition is smaller than "
    ...
    if (!ntfs_boot_sector_is_ntfs(bs))
    {
        fs_log_debug("Error: not a valid NTFS partition!\n");
        fs_err = EINVAL;
        res = 0;
        goto error_exit;
    }
    ...
    res = 1;
error_exit:
    pfree(bs, sizeof(NTFS_BOOT_SECTOR));
...
int ntfs_statfs(struct super_block *sb, struct kstatfs *buf, __u32 flags)
{
    ntfs_volume *vol;
    int error = 0;
    ...
    {
        /* get free mft record number if necessary */
        if (vol->free_mft_records <= 0)
        {
            if (ntfs_volume_get_nr_free_mft_records(vol))
            {
                fs_log_warning("Failed to calculate number of free MFTs");
            }
        }
        /* get free clusters number if necessary */
        if (vol->free_clusters <= 0)
        {
            if (ntfs_volume_get_nr_free_clusters(vol))
            {
                fs_log_warning("Failed to calculate number of free clusters");
            }
        }
    }
    ...
    buf->f_namelen  = vol->upcase_len << 1;
    strcpy(buf->f_volname, sb->s_volname);
    strcpy(buf->f_fsname, "NTFS");
...
static ntfs_inode *ntfs_hiberfile_open(ntfs_volume *vol)
{
    ...
    if (!ni_root)
    {
        fs_log_debug("Couldn't open the root directory.\n");
        return NULL;
    }
    ...
    if (unicode_len < 0)
    {
        fs_log_perror("Couldn't convert 'hiberfil.sys' to Unicode");
        goto out;
    }
    ...
    if (inode == (u64) - 1)
    {
        fs_log_debug("Couldn't find file '%s'.\n", hiberfile);
        goto out;
    }
    ...
    if (!ni_hibr)
    {
        fs_log_debug("Couldn't open inode %lld.\n", (long long)inode);
        goto out;
    }
out:
 ...
 *         -1 otherwise and fs_err is set to the appropriate value
 */
int ntfs_volume_check_hiberfile(ntfs_volume *vol, int verbose)
{
    ...
    if (!na)
    {
        fs_log_perror("Failed to open hiberfil.sys data attribute");
        goto out;
    }
    ...
    if (bytes_read == -1)
    {
        fs_log_perror("Failed to read hiberfil.sys");
        goto out;
    }
    if (bytes_read < NTFS_HIBERFILE_HEADER_SIZE)
    {
        if (verbose)
            fs_log_error("Hibernated non-system partition, "
                         "refused to mount.\n");
        fs_err = EPERM;
        goto out;
    }
    if (memcmp(buf, "hibr", 4) == 0)
    {
        if (verbose)
        {
            fs_log_error("Windows is hibernated, refused to mount.\n");
        }
        fs_err = EPERM;
        goto out;
    }
 ...
 *
 * Return 0 on success and -1 on error with fs_err set error code.
 */
static int ntfs_volume_check_logfile(ntfs_volume *vol)
{
    ntfs_inode *ni;
    ...
    if (!ni)
    {
        fs_log_perror("Failed to open inode FILE_LogFile");
        fs_err = EIO;
        return -1;
    }
    ...
    if (!na)
    {
        fs_log_perror("Failed to open $FILE_LogFile/$DATA");
        err = EIO;
        goto out;
    }
 ...
 *
 * Return 0 if NTFS version is supported otherwise -1 with fs_err set.
 *
 ...
 *
 * Return 0 if successful and -1 if not with fs_err set to the error code.
 */
int ntfs_volume_write_flags(ntfs_volume *vol, const u16 flags)
{
    ...
    if (ntfs_attr_lookup(AT_VOLUME_INFORMATION, AT_UNNAMED, 0, 0, 0, NULL,
                         0, ctx))
    {
        fs_log_error("Attribute $VOLUME_INFORMATION was not found "
                     "in $Volume!\n");
        goto err_out;
    }
    a = ctx->attr;
    ...
    if (a->non_resident)
    {
        fs_log_error("Attribute $VOLUME_INFORMATION must be resident "
                     "but it isn't.\n");
        fs_err = EIO;
        goto err_out;
    }
    ...
    {
        fs_log_error("Attribute $VOLUME_INFORMATION in $Volume is "
                     "corrupt!\n");
        fs_err = EIO;
        goto err_out;
    }
...
void ntfs_mount_error(const char *volume, const char *mntpoint, int err)
{
    switch (err)
    {
        case NTFS_VOLUME_NOT_NTFS:
            fs_log_error(invalid_ntfs_msg, volume);
            break;
        case NTFS_VOLUME_CORRUPT:
            fs_log_error("%s", corrupt_volume_msg);
            break;
        case NTFS_VOLUME_HIBERNATED:
            fs_log_error(hibernated_volume_msg, volume, mntpoint);
            break;
        case NTFS_VOLUME_UNCLEAN_UNMOUNT:
            fs_log_error("%s", unclean_journal_msg);
            break;
        case NTFS_VOLUME_LOCKED:
            fs_log_error("%s", opened_volume_msg);
            break;
        case NTFS_VOLUME_RAID:
            fs_log_error("%s", fakeraid_msg);
            break;
        case NTFS_VOLUME_NO_PRIVILEGE:
            fs_log_error(access_denied_msg, volume);
            break;
    }
}
#endif  /* FSYS_NTFS_RW */
...
static const struct super_operations ntfs_sops =
{
    .alloc_inode    = ntfs_big_inode_alloc,   /* VFS: Allocate new inode. */
    .destroy_inode    = ntfs_big_inode_destroy,   /* VFS: Allocate new inode. */
    .statfs     = ntfs_statfs,
    .put_super  = ntfs_put_super,   /* Syscall: umount. */
    .drop_inode = ntfs_drop_inode,
};
