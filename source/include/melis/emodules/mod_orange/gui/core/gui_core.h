...
/**********************************************************************
    macros,   malloc       mcl add  for epdk
*/

/**********************************************************************
*                  BMP  alpha enable    // mcl add  2008.12.01
*/
#define  BMP_ALPHA_ENABLE  0xffffffff

/**********************************************************************
*                  select screen    // added by Derek,2020-12-13 15:43:49
*/
#define SEL_SCREEN  0x0
...
#define GUI_CYAN          0xffFFFF00
#define GUI_MAGENTA       0xffFF00FF
#define GUI_YELLOW        0xff00FFFF
#define GUI_LIGHTBLUE     0xffFF8080
#define GUI_LIGHTGREEN    0xff80FF80
#define GUI_LIGHTRED      0xff8080FF
#define GUI_LIGHTCYAN     0xffFFFF80
#define GUI_LIGHTMAGENTA  0xffFF80FF
#define GUI_LIGHTYELLOW   0xff80FFFF
#define GUI_DARKBLUE      0xff800000
#define GUI_DARKGREEN     0xff008000
#define GUI_DARKRED       0xff000080
#define GUI_DARKCYAN      0xff808000
#define GUI_DARKMAGENTA   0xff800080
#define GUI_DARKYELLOW    0xff008080
...
/*AA Fonts*/
extern GUI_CONST_STORAGE GUI_FONT GUI_FontFixedsys16AA4;
#endif
/*
      *********************************
      *                               *
...
#define GUI_DRAWMODE_SHADOW     LCD_DRAWMODE_SHADOW
...
//typedef  struct  FB   BIT_STRING;

typedef union
{
    U8  aColorIndex8[2];
    U16 aColorIndex16[2];
    U32 aColorIndex32[2];    // mcl add for aRGB
} LCD_COLORINDEX_UNION;

/* mcl  add for LCD screen device*/
typedef struct
{
    int LCDXSize[GUI_NUM_LAYERS];
    int  LCDYSize[GUI_NUM_LAYERS];
    U8  LCDDirection[GUI_NUM_LAYERS];  /*LCDDirection=0, H screen; LCDDirection=1, v screen */
} SCREEN_CONTEXT;

typedef struct
{
    int layerState[GUI_NUM_LAYERS];
    int  colorMode[GUI_NUM_LAYERS];
...
typedef struct
{
    ...
    LCD_RECT       ClipRect[GUI_NUM_LAYERS];
    U8             DrawMode;
    U8             FontMode;
    U8             SelLayer;
    U8             TextStyle;
    /* Variables in GL module */
    GUI_RECT *pClipRect_HL[GUI_NUM_LAYERS];                /* High level clip rectangle ... Speed optimization so drawing routines can optimize */
    U8        PenSize;
    U8        PenShape;
    U8        LineStyle;
    U8        FillStyle;
    ...
    I16P LBorder;
    I16P DispPosX[GUI_NUM_LAYERS], DispPosY[GUI_NUM_LAYERS];
    I16P DrawPosX[GUI_NUM_LAYERS], DrawPosY[GUI_NUM_LAYERS];
    I16P TextMode, TextAlign;
    GUI_COLOR Color, BkColor;           /* Required only when changing devices and for speed opt (caching) */
    __epdk_charset_enm_e charset_enum;    // add by Derek  2010.11.03
    /* Variables in WM module */
#if GUI_WINSUPPORT
    const GUI_RECT *WM__pUserClipRect;
    GUI_HWIN hAWin;
    int xOff[GUI_NUM_LAYERS];
    int yOff[GUI_NUM_LAYERS];
#endif
...
U8           GUI_Set_FrameColor_8bpp32(U8 frameColor);
const char *GUI_GetVersionString(void);
void         GUI_SaveContext(GUI_CONTEXT *pContext);
void         GUI_RestoreContext(const GUI_CONTEXT *pContext);

void        GUI_OpenAlphaBlend(void);
void        GUI_CloseAlphaBlend(void);
/*********************************************************************
*
...
const GUI_RECT *GUI_SetClipRect(const GUI_RECT *pRect);      //mcl  2008.12.18
...
int  GUI_BMP_DrawEx(const void *pFileData, int x0, int y0);
int GUI_BMP_Draw_16bpp(const void *pBMP, int x0, int y0);
int GUI_BitString_Draw(FB *fb, int x0, int y0);      // mcl add for fast draw ARGB 2008.11.18
int GUI_BitString_DrawEx(FB *fb, int x0, int y0);   // mcl add for accelerate drawing speed
#if defined(CONFIG_SOC_SUN3IW2) || defined(CONFIG_SOC_SUN8IW19) || defined (CONFIG_SOC_SUN20IW1)
int GUI_BitString_Draw2(FB *fb, int dst_x, int dst_y, int src_width, int src_height);//add by bayden, 支持指定源宽度大小的绘制
#endif
int GUI_ARGB_Draw(const void *pBMP, int x0, int y0);  // mcl add 2008.12.04
int GUI_MyDrawBMP(const U8 *pData, int x0, int y0, int XSize, int YSize);  // added by MCL for Game
int  GUI_BMP_Draw_Trans(const void *pBMP, int x0, int y0, GUI_COLOR  color, U8 index); //added by MCL  for bmp trans
...
// 2008/07/11 11:25:43   mcl  add  GUI_DrawBitmapEx declare
void GUI_DrawBitmapEx_Trans(const GUI_BITMAP GUI_UNI_PTR *pBitmap, int x0, int y0, int xCenter, int yCenter, int xMag, int yMag, int color);
//end
...
void GUI_GetRectMem(GUI_RECT *pSrc, void *pBuffer, U32 bufferXSize, U32 bufferYSize, U32 rectX0, U32 rectY0);
void GUI_SetRectMemEx(GUI_RECT *pDestRect, void *pBuffer, U32 bufferXSize, U32 bufferYSize, U32 rectX0, U32 rectY0);
void GUI_GetRectMemEx(GUI_RECT *pSrc, void *pBuffer, U32 bufferXSize, U32 bufferYSize, U32 rectX0, U32 rectY0);
/*********************************************************************
*
*       JPEG support
*
...
__s32  GUI_DispStringInRectWrap(const char GUI_UNI_PTR *s, GUI_RECT *pRect, int TextAlign, GUI_WRAPMODE WrapMode);     //mcl add
...
void  GUI_UC_SetEncodeISO885916(void);
void  GUI_UC_SetEncodeISO885915(void);
void  GUI_UC_SetEncodeISO885914(void);
void  GUI_UC_SetEncodeISO885913(void);
void  GUI_UC_SetEncodeISO885912(void);
void  GUI_UC_SetEncodeISO885911(void);
void  GUI_UC_SetEncodeISO885910(void);
void  GUI_UC_SetEncodeISO88599(void);
void  GUI_UC_SetEncodeISO88598(void);
void  GUI_UC_SetEncodeISO88597(void);
void  GUI_UC_SetEncodeISO88596(void);
void  GUI_UC_SetEncodeISO88595(void);
void  GUI_UC_SetEncodeISO88594(void);
void  GUI_UC_SetEncodeISO88593(void);
void  GUI_UC_SetEncodeISO88592(void);
void  GUI_UC_SetEncodeISO88591(void);
void  GUI_UC_SetEncodeKIO8R(void);
void  GUI_UC_SetEncodeEUCKR(void);
void  GUI_UC_SetEncodeEUCJP(void);
void  GUI_UC_SetEncodeCP874(void);
void  GUI_UC_SetEncodeCP1250(void);
void  GUI_UC_SetEncodeCP1251(void);
void  GUI_UC_SetEncodeCP1253(void);
void  GUI_UC_SetEncodeCP1255(void);
void  GUI_UC_SetEncodeCP1256(void);
void  GUI_UC_SetEncodeCP1257(void);
void  GUI_UC_SetEncodeCP1258(void);
...
int GUI_CreateLayer(void *pFBAddr, unsigned int width, unsigned int height, int colorMode); // mcl add
int GUI_ReleaseLayer(int index);     // mcl add
...
/*********************************************************************
*
*         layer format convert
*/ //add by Derek,2010-12-13 15:40:51
...
GUI_FONT *GUI_GetDefaultFont(void); //added be Derek,2010-12-14 19:55:21
U8 GUI_SetFrameColor8bpp32(U8 frameColor);
#ifdef CONFIG_SOC_SUN3IW1
unsigned int GUI_SetFrameColor32bpp(unsigned int frameColor);
#endif
