...
*                                            All Rights Reserved
*
* File      : dtv_setting_search_general.c
* By        : hot.lee
...
typedef struct tag_dtv_setting_search_focus_item_id
{
    __s32  id_bmp_focus_item;
    __s32  id_bmp_unfocus_item;
} dtv_setting_search_focus_item_id_t;

typedef struct tag_dtv_setting_search_attr
{

    HTHEME      h_bg_bmp;
    HTHEME      h_search_top_bmp;
    ...
    __s32 item_nr;
    dtv_setting_search_para_t *para;

    __s32 destroy_timerid;
    __s32 total;
    //set_para* uipara;
} dtv_setting_search_attr_t;

extern void  main_cmd2parent(H_WIN hwin, __s32 id, __u32 data1, __u32 data2);


static __s32 dtv_setting_search_item_paint_ex(__lbar_draw_para_t *draw_param);
static void dtv_setting_res_init(dtv_setting_search_attr_t *attr);
static void _dtv_setting_search_res_uninit(dtv_setting_search_attr_t *attr) ;
static void dtv_setting_title_update(dtv_setting_search_attr_t *gattr, __u32 string_id);
...
static void dtv_setting_title_update(dtv_setting_search_attr_t *gattr, __u32 string_id)
{
    ...
    GUI_BMP_Draw(theme_hdl2buf(gattr->h_search_top_bmp), 0, 0);
...
static __s32 dtv_setting_search_item_paint_ex(__lbar_draw_para_t *draw_param)
{
    ...
    RECT rect;
    GUI_RECT gui_rect;
    dtv_setting_search_attr_t *gattr;
    //GUI_MEMDEV_Handle draw_mem;
    maple_schdl_event_t *shdl_event = NULL;
    char time[128];
    char tmp[128];
    char shdl_name[128];
    gattr = (dtv_setting_search_attr_t *)draw_param->attr;
    ...
    switch (draw_param->mode)
    {
        ...
        {
            __msg("LBAR_MODE_NORMAL draw_param->index=%d\n", draw_param->index);
            ...
            if (maple_get_search_freq(draw_param->index) % (1000 * 1000) != 0)
            {
                eLIBs_sprintf(shdl_name, "%d.%d MHz\n", maple_get_search_freq(draw_param->index) / 1000 / 1000, maple_get_search_freq(draw_param->index) % (1000 * 1000) / (100 * 1000));
            }
            else
            {
                eLIBs_sprintf(shdl_name, "%d MHz\n", maple_get_search_freq(draw_param->index) / 1000 / 1000);
            }

            gui_rect.x0 = draw_param->rect.x;
            gui_rect.y0 = draw_param->rect.y + 8;
            gui_rect.x1 = gui_rect.x0 + LISTBAR_WIDTH;
        ...
        {
            __msg("LBAR_MODE_FOCUS draw_param->index=%d\n", draw_param->index);
...
static void dtv_setting_search_LISTBAR_init(__gui_msg_t *msg)
{
    __listbar_config_t config = {0};
    RECT rect;
    dtv_setting_search_attr_t *attr;
    __s32 index;
    __s32 start_id, focus_id;
    __u32 cur_freq = 0;
    attr = (dtv_setting_search_attr_t *)GUI_WinGetAddData(msg->h_deswin);
    dsk_display_get_size((__s32 *)&rect.width, (__s32 *)&rect.height);
    cur_freq = maple_get_freq_from_index(maple_get_cur_program_preview_index());
    ...
    {
        focus_id = maple_get_cur_foucs_index_from_freq(cur_freq);
    ...
    LISTBAR_Config(attr->h_LISTBAR, dtv_setting_search_item_paint_ex, &config);
...
static void dtv_setting_res_init(dtv_setting_search_attr_t *attr)
{
    ...
    attr->h_search_top_bmp = dsk_theme_open(ID_DTV_SETTING_TOP_BMP);
}

static void _dtv_setting_search_res_uninit(dtv_setting_search_attr_t *attr)
{
    __u32   i;
    theme_close(attr->h_search_top_bmp);
    attr->h_search_top_bmp = 0;
...
void dtv_setting_search_enter_action(__gui_msg_t *msg)
{
    dtv_setting_search_attr_t *attr;
    attr = (dtv_setting_search_attr_t *)GUI_WinGetAddData(msg->h_deswin);
}
...
static __s32 _dtv_setting_search_LISTBAR_key_proc(__gui_msg_t *msg)
{
    __s32 ret = EPDK_OK;
    static int active_key = -1;
    dtv_setting_search_attr_t *attr;
    attr = (dtv_setting_search_attr_t *)GUI_WinGetAddData(msg->h_deswin);
    __wrn("_dvt_setting_search_LISTBAR_key_proc() : msg->dwAddData1 = %d, msg->dwAddData2 = %d\n", msg->dwAddData1, msg->dwAddData2);
    ...
    switch (msg->dwAddData1)
    {
        ...
        case GUI_MSG_KEY_ENTER:
        {
            ...
            {
                attr->new_focus = LISTBAR_GetFocusItem(attr->h_LISTBAR);
                __log("attr->new_focus = %d \n", attr->new_focus);

                if (attr->new_focus >= 0 && attr->new_focus < attr->total)
                {
                    dtv_setting_send_cmd_to_parent(msg, GUI_MSG_COMMAND, DTV_SETTING_2_MANU_SEARCH, attr->new_focus);
                    break;
                }
            }
...
static __s32 _dtv_setting_search_LISTBAR_touch_proc(__gui_msg_t *msg)
{
    __s32 x = 0, y = 0;
...
static __s32  dtv_setting_search_touch_proc(__gui_msg_t *msg)
{
    return EPDK_OK;
}
...
/*
    回调
*/
static __s32 _dtv_setting_search_Proc(__gui_msg_t *msg)
{
    __msg("_dvt_setting_search_Proc()::msg->id=%08x, dwAddData1=%08x, dwAddData2=%08x\n", msg->id, msg->dwAddData1, msg->dwAddData2);

    switch (msg->id)
    {
        case GUI_MSG_CREATE:
        {
            dtv_setting_search_attr_t *attr = NULL;
            dtv_setting_search_para_t *para = NULL;
            HTHEME  bg_bmp;
            para = (dtv_setting_search_para_t *)GUI_WinGetAttr(msg->h_deswin);
            attr = (dtv_setting_search_attr_t *)My_Malloc(0, sizeof(dtv_setting_search_attr_t));
            ...
            eLIBs_memset(attr, 0, sizeof(dtv_setting_search_attr_t));
            ...
            attr->total = maple_get_cur_search_freq_tatol();
            __msg("attr->total = %d\n", attr->total);
            GUI_WinSetAddData(msg->h_deswin, (unsigned long)attr);
            GUI_LyrWinSetSta(attr->layer, GUI_LYRWIN_STA_SLEEP);
            dtv_setting_search_LISTBAR_init(msg);
            dtv_setting_title_update(attr, STRING_DTV_MANU_SELECT);
        ...
        case GUI_MSG_DESTROY:
        {
            ...
            attr->h_LISTBAR = NULL;
            _dtv_setting_search_res_uninit(attr);
        ...
        case GUI_MSG_KEY:
            return _dtv_setting_search_LISTBAR_key_proc(msg);

        case GUI_MSG_TOUCH:
            return dtv_setting_search_touch_proc(msg);////

        case GUI_MSG_COMMAND:
        {
            /*switch(LOWORD(msg->dwAddData1))
            {
                case DVBEPG_ID_CREATE_EPG_search:
                {
                    dtv_setting_search_LISTBAR_init(msg);
                }
                break;
...
H_WIN dtv_setting_search_win_create(H_WIN h_parent, dtv_setting_search_para_t *search_para)
{
    ...
    dtv_setting_search_para_t  *para;
    FB fb;
    GUI_LyrWinGetFB(search_para->layer, &fb);
    para = (dtv_setting_search_para_t *)My_Malloc(0, sizeof(dtv_setting_search_para_t));
    ...
    eLIBs_memset((void *)para, 0, sizeof(dtv_setting_search_para_t));
    para->font = SWFFont;
    para->layer = search_para->layer;
    para->focus_id = search_para->focus_id;
    ...
    framewin_para.id         = SUBMENU_DVB_SETTING_SEARCH_ID;
    framewin_para.hHosting = h_parent;
    framewin_para.FrameWinProc = (__pGUI_WIN_CB)esKRNL_GetCallBack((__pCBK_t)_dtv_setting_search_Proc);
