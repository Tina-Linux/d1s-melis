...
static void append(uint32_t handle, uint32_t property, uint32_t type, void *Value, struct MtpPropertyList *list)
{
    ...
    {
        eLIBs_printf("mStringValues[%u] = %p, mLongValues[%u] = %p, may error?\n",
    ...
    {
        case MTP_TYPE_STR:
			list->mStringValues[index] = esMEMS_Malloc(0, sizeof(char)*(eLIBs_strlen((char *)Value) + 1));
			eLIBs_memset(list->mStringValues[index], 0, sizeof(char)*(eLIBs_strlen((char *)Value) + 1));
            eLIBs_memcpy(list->mStringValues[index], (char *)Value, sizeof(char)*eLIBs_strlen((char *)Value));
            break;
        case MTP_TYPE_UINT16:
            list->mLongValues[index] = esMEMS_Malloc(0, sizeof(uint16_t));
			eLIBs_memset(list->mLongValues[index], 0, sizeof(uint16_t));
            eLIBs_memcpy(list->mLongValues[index], Value, sizeof(uint16_t));
            break;
        case MTP_TYPE_UINT32:
            list->mLongValues[index] = esMEMS_Malloc(0, sizeof(uint32_t));
			eLIBs_memset(list->mLongValues[index], 0, sizeof(uint32_t));
            *list->mLongValues[index] = *((uint32_t *)Value);
            break;
        case MTP_TYPE_UINT64:
            list->mLongValues[index] = esMEMS_Malloc(0, sizeof(uint64_t));
			eLIBs_memset(list->mLongValues[index], 0, sizeof(uint64_t));
            eLIBs_memcpy(list->mLongValues[index], Value, sizeof(uint64_t));
            break;
        case MTP_TYPE_UINT128:
            list->mLongValues[index] = esMEMS_Malloc(0, sizeof(uint128_t));
			eLIBs_memset(list->mLongValues[index], 0, sizeof(uint128_t));
            eLIBs_memcpy(list->mLongValues[index], Value, sizeof(uint128_t));
            break;
    }
    return ;
}
...
static struct MtpPropertyList *MtpPropertyListInit(uint32_t maxCount)
{
    ...
    list = (struct MtpPropertyList *)esMEMS_Malloc(0, sizeof(struct MtpPropertyList));
    eLIBs_memset(list, 0, sizeof(struct MtpPropertyList));

    list->mCount = 0;
    list->mObjectHandles = esMEMS_Malloc(0, maxCount*sizeof(uint32_t));
    eLIBs_memset(list->mObjectHandles, 0x00, maxCount*sizeof(uint32_t));
    list->mPropertyCodes = esMEMS_Malloc(0, maxCount*sizeof(uint16_t));
    eLIBs_memset(list->mPropertyCodes, 0x00, maxCount*sizeof(uint16_t));
    list->mDataTypes = esMEMS_Malloc(0, maxCount*sizeof(uint16_t));
    eLIBs_memset(list->mDataTypes, 0x00, maxCount*sizeof(uint16_t));
    list->mLongValues = esMEMS_Malloc(0, maxCount*sizeof(uint32_t **));
    eLIBs_memset(list->mLongValues, 0x00, maxCount*sizeof(uint32_t **));
    list->mStringValues = esMEMS_Malloc(0, maxCount*sizeof(char **));
    eLIBs_memset(list->mStringValues, 0x00, maxCount*sizeof(char **));
...
void MtpPropertyListRelease(struct MtpPropertyList *list)
{
    ...
    {
        esMEMS_Mfree(0, list->mObjectHandles);
    }
    if (list->mPropertyCodes)
    {
        esMEMS_Mfree(0, list->mPropertyCodes);
    }
    if (list->mDataTypes)
    {
        esMEMS_Mfree(0, list->mDataTypes);
    }
    if (list->mLongValues)
    {
        int i;
        for (i = 0; i < list->mCount; i++)
        {
            ...
            {
                esMEMS_Mfree(0, list->mLongValues[i]);
                list->mLongValues[i] = NULL;
            }
        }
        esMEMS_Mfree(0, list->mLongValues);
    }
    if (list->mStringValues)
    {
        ...
        for (i = 0; i < list->mCount; i++)
        {
            if (list->mStringValues[i] != NULL)
            {
                esMEMS_Mfree(0, list->mStringValues[i]);
                list->mStringValues[i] = NULL;
            }
        }
        esMEMS_Mfree(0, list->mStringValues);
    }
    esMEMS_Mfree(0, list);
    return ;
}
...
MtpResponseCode getSupportedObjectProperties(uint32_t format, struct MtpPacket *mData)
{
    ...
    switch (format)
    {
        ...
        case MTP_FORMAT_WMV:
            list = VIDEO_PROPERTIES;
            num = ARRAY_SIZE(VIDEO_PROPERTIES);
            break;
        ...
        case MTP_FORMAT_BMP:
        case MTP_FORMAT_DNG:
            list = IMAGE_PROPERTIES;
            num = ARRAY_SIZE(IMAGE_PROPERTIES);
            break;
...
struct MtpProperty *MtpPropertyInit(MtpPropertyCode propCode, MtpDataType type, bool writeable, int defaultValue)
{
    ...
    mProperty = (struct MtpProperty *)esMEMS_Malloc(0, sizeof(struct MtpProperty));
	eLIBs_memset(mProperty, 0x00, sizeof(struct MtpProperty));
