...
void putUInt32(uint8_t *buf, int offset, uint32_t value)
{
...
void putUInt16(uint8_t *buf, int offset, uint16_t value)
{
    buf[offset++] = (uint8_t)(value & 0xFF);
    buf[offset++] = (uint8_t)((value >> 8) & 0xFF);
}
...
static void MtpPacketDataReset(struct MtpPacket *mPacket)
{
    ...
    eLIBs_memset(mPacket->mBuffer, 0, mPacket->mBufferSize);
}
...
{
    size_t size = 0, count = eLIBs_strlen(string);
    int i;
...
{
    size_t count = eLIBs_strlen(string);
...
struct MtpPacket *MtpPacketInit(int bufferSize, int type)
{
    ...
    mPacket = (struct MtpPacket *)esMEMS_Malloc(0, sizeof(struct MtpPacket));
	eLIBs_memset(mPacket, 0x00, sizeof(struct MtpPacket));
    mPacket->mBufferSize = bufferSize;
    mPacket->mBuffer = esMEMS_Malloc(0, mPacket->mBufferSize);
	eLIBs_memset(mPacket->mBuffer, 0x00, mPacket->mBufferSize);
    mPacket->type = type;
...
void MtpPacketRelease(struct MtpPacket *mPacket)
{
    if (mPacket->mBuffer)
    {
        esMEMS_Mfree(0, mPacket->mBuffer);
        mPacket->mBuffer = NULL;
    }
    esMEMS_Mfree(0, mPacket);
}
