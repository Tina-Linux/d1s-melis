...
/*
 *static inline uint32_t irq_get_trigger_type(unsigned int irq)
 *{
 *        struct irq_data *d = irq_get_irq_data(irq);
...
{
        uint32_t (*reg_readl)(void *addr);
        void (*reg_writel)(uint32_t val, void *addr);
        ...
        uint32_t            mask_cache;
        uint32_t            type_cache;
        uint32_t            polarity_cache;
        uint32_t            wake_enabled;
        uint32_t            wake_active;
        unsigned int        num_ct;
        void            *private;
