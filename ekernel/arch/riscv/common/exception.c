...
static void show_register(irq_regs_t *regs, unsigned long stval)
{
    ...
    printk("sepc    :0x%016x\n" \
               ...
               "sscratch:0x%016x\n" \
               "stval   :0x%016x\n" \
               ,regs->sepc, regs->sstatus, regs->sscratch, stval);
}

static unsigned long default_handle(char *str, unsigned long scause, unsigned long sepc, unsigned long stval, irq_regs_t *regs)
{
    printk("=====================================================================================================\n");
    printk("                                         %s              \n", str);
    printk("=====================================================================================================\n");
    show_register(regs, stval);

    printk("\r\n");

#ifdef CONFIG_DEBUG_BACKTRACE
    printk("-------backtrace-----------\n");
    backtrace_exception(printk, regs->sstatus, regs->x2, regs->sepc, regs->x1);
    printk("---------------------------\n");
#endif
    ...
    dump_register_memory("sepc", regs->sepc, 128);
...
static unsigned long error_handle(unsigned long scause, unsigned long sepc, unsigned long stval, irq_regs_t *regs)
{
    int i;
    int cause = scause & ~SCAUSE_IRQ_FLAG;
    ...
    for(i = 0; i < sizeof(exception_table) / sizeof(exception_table[0]); i++)
    {
        if (exception_table[i].cause == cause && exception_table[i].error_str)
        {
            ...
            {
                return exception_table[i].callback(exception_table[i].error_str, scause, sepc, stval, regs);
            }
            else
            {
                return default_handle(exception_table[i].error_str, scause, sepc, stval, regs);
            }
        }
    }
    return default_handle("UNKNOWN ERROR", scause, sepc, stval, regs);
}

unsigned long riscv_cpu_handle_exception(unsigned long scause, unsigned long sepc, unsigned long stval, irq_regs_t *regs)
{
    unsigned long ret = 0;
    ...
    switch (scause & ~SCAUSE_IRQ_FLAG)
    {
        ...
        case EXC_STORE_PAGE_FAULT:
            return error_handle(scause, sepc, stval, regs);
