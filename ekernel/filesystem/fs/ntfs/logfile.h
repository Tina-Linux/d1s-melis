...
} __attribute__((packed)) RESTART_PAGE_HEADER;
...
} __attribute__((packed))  RESTART_AREA;
...
} __attribute__((packed)) LOG_CLIENT_RECORD;
...
} __attribute__((packed)) RECORD_PAGE_HEADER;
...
} __attribute__((packed)) LOG_RECORD_FLAGS;
...
typedef struct
{
    u16 seq_number;
    u16 client_index;
} __attribute__((packed)) LOG_CLIENT_ID;
...
typedef struct
{
    ...
    u16 lcns_to_follow;        /* Number of lcn_list entries following this entry. */
    /* Now at ofs 0x40. */
    u16 record_offset;
    ...
    {
        LCN lcn;
    } __attribute__((packed)) lcn_list[1];
} __attribute__((packed)) LOG_RECORD;
