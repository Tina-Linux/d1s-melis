...
static void NAttrSetFlag(ntfs_attr *na, FILE_ATTR_FLAGS flag)
{
    if (na->type == AT_DATA && na->name == AT_UNNAMED)
    {
        ...
        fs_log_trace01("Denied setting flag %d for not unnamed data "
                       "attribute\n", flag);
}
                             ...
                             const ATTR_RECORD *a, u8 *b)
{
    ...
    {
        fs_log_error("Non-zero (%04x) attribute flags. Cannot handle "
                     "this yet.\n", le16_to_cpu(a->flags));
        fs_err = EOPNOTSUPP;
        return 0;
    }
    ...
    for (i = 0, total = 0; rl[i].length; i++)
    {
            ...
            sle64_to_cpu(a->data_size))
        {
             ...
             */
            r = ntfs_pread(vol->_part, rl[i].lcn <<
...
#define ESTR "Error reading attribute value"
                ...
                {
                    fs_log_debug(ESTR ": Ran out of input data.\n");
                    fs_err = EIO;
                }
                else
                {
                    fs_log_debug(ESTR ": unknown error\n");
                    fs_err = EIO;
                }
#undef ESTR
                free(rl);
         ...
         * 16, just run length.
         */
        r = ntfs_pread(vol->_part, rl[i].lcn << vol->cluster_size_bits,
                          ...
                          ntfschar *name, u32 name_len)
{
    ...
    fs_log_trace01("Entering for inode %lld, attr 0x%x.\n",
    ...
    {
        fs_err = EIO;
        fs_log_perror("Inode %lld has corrupt attribute flags "
    ...
    if (a->non_resident)
    {
        ...
        {
            fs_err = EIO;
            fs_log_perror("Compressed inode %lld attr 0x%x has "
                          "no compression unit",
                          (unsigned long long)ni->mft_no, type);
            goto put_err_out;
        }
    ...
    else
    {
        ...
        ntfs_attr_init(na, FALSE, a->flags & ATTR_IS_COMPRESSED,
                       a->flags & ATTR_IS_ENCRYPTED ? 1 : 0,
                       a->flags & ATTR_IS_SPARSE ? 1 : 0, (l + 7) & ~7, l, l,
                       cs ? (l + 7) & ~7 : 0, 0);
    }
...
int ntfs_attr_map_runlist(ntfs_attr *na, VCN vcn)
{
    ...
    fs_log_trace01("Entering for inode 0x%llx, attr 0x%x, vcn 0x%llx.\n",
 ...
 *
 * If there is an error return NULL with fs_err set to the error code. The
...
runlist_element *ntfs_attr_find_vcn(ntfs_attr *na, const VCN vcn)
{
    ...
    fs_log_trace01("Entering for inode 0x%llx, attr 0x%x, vcn %llx\n",
...
map_rl:
     ...
     * in our code paths.
     */
    if (is_retry || fs_err == EINVAL || fs_err == ENOENT)
    {
        fs_err = EIO;
    }
    return NULL;
...
res_err_out:
        ...
        {
            fs_err = EIO;
            fs_log_perror("%s: Sanity check failed", __FUNCTION__);
            goto res_err_out;
        }
    ...
    if (!rl)
    {
        ...
        {
            fs_err = EIO;
            fs_log_perror("%s: Failed to find VCN #1", __FUNCTION__);
        }
        return -1;
    }
    /*
    ...
    for (; count; rl++, ofs = 0)
    {
        if (rl->lcn == LCN_RL_NOT_MAPPED)
        {
            ...
            if (!rl)
            {
                ...
                {
                    fs_err = EIO;
                    fs_log_perror("%s: Failed to find VCN #2",
                                  __FUNCTION__);
                }
                goto rl_err_out;
            }
        ...
        {
            fs_err = EIO;
            fs_log_perror("%s: Zero run length", __FUNCTION__);
            goto rl_err_out;
        }
        if (rl->lcn < (LCN)0)
        {
        ...
        to_read = min(count, (rl->length << vol->cluster_size_bits) -
                      ofs);
retry:
        fs_log_trace01("Reading %lld bytes from vcn %lld, lcn %lld, ofs"
                       " %lld.\n", (long long)to_read, (long long)rl->vcn,
                       (long long)rl->lcn, (long long)ofs);
        br = ntfs_pread(vol->_part, (rl->lcn << vol->cluster_size_bits) +
                        ofs, to_read, b);
        ...
        fs_log_perror("%s: ntfs_pread failed", __FUNCTION__);
        return -1;
    }
...
s64 ntfs_attr_pread(ntfs_attr *na, const s64 pos, s64 count, void *b)
{
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("%s: na=%p  b=%p  pos=%lld  count=%lld",
                      __FUNCTION__, na, b, (long long)pos,
                      (long long)count);
        return -1;
    }

    fs_log_trace01("Entering for inode %lld attr 0x%x pos %lld count "
                        ...
                        const u32 bk_size, void *dst)
{
    ...
    fs_log_trace01("Entering for inode 0x%llx, attr type 0x%x, pos 0x%llx.\n",
                   (unsigned long long)na->ni->mft_no, na->type,
                   (long long)pos);
 ...
 *
 * If not found, ntfs_attr_find() returns -1, with fs_err set to ENOENT and
 * @ctx->attr will point to the attribute before which the attribute being
 * searched for would need to be inserted if such an action were to be desired.
 *
 * On actual error, ntfs_attr_find() returns -1 with fs_err set to the error
 ...
 * ntfs_attr_find() repeatedly until it returns -1 with fs_err set to ENOENT to
 ...
 * mft record @ctx->mrec.
 *
 * If @type is AT_END, seek to the end and return -1 with fs_err set to ENOENT.
                          ...
                          const u8 *val, const u32 val_len, ntfs_attr_search_ctx *ctx)
{
    ...
    fs_log_trace01("attribute type 0x%x.\n", type);
    ...
    fs_err = EIO;
    fs_log_perror("%s: Corrupt inode (%lld)", __FUNCTION__,
 ...
 * If @type is AT_UNUSED, return the first found attribute, i.e. one can
 * enumerate all attributes by setting @type to AT_UNUSED and then calling
 * ntfs_external_attr_find() repeatedly until it returns -1 with fs_err set to
 ...
 * attribute list completely and return -1 with fs_err set to ENOENT.  AT_END is
 ...
 *
 * Return 0 if the search was successful and -1 if not, with fs_err set to the
 * error code.
 *
                                   ...
                                   ntfs_attr_search_ctx *ctx)
{
    ...
    fs_log_trace01("Entering for inode %lld, attribute type 0x%x.\n",
                   (unsigned long long)ni->mft_no, type);
    if (!base_ni)
    {
...
find_attr_list_attr:
            ...
            /* Not found?!? Absurd! */
            fs_err = EIO;
            fs_log_error("Attribute list wasn't found");
            return -1;
        }
    }
...
is_enumeration:
        if (MREF_LE(al_entry->mft_reference) == ni->mft_no)
        {
            ...
            {
                fs_log_error("Found stale mft reference in "
                             "attribute list!\n");
                break;
            }
        }
...
do_next_attr:
    ...
    fs_err = EIO;
    fs_log_perror("Inode is corrupt (%lld)", (long long)base_ni->mft_no);
    return -1;
not_found:
    /*
 ...
 * If @type is AT_UNUSED, return the first found attribute, i.e. one can
 * enumerate all attributes by setting @type to AT_UNUSED and then calling
 * ntfs_attr_lookup() repeatedly until it returns -1 with fs_err set to ENOENT
                     ...
                     ntfs_attr_search_ctx *ctx)
{
    ...
    fs_log_trace01("Entering for attribute type 0x%x\n", type);
 ...
 * and return it. Return NULL on error with fs_err set.
 *
...
{
    // NOTE: save fs_err if it could change and function stays void!
    free(ctx);
}
                        ...
                        ntfschar *name, u32 name_len, s64 *data_size)
{
    ...
    if (!na)
    {
        fs_log_perror("ntfs_attr_open failed");
        goto err_exit;
    }
    ...
    {
        fs_log_perror("ntfs_attr_pread failed");
        free(data);
        goto out;
    }
    ret = data;
...
int ntfs_attr_map_whole_runlist(ntfs_attr *na)
{
    ...
    fs_log_trace01("Entering for inode %llu, attr 0x%x.\n",
                   (unsigned long long)na->ni->mft_no, na->type);
    ...
    while (1)
    {
        ...
        if (!next_vcn)
        {
            ...
            {
                fs_err = EIO;
                fs_log_perror("First extent of inode %llu "
        ...
        {
            fs_err = EIO;
            fs_log_perror("Inode %llu has corrupt attribute list",
                          (unsigned long long)na->ni->mft_no);
            goto err_out;
        }
    }
    if (!a)
    {
        fs_log_perror("Couldn't find attribute for runlist mapping");
        goto err_out;
    }
    if (highest_vcn && highest_vcn != last_vcn - 1)
    {
        fs_err = EIO;
        fs_log_perror("Failed to load full runlist: inode: %llu "
 ...
 * or an error occurred. On error the error code is stored in fs_err. The
 * following error codes are defined:
                                    ...
                                    const ATTR_TYPES type)
{
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("%s: type=%d", __FUNCTION__, type);
        return NULL;
    }
 ...
 * error code is stored in fs_err. The following error codes are defined:
                                ...
                                const s64 size)
{
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("%s: size=%lld", __FUNCTION__,
                      (long long)size);
        return -1;
    }
    ...
    {
        fs_err = ERANGE;
        fs_log_perror("Too large attrlist (%lld)", (long long)size);
        return -1;
    }
    ...
    {
        fs_err = ERANGE;
        fs_log_perror("Attr type %d size check failed (min,size,max="
 ...
 * error occurred. On error the error code is stored in fs_err. The following
...
int ntfs_attr_can_be_non_resident(const ntfs_volume *vol, const ATTR_TYPES type)
{
    ...
    {
        fs_err = EPERM;
        fs_log_trace01("Attribute can't be non-resident\n");
        return -1;
    }
    return 0;
}
...
int ntfs_attr_can_be_resident(const ntfs_volume *vol, const ATTR_TYPES type)
{
    ...
    fs_log_trace01("Attribute can't be resident\n");
    fs_err = EPERM;
    return -1;
}
...
static int ntfs_attr_fill_zero(ntfs_attr *na, s64 pos, s64 count)
{
    ...
    fs_log_trace01("pos %lld, count %lld\n", (long long)pos,
                   (long long)count);
    ...
    while (pos < end)
    {
        ...
        if (written <= 0)
        {
            fs_log_perror("Failed to zero space");
            goto err_free;
        }
        pos += written;
    }
                               ...
                               runlist_element **rl, VCN *update_from)
{
    ...
    fs_log_trace01("count: %lld, cur_vcn: %lld, from: %lld, to: %lld, ofs: "
    ...
    *rl = ntfs_attr_find_vcn(na, cur_vcn);
    if (!*rl)
    {
        fs_log_error("Failed to find run after mapping runlist.\n");
        fs_err = EIO;
        goto err_out;
    }
    ...
    *rl = ntfs_runlists_merge(na->rl, rlc);
    ...
    {
        eo = fs_err;
        fs_log_perror("Failed to merge runlists");
        if (ntfs_cluster_free_from_rl(vol, rlc))
        {
            fs_log_perror("Failed to free hot clusters. "
                          "Please run chkdsk /f");
        }
        fs_err = eo;
        goto err_out;
    ...
    *rl = ntfs_attr_find_vcn(na, cur_vcn);
    if (!*rl)
    {
         ...
         */
        fs_log_error("Failed to find run after hole instantiation.\n");
        fs_err = EIO;
        goto err_out;
    }
    ...
    if ((*rl)->lcn < 0)
    {
        fs_log_error("BUG! LCN is lesser than 0. \n");
        fs_err = EIO;
        goto err_out;
    }
    if (*ofs)
 ...
 * fs_err. Possible error codes are:
...
int ntfs_make_room_for_attr(MFT_RECORD *m, u8 *pos, u32 size)
{
    ...
    fs_log_trace01("Entering for pos 0x%d, size %u.\n",
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("%s: pos=%p  m=%p", __FUNCTION__, pos, m);
        return -1;
    }
    ...
    {
        fs_err = ENOSPC;
        fs_log_trace01("No enough space in the MFT record\n");
        return -1;
    }
 ...
 * and -1 on error. On error the error code is stored in fs_err.
                                  ...
                                  ATTR_FLAGS flags)
{
    ...
    fs_log_trace01("Entering for inode 0x%llx, attr 0x%x, flags 0x%x.\n",
    ...
    if (ntfs_attr_can_be_resident(ni->vol, type))
    {
        ...
        {
            fs_log_trace01("ntfs_attr_can_be_resident failed.\n");
        }
        return -1;
    }
    ...
    {
        err = EEXIST;
        fs_log_trace01("Attribute already present.\n");
        goto put_err_out;
    }
    if (fs_err != ENOENT)
    {
    ...
    {
        err = fs_err;
        fs_log_trace01("Failed to make room for attribute.\n");
        goto put_err_out;
    }
    ...
    if (type != AT_ATTRIBUTE_LIST && NInoAttrList(base_ni))
    {
        if (ntfs_attrlist_entry_add(ni, a))
        {
            ...
            fs_log_trace01("Failed add attribute entry to "
                           "ATTRIBUTE_LIST.\n");
            goto put_err_out;
        }
    }
                                      ...
                                      ATTR_FLAGS flags)
{
    ...
    fs_log_trace01("Entering for inode 0x%llx, attr 0x%x, lowest_vcn %lld, "
    ...
    if (ntfs_attr_can_be_non_resident(ni->vol, type))
    {
        if (fs_err == EPERM)
        {
            fs_log_perror("Attribute can't be non resident");
        }
        else
        {
            fs_log_perror("ntfs_attr_can_be_non_resident failed");
        }
        return -1;
    }
    ...
    {
        err = EEXIST;
        fs_log_perror("Attribute 0x%x already present", type);
        goto put_err_out;
    }
    if (fs_err != ENOENT)
    {
        fs_log_perror("ntfs_attr_find failed");
        err = EIO;
        goto put_err_out;
    }
    a = ctx->attr;
    ...
    {
        err = fs_err;
        fs_log_perror("Failed to make room for attribute."
                      "(biu:%d, alcsize:%d, len:%d)", ctx->mrec->bytes_in_use,
                      ctx->mrec->bytes_allocated, length);
        goto put_err_out;
    }
    ...
    if (type != AT_ATTRIBUTE_LIST && NInoAttrList(base_ni))
    {
        ...
        {
            err = fs_err;
            fs_log_perror("Failed add attr entry to attrlist");
            ntfs_attr_record_resize(m, a, 0);
            goto put_err_out;
        }
    }
    ...
    {
        fs_log_perror("%s: attribute lookup failed", __FUNCTION__);
        ntfs_attr_put_search_ctx(ctx);
        return -1;
...
int ntfs_attr_record_rm(ntfs_attr_search_ctx *ctx)
{
    ...
    fs_log_trace01("Entering for inode 0x%llx, attr 0x%x.\n",
    ...
    {
        fs_log_trace01("Couldn't remove attribute record. Bug or damaged MFT "
                       "record.\n");
        if (NInoAttrList(base_ni) && type != AT_ATTRIBUTE_LIST)
            if (ntfs_attrlist_entry_add(ni, ctx->attr))
                fs_log_trace01("Rollback failed. Leaving inconstant "
                               "metadata.\n");
        err = EIO;
        return -1;
    }
    ...
    if (NInoAttrList(base_ni) && type != AT_ATTRIBUTE_LIST)
    {
        ...
        {
            fs_log_trace01("Couldn't delete record from "
                           "$ATTRIBUTE_LIST.\n");
            return -1;
        }
    }
        ...
        le16_to_cpu(ctx->mrec->attrs_offset) == 8)
    {
        if (ntfs_mft_record_free(ni->vol, ni))
        {
            ...
            fs_log_trace01("Couldn't free MFT record.\n");
            fs_err = EIO;
            return -1;
        }
    ...
    if (!ntfs_attrlist_need(base_ni))
    {
                             ...
                             0, NULL, 0, ctx))
        {
             ...
             */
            fs_log_trace01("Couldn't find attribute list. Succeed "
                           "anyway.\n");
            return 0;
        }
        /* Deallocate clusters. */
        if (ctx->attr->non_resident)
        {
            ...
            if (!al_rl)
            {
                fs_log_trace01("Couldn't decompress attribute list "
                               "runlist. Succeed anyway.\n");
                return 0;
            }
            if (ntfs_cluster_free_from_rl(base_ni->vol, al_rl))
            {
                fs_log_trace01("Leaking clusters! Run chkdsk. "
        ...
        if (ntfs_attr_record_rm(ctx))
        {
             ...
             * but without extents.
             */
            fs_log_trace01("Couldn't remove attribute list. Succeed "
                           "anyway.\n");
            return 0;
        }
    }
...
int ntfs_attr_record_resize(MFT_RECORD *m, ATTR_RECORD *a, u32 new_size)
{
    ...
    fs_log_trace01("Sizes: old=%u alloc=%u attr=%u new=%u\n",
    ...
    if (new_size != attr_size)
    {
        ...
        {
            fs_err = ENOSPC;
            fs_log_trace01("Not enough space in the MFT record "
                           "(%u > %u)\n", new_muse, alloc_size);
            return -1;
        }
        ...
        {
            fs_err = ENOSPC;
            fs_log_trace01("Too big INDEX_ROOT (%u > %u)\n",
                           new_muse, alloc_size);
                                    ...
                                    const u32 new_size)
{
    ...
    fs_log_trace01("Entering for new size %u.\n", (unsigned)new_size);
...
int ntfs_attr_record_move_to(ntfs_attr_search_ctx *ctx, ntfs_inode *ni)
{
    ...
    {
        fs_log_trace01("Invalid arguments passed.\n");
        fs_err = EINVAL;
        return -1;
    }

    fs_log_trace01("Entering for ctx->attr->type 0x%x, ctx->ntfs_ino->mft_no "
    ...
    if (!ctx->al_entry)
    {
        fs_log_trace01("Inode should contain attribute list to use this "
                       "function.\n");
        fs_err = EINVAL;
        return -1;
    }
    ...
    if (!ntfs_attr_find(a->type, (ntfschar *)((u8 *)a + le16_to_cpu(
                        ...
                        0, nctx))
    {
        fs_log_trace01("Attribute of such type, with same name already "
    ...
    {
        err = fs_err;
        fs_log_debug("Attribute lookup failed.\n");
        goto put_err_out;
    }
    ...
    {
        err = fs_err;
        fs_log_trace01("Couldn't make space for attribute.\n");
        goto put_err_out;
    }
...
int ntfs_attr_record_move_away(ntfs_attr_search_ctx *ctx, int extra)
{
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("%s: ctx=%p ctx->attr=%p extra=%d", __FUNCTION__,
                      ctx, ctx ? ctx->attr : NULL, extra);
        return -1;
    }

    fs_log_trace01("Entering for attr 0x%x, inode %llu\n",
                   (unsigned) le32_to_cpu(ctx->attr->type),
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("Inode %llu has no attrlist",
    ...
    {
        fs_log_perror("Couldn't attach extents, inode=%llu",
                      (unsigned long long)base_ni->mft_no);
        return -1;
    }
    ...
    if (!ni)
    {
        fs_log_perror("Couldn't allocate MFT record");
        return -1;
    }
    if (ntfs_attr_record_move_to(ctx, ni))
    {
        fs_log_perror("Couldn't move attribute to MFT record");
        return -1;
    }
    return 0;
}
...
static int ntfs_resident_attr_resize_i(ntfs_attr *na, const s64 newsize)
{
    ...
    fs_log_trace01("Inode 0x%llx attr 0x%x new size %lld\n",
                   (unsigned long long)na->ni->mft_no, na->type,
                   (long long)newsize);
    ...
    {
        err = fs_err;
        fs_log_perror("ntfs_attr_lookup failed");
        goto put_err_out;
    }
    vol = na->ni->vol;
    /*
    ...
    if (ntfs_attr_size_bounds_check(vol, na->type, newsize) < 0)
    {
        ...
        fs_log_perror("%s: bounds check failed", __FUNCTION__);
        goto put_err_out;
    }
    /*
    ...
    else if (fs_err != ENOSPC && fs_err != EPERM)
    {
        err = fs_err;
        fs_log_perror("Failed to make attribute non-resident");
        goto put_err_out;
    }
    ...
    while (!ntfs_attr_lookup(AT_UNUSED, NULL, 0, 0, 0, NULL, 0, ctx))
    {
        ...
        {
            err = fs_err;
            fs_log_perror("Couldn't open attribute");
            goto put_err_out;
        }
    ...
    {
        err = fs_err;
        fs_log_perror("%s: Attribute lookup failed 1", __FUNCTION__);
        goto put_err_out;
    }
    ...
    if (na->type == AT_STANDARD_INFORMATION || na->type == AT_ATTRIBUTE_LIST)
    {
        ...
        if (ntfs_inode_free_space(na->ni, offsetof(ATTR_RECORD,
                                  non_resident_end) + 8))
        {
            fs_log_perror("Could not free space in MFT record");
            return -1;
        }
        return ntfs_resident_attr_resize(na, newsize);
    }
    ...
    if (ntfs_attr_lookup(na->type, na->name, na->name_len, CASE_SENSITIVE,
                         0, NULL, 0, ctx))
    {
        fs_log_perror("%s: Attribute lookup failed 2", __FUNCTION__);
        err = fs_err;
        goto put_err_out;
    }
    ...
    {
        err = ENOSPC;
        fs_log_trace01("MFT record is filled with one attribute\n");
    ...
    {
        err = fs_err;
        fs_log_perror("Couldn't allocate new MFT record");
        goto put_err_out;
    }
    ...
    {
        err = fs_err;
        fs_log_perror("Couldn't move attribute to new MFT record");
        goto put_err_out;
    }
 ...
 *
 * Return 0 on success and -1 on error with fs_err set to the error code. The
 * following error codes are defined:
...
static int ntfs_attr_make_resident(ntfs_attr *na, ntfs_attr_search_ctx *ctx)
{
    ...
    {
        fs_log_trace01("Eeek!  Should be called for the first extent of the "
                       "attribute.  Aborting...\n");
        err = EINVAL;
        return -1;
    }
    ...
    if (!NAttrNonResident(na))
    {
        fs_log_trace01("Eeek!  Trying to make resident attribute resident.  "
                       "Aborting...\n");
        fs_err = EINVAL;
        return -1;
    }
    ...
    {
        fs_log_trace01("Making compressed or encrypted files resident is not "
                       "implemented yet.\n");
        fs_err = EOPNOTSUPP;
        return -1;
    }
    ...
    {
        fs_err = ENOSPC;
        fs_log_trace01("Not enough space to make attribute resident\n");
        return -1;
    }
    ...
    if (ntfs_attr_record_resize(ctx->mrec, a, arec_size) < 0)
    {
         ...
         */
        fs_log_error("BUG! Failed to resize attribute record."
                     " Aborting...\n");
        fs_err = EIO;
        return -1;
    }
    ...
    if (bytes_read != na->initialized_size)
    {
        ...
        fs_log_trace01("Eeek! Failed to read attribute data. Leaving "
    ...
    {
        err = fs_err;
        fs_log_perror("Eeek! Failed to release allocated clusters");
        fs_log_trace01("Ignoring error and leaving behind wasted "
                       "clusters.\n");
    }
                                 ...
                                 ntfs_attr_search_ctx *ctx)
{
    ...
    if (sparse && !(a->flags & (ATTR_IS_SPARSE | ATTR_IS_COMPRESSED)))
    {
                 ...
                 le32_to_cpu(m->bytes_in_use)))
        {
            ...
            {
                fs_log_perror("Failed to move attribute");
                goto error;
            }
            ntfs_attr_put_search_ctx(ctx);
            goto retry;
        ...
        {
            fs_err = EIO;
            fs_log_perror("Mapping pairs space is 0");
            goto error;
        }
...
retry:
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("%s: na=%p", __FUNCTION__, na);
        return -1;
    }
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("%s: resident attribute", __FUNCTION__);
        return -1;
    }
                             ...
                             CASE_SENSITIVE, from_vcn, NULL, 0, ctx))
    {
        ...
        if (finished_build)
        {
            fs_log_trace01("Mark attr 0x%x for delete in inode "
        ...
        if (mp_size <= 0)
        {
            fs_log_perror("%s: get MP size failed", __FUNCTION__);
            goto put_err_out;
        }
        /*
        ...
        if (mp_size > exp_max_mp_size)
        {
            ...
            if (na->type == AT_ATTRIBUTE_LIST)
            {
                ...
                if (ntfs_inode_free_space(na->ni, mp_size -
                                          cur_max_mp_size))
                {
                    fs_log_perror("Attribute list is too "
                                  "big. Defragment the "
                                  "volume\n");
                    return -1;
                }
            ...
            if (!NInoAttrList(base_ni))
            {
                ...
                {
                    fs_log_perror("Can not add attrlist");
                    return -1;
                }
                goto retry;
            }
        ...
        if (((mp_size + 7) & ~7) != cur_max_mp_size)
        {
            ...
            {
                fs_err = EIO;
                fs_log_perror("Failed to resize attribute");
                goto put_err_out;
            }
        }
        ...
        if (!finished_build && fs_err != ENOSPC)
        {
            fs_log_perror("Failed to build mapping pairs");
            goto put_err_out;
        }
    ...
    {
        ntfs_attr_reinit_search_ctx(ctx);
        fs_log_trace01("Deallocate marked extents.\n");
        while (!ntfs_attr_lookup(na->type, na->name, na->name_len,
                                 CASE_SENSITIVE, 0, NULL, 0, ctx))
        {
            ...
            if (ntfs_attr_record_rm(ctx))
            {
                fs_log_perror("Could not remove unused attr");
                goto put_err_out;
            }
            ntfs_attr_reinit_search_ctx(ctx);
        }
        if (fs_err != ENOENT)
        {
            fs_log_perror("%s: Attr lookup failed", __FUNCTION__);
            goto put_err_out;
        }
        fs_log_trace01("Deallocate done.\n");
        ntfs_attr_put_search_ctx(ctx);
        goto ok;
    }
    ntfs_attr_put_search_ctx(ctx);
    ...
    while (1)
    {
        ...
        if (!ni)
        {
            fs_log_perror("Could not allocate new MFT record");
            goto put_err_out;
        }
        m = ni->mrec;
        /*
        ...
        {
            err = fs_err;
            fs_log_perror("Could not add attribute extent");
            if (ntfs_mft_record_free(na->ni->vol, ni))
            {
                fs_log_perror("Could not free MFT record");
            }
            fs_err = err;
            goto put_err_out;
        }
        ...
        err = ntfs_mapping_pairs_build(na->ni->vol, (u8 *)a +
                                       ...
                                       stop_vcn, &stop_vcn);
        if (err < 0 && fs_err != ENOSPC)
        {
            err = fs_err;
            fs_log_perror("Failed to build MP");
            if (ntfs_mft_record_free(na->ni->vol, ni))
            {
                fs_log_perror("Couldn't free MFT record");
            }
            fs_err = err;
            goto put_err_out;
        }
...
static int ntfs_non_resident_attr_shrink(ntfs_attr *na, const s64 newsize)
{
    ...
    if (ntfs_attr_size_bounds_check(vol, na->type, newsize) < 0)
    {
        if (fs_err == ERANGE)
        {
            fs_log_trace01("Eeek! Size bounds check failed. "
                           "Aborting...\n");
        }
        else if (fs_err == ENOENT)
        {
            fs_err = EIO;
        }
        return -1;
    ...
    if ((na->allocated_size >> vol->cluster_size_bits) != first_free_vcn)
    {
        if (ntfs_attr_map_whole_runlist(na))
        {
            fs_log_trace01("Eeek! ntfs_attr_map_whole_runlist "
                           "failed.\n");
            return -1;
        }
        ...
        {
            fs_log_trace01("Eeek! Freeing of clusters failed. "
                           "Aborting...\n");
            return -1;
        }
        ...
        if (ntfs_rl_truncate(&na->rl, first_free_vcn))
        {
            ...
            free(na->rl);
            na->rl = NULL;
            fs_log_trace01("Eeek! Run list truncation failed.\n");
            return -1;
        }
        ...
        {
            fs_log_trace01("Eeek! Mapping pairs update failed. "
                         ...
                         0, NULL, 0, ctx))
    {
        ...
        fs_log_trace01("Eeek! Lookup of first attribute extent failed. "
                       "Leaving inconstant metadata.\n");
        goto put_err_out;
    }
    ...
    if (!newsize)
    {
        if (ntfs_attr_make_resident(na, ctx))
        {
            ...
            if (fs_err != EPERM)
                fs_log_error("Failed to make attribute "
...
static int ntfs_non_resident_attr_expand_i(ntfs_attr *na, const s64 newsize)
{
    ...
    fs_log_trace01("Inode %lld, attr 0x%x, new size %lld old size %lld\n",
                   (unsigned long long)na->ni->mft_no, na->type,
    ...
    if ((na->allocated_size >> vol->cluster_size_bits) < first_free_vcn)
    {
        if (ntfs_attr_map_whole_runlist(na))
        {
            fs_log_perror("ntfs_attr_map_whole_runlist failed");
            return -1;
        }
        ...
        else
        {
            ...
            if (!rl)
            {
                fs_log_perror("Cluster allocation failed "
                              "(%lld)",
        ...
        if (!rln)
        {
            ...
            err = fs_err;
            fs_log_perror("Run list merge failed");
        ...
        {
            err = fs_err;
            fs_log_perror("Mapping pairs update failed");
            goto rollback;
        }
    }
    ...
    {
        err = fs_err;
        fs_log_perror("Lookup of first attribute extent failed");
        if (err == ENOENT)
        {
            err = EIO;
        }
...
rollback:
    ...
    {
        err = EIO;
        fs_log_perror("Leaking clusters");
    }
                         ...
                         vol->cluster_size_bits))
    {
        ...
        free(na->rl);
        na->rl = NULL;
        fs_log_perror("Couldn't truncate runlist. Rollback failed");
    }
    else
    {
        /* Prepare to mapping pairs update. */
        ...
        if (ntfs_attr_update_mapping_pairs(na, 0 /*na->allocated_size >>
                    vol->cluster_size_bits*/))
        {
            fs_log_perror("Failed to restore old mapping pairs");
        }
    }
    fs_err = err;
    return -1;
...
int ntfs_attr_truncate(ntfs_attr *na, const s64 newsize)
{
    ...
    fs_log_trace01("Entering for inode %lld, attr 0x%x, size %lld\n",
                   (unsigned long long)na->ni->mft_no, na->type,
                   (long long)newsize);

    if (na->data_size == newsize)
    {
        fs_log_trace01("Size is already ok\n");
        ret = STATUS_OK;
        goto out;
    }
    /*
    ...
    {
        fs_err = EACCES;
        fs_log_perror("Failed to truncate encrypted attribute");
        goto out;
    }
    /*
    ...
    {
        fs_err = EOPNOTSUPP;
        fs_log_perror("Failed to truncate compressed attribute");
        goto out;
    }
    if (NAttrNonResident(na))
    {
...
out:
    fs_log_trace01("Return status %d\n", ret);
    return ret;
}
                                       ...
                                       ntfs_attr_search_ctx *ctx)
{
    ...
    if (NAttrNonResident(na))
    {
        fs_log_trace01("Eeek!  Trying to make non-resident attribute "
    ...
    if (rl)
    {
        ...
        {
            err = fs_err;
            fs_log_debug("Eeek!  Failed to write out attribute value "
                         "(bw = %lli, fs_err = %i).  "
                         "Aborting...\n", (long long)bw, err);
            if (bw >= 0)
            {
    ...
    {
        err = fs_err;
        fs_log_debug("Eeek!  Failed to get size for mapping pairs array.  "
                     "Aborting...\n");
        goto cluster_free_err_out;
    }
                                 ...
                                 rl, 0, NULL) < 0)
    {
        ...
        fs_log_trace01("Eeek!  Failed to build mapping pairs.  Leaving "
...
cluster_free_err_out:
    if (rl && ntfs_cluster_free(vol, na, 0, -1) < 0)
        fs_log_trace01("Eeek!  Failed to release allocated clusters in error "
...
s64 ntfs_attr_pwrite(ntfs_attr *na, const s64 pos, s64 count, const void *b)
{
    ...
    fs_log_trace01("Entering for inode %lld, attr 0x%x, pos 0x%llx, count "
    ...
    if (pos + count > na->data_size)
    {
        ...
        {
            fs_log_perror("Failed to enlarge attribute");
            goto fs_err_set;
        }
    ...
    if (!NAttrNonResident(na))
    {
        ...
        if (ntfs_attr_lookup(na->type, na->name, na->name_len, 0,
                             0, NULL, 0, ctx))
        {
            fs_log_perror("%s: lookup failed", __FUNCTION__);
            goto err_out;
        }
        val = (char *)ctx->attr + le16_to_cpu(ctx->attr->value_offset);
        if (val < (char *)ctx->attr || val +
                                  ...
                                  ctx->mrec))
        {
             ...
             */
            fs_log_perror("%s: failed to write mft record", __FUNCTION__);
            goto err_out;
        }
        ntfs_attr_put_search_ctx(ctx);
        total = count;
    ...
    for (hole_end = 0; count; rl++, ofs = 0, hole_end = 0)
    {
        if (rl->lcn == LCN_RL_NOT_MAPPED)
        {
            ...
            if (!rl)
            {
                ...
                {
                    fs_err = EIO;
                    fs_log_perror("%s: Failed to find VCN"
                                  " #2", __FUNCTION__);
                }
                goto rl_err_out;
            }
        ...
        if (rl->lcn < (LCN)0)
        {
            ...
            {
                fs_err = EIO;
                fs_log_perror("%s: Unexpected LCN (%lld)",
                              __FUNCTION__,
                              (long long)rl->lcn);
                goto rl_err_out;
            }
        ...
        to_write = min(count, (rl->length << vol->cluster_size_bits) - ofs);
retry:
        fs_log_trace01("Writing %lld bytes to vcn %lld, lcn %lld, ofs "
        ...
        if (!NVolReadOnly(vol))
        {
                             ...
                             (wend < (hole_end << vol->cluster_size_bits))))
            {
                ...
                written = ntfs_pwrite(vol->_part, wpos, rounding, cb);
                if (written == rounding)
                {
                    written = to_write;
                }
            ...
            {
                written = ntfs_pwrite(vol->_part, wpos, to_write, b);
            }
        }
        else
        {
...
err_out:
    ...
    if (need_to.undo_initialized_size)
    {
        ...
        if (err)
        {
             ...
             */
            fs_log_error("Eeek! Failed to recover from error. "
    ...
    {
        fs_log_perror("Failed to restore data_size");
    }
    fs_err = eo;
fs_err_set:
    total = -1;
                         ...
                         const u32 bk_size, void *src)
{
    ...
    for (i = 0; i < bk_cnt; ++i)
    {
        ...
        if (err < 0)
        {
            ...
            fs_log_perror("%s #1", __FUNCTION__);
            if (!i)
            {
                return err;
            }
    ...
    if (written <= 0)
    {
        fs_log_perror("%s: written=%lld", __FUNCTION__,
                      (long long)written);
    }
...
int ntfs_attr_rm(ntfs_attr *na)
{
    ...
    if (NAttrNonResident(na))
    {
        ...
        {
            fs_log_trace01("Failed to free cluster allocation. Leaving "
                           "inconstant metadata.\n");
            ret = -1;
        }
    }
                             ...
                             CASE_SENSITIVE, 0, NULL, 0, ctx))
    {
        if (ntfs_attr_record_rm(ctx))
        {
            fs_log_trace01("Failed to remove attribute extent. Leaving "
                           "inconstant metadata.\n");
            ret = -1;
        }
        ntfs_attr_reinit_search_ctx(ctx);
    ...
    if (fs_err != ENOENT)
    {
        fs_log_trace01("Attribute lookup failed. Probably leaving inconstant "
                       "metadata.\n");
        ret = -1;
    }
 ...
 *
 * On success return 0. On error return -1 with fs_err set to the error code.
 */
                  ...
                  ntfschar *name, u8 name_len, u8 *val, s64 size)
{
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("%s: ni=%p  size=%lld", __FUNCTION__, ni,
                      (long long)size);
        return -1;
    }

    fs_log_trace01("Entering for inode %lld, attr %x, size %lld.\n",
    ...
    if (ntfs_attr_can_be_non_resident(ni->vol, type))
    {
        ...
        {
            fs_err = EINVAL;
            fs_log_perror("val is mandatory for always resident "
                          "attributes");
            return -1;
        }
        ...
        {
            fs_err = ERANGE;
            fs_log_perror("Attribute is too big");
            return -1;
        }
    }
    else
    ...
    else
    {
        ...
        {
            err = fs_err;
            fs_log_perror("ntfs_attr_can_be_resident failed");
            goto err_out;
        }
        is_resident = FALSE;
    }
    ...
    {
        err = fs_err;
        fs_log_perror("Failed to attach all extents to inode");
        goto err_out;
    }
    ...
    if (!NInoAttrList(ni))
    {
        ...
        {
            err = fs_err;
            fs_log_perror("Failed to add attribute list");
            goto err_out;
        }
    ...
    {
        err = fs_err;
        fs_log_perror("Failed to allocate extent record");
        goto err_out;
    }
...
add_attr_record:
    if (is_resident)
    {
        ...
        if (offset < 0)
        {
            if (fs_err == ENOSPC && can_be_non_resident)
            {
                goto add_non_resident;
            }
            err = fs_err;
            fs_log_perror("Failed to add resident attribute");
            goto free_err_out;
        }
        return 0;
    }
...
add_non_resident:
    ...
    {
        err = fs_err;
        fs_log_perror("Failed to add non resident attribute");
        goto free_err_out;
    }
    ...
    {
        err = fs_err;
        fs_log_perror("Failed to open just added attribute");
    ...
    {
        err = fs_err;
        fs_log_perror("Failed to initialize just added attribute");
        if (ntfs_attr_rm(na))
        {
            fs_log_perror("Failed to remove just added attribute");
        }
        ntfs_attr_close(na);
        goto err_out;
    }
...
rm_attr_err_out:
    ...
    {
        fs_log_perror("Failed to remove just added attribute #2");
    }
free_err_out:
    /* Free MFT record, if it doesn't contain attributes. */
    if (le32_to_cpu(attr_ni->mrec->bytes_in_use) -
        ...
        {
            fs_log_perror("Failed to free MFT record");
        }
err_out:
    fs_err = err;
    return -1;
                     ...
                     u32 name_len)
{
    ...
    if (!ni)
    {
        fs_log_error("%s: NULL inode pointer", __FUNCTION__);
        fs_err = EINVAL;
        return -1;
    }
    ...
    if (!na)
    {
        fs_log_perror("Failed to open attribute 0x%02x of inode "
    ...
    ret = ntfs_attr_rm(na);
    if (ret)
        fs_log_perror("Failed to remove attribute 0x%02x of inode "
                      "0x%llx", type, (unsigned long long)ni->mft_no);
    ntfs_attr_close(na);
...
s64 ntfs_attr_get_free_bits(ntfs_attr *na)
{
    ...
    while (1)
    {
        ...
        {
            case 3:
                nr_free += lut[*(buf + br - 3)];
            case 2:
                nr_free += lut[*(buf + br - 2)];
            case 1:
                nr_free += lut[*(buf + br - 1)];
        }
    }
    free(buf);
out:
