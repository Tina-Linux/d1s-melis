...
int ntfs_mst_post_read_fixup(NTFS_RECORD *b, const u32 size)
{
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("%s: magic: 0x%08x  size: %d  usa_ofs: %d  "
    ...
    while (usa_count--)
    {
        if (*data_pos != usn)
        {
             ...
             */
            fs_err = EIO;
            fs_log_perror("Incomplete multi-sector transfer: "
 ...
 * due to errors. In that case fs_err i set to the error code (EINVAL).
 *
...
int ntfs_mst_pre_write_fixup(NTFS_RECORD *b, const u32 size)
{
    ...
    {
        fs_err = EINVAL;
        fs_log_perror("%s: bad argument", __FUNCTION__);
        return -1;
    }
    /* Setup the variables. */
    usa_ofs = le16_to_cpu(b->usa_ofs);
