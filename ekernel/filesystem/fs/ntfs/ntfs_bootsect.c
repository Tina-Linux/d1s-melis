...
BOOL ntfs_boot_sector_is_ntfs(NTFS_BOOT_SECTOR *b)
{
    ...
    fs_log_debug("Beginning bootsector check.\n");

    fs_log_debug("Checking OEMid, NTFS signature.\n");
    if (b->oem_id != cpu_to_le64(0x202020205346544eULL))   /* "NTFS    " */
    {
        fs_log_verbose("NTFS signature is missing.\n");
        goto not_ntfs;
    }

    fs_log_debug("Checking bytes per sector.\n");
    if (le16_to_cpu(b->bpb.bytes_per_sector) <  256 ||
        le16_to_cpu(b->bpb.bytes_per_sector) > 4096)
    {
        fs_log_error("Unexpected bytes per sector value (%d).\n",
                     le16_to_cpu(b->bpb.bytes_per_sector));
        goto not_ntfs;
    }

    fs_log_debug("Checking sectors per cluster.\n");
    switch (b->bpb.sectors_per_cluster)
    {
        ...
        case 128:
            break;
        default:
            fs_log_error("Unexpected sectors per cluster value (%d).\n",
                         b->bpb.sectors_per_cluster);
            goto not_ntfs;
    }

    fs_log_debug("Checking cluster size.\n");
    i = (u32)le16_to_cpu(b->bpb.bytes_per_sector) *
        b->bpb.sectors_per_cluster;
    if (i > 65536)
    {
        fs_log_error("Unexpected cluster size (%d).\n", i);
        goto not_ntfs;
    }

    fs_log_debug("Checking reserved fields are zero.\n");
    if (le16_to_cpu(b->bpb.reserved_sectors) ||
        ...
        b->bpb.fats)
    {
        fs_log_error("Reserved fields aren't zero "
                     "(%d, %d, %d, %d, %d, %d).\n",
    ...
    fs_log_debug("Checking clusters per mft record.\n");
        ...
        (u8)b->clusters_per_mft_record > 0xf7)
    {
        switch (b->clusters_per_mft_record)
        {
            ...
            case 0x40:
                break;
            default:
                fs_log_error("Unexpected clusters per mft record "
                             "(%d).\n", b->clusters_per_mft_record);
                goto not_ntfs;
        }
    }

    fs_log_debug("Checking clusters per index block.\n");
        ...
        (u8)b->clusters_per_index_record > 0xf7)
    {
        switch (b->clusters_per_index_record)
        {
            ...
            case 0x40:
                break;
            default:
                fs_log_error("Unexpected clusters per index record "
    ...
    if (b->end_of_sector_marker != cpu_to_le16(0xaa55))
        fs_log_debug("Warning: Bootsector has invalid end of sector "
                     "marker.\n");

    fs_log_debug("Bootsector check completed successfully.\n");
 ...
 * obtained values.
 *
 * Return 0 on success or -1 on error with fs_err set to the error code EINVAL.
 */
int ntfs_boot_sector_parse(ntfs_volume *vol, const NTFS_BOOT_SECTOR *bs)
{
    ...
    /* We return -1 with fs_err = EINVAL on error. */
    fs_err = EINVAL;

    vol->sector_size = le16_to_cpu(bs->bpb.bytes_per_sector);
    vol->sector_size_bits = ffs(vol->sector_size) - 1;
    fs_log_debug("SectorSize = 0x%x\n", vol->sector_size);
    fs_log_debug("SectorSizeBits = %u\n", vol->sector_size_bits);
    /*
    ...
    fs_log_debug("SectorsPerCluster = 0x%x\n", sectors_per_cluster);
    if (sectors_per_cluster & (sectors_per_cluster - 1))
    {
        fs_log_error("sectors_per_cluster (%d) is not a power of 2."
                     "\n", sectors_per_cluster);
        return -1;
    }

    sectors = sle64_to_cpu(bs->number_of_sectors);
    fs_log_debug("NumberOfSectors = %lld\n", (long long)sectors);
    if (!sectors)
    {
        fs_log_error("Volume size is set to zero.\n");
        return -1;
    }
    //  if (vol->dev->d_ops->seek(vol->dev,
    //                (sectors - 1) << vol->sector_size_bits,
    //                SEEK_SET) == -1) {
    //      fs_log_perror("Failed to read last sector (%lld)",
    //                  (long long)sectors);
    //      fs_log_error("%s", last_sector_error);
    //      return -1;
    //  }
    ...
    fs_log_debug("MFT LCN = %lld\n", (long long)vol->mft_lcn);
    fs_log_debug("MFTMirr LCN = %lld\n", (long long)vol->mftmirr_lcn);
    if (vol->mft_lcn     > vol->nr_clusters ||
        vol->mftmirr_lcn > vol->nr_clusters)
    {
        fs_log_error("$MFT LCN (%lld) or $MFTMirr LCN (%lld) is "
    ...
    {
        fs_log_error("cluster_size (%d) is not a power of 2.\n",
                     vol->cluster_size);
        return -1;
    }
    ...
    fs_log_debug("ClusterSize = 0x%x\n", (unsigned)vol->cluster_size);
    fs_log_debug("ClusterSizeBits = %u\n", vol->cluster_size_bits);
    fs_log_debug("ClustersPerMftRecord = 0x%x\n", c);
    /*
    ...
    {
        fs_log_error("mft_record_size (%d) is not a power of 2.\n",
                     vol->mft_record_size);
        return -1;
    }
    vol->mft_record_size_bits = ffs(vol->mft_record_size) - 1;
    fs_log_debug("MftRecordSize = 0x%x\n", (unsigned)vol->mft_record_size);
    fs_log_debug("MftRecordSizeBits = %u\n", vol->mft_record_size_bits);
    /* Same as above for INDX record. */
    c = bs->clusters_per_index_record;
    fs_log_debug("ClustersPerINDXRecord = 0x%x\n", c);
    if (c < 0)
    {
    ...
    fs_log_debug("INDXRecordSize = 0x%x\n", (unsigned)vol->indx_record_size);
    fs_log_debug("INDXRecordSizeBits = %u\n", vol->indx_record_size_bits);
    /*
