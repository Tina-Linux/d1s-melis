 ...
 *
 * Return 1 if inode need attribute list, 0 if not, -1 on error with fs_err set
 * to the error code. If function succeed fs_err set to 0. The following error
 * codes are defined:
...
int ntfs_attrlist_need(ntfs_inode *ni)
{
    ...
    if (!NInoAttrList(ni))
    {
        fs_log_trace01("Inode haven't got attribute list.\n");
        fs_err = EINVAL;
        return -1;
    }
...
int ntfs_attrlist_entry_add(ntfs_inode *ni, ATTR_RECORD *attr)
{
    ...
    if (!NInoAttrList(ni))
    {
        fs_log_trace01("Attribute list isn't present.\n");
        fs_err = ENOENT;
        return -1;
    }
                          ...
                          0 : le32_to_cpu(attr->value_length), ctx))
    {
        ...
        {
            err = EEXIST;
            fs_log_trace01("Such attribute already present in the "
                           "attribute list.\n");
    ...
    else
    {
        ...
        {
            err = fs_err;
            fs_log_trace01("Attribute lookup failed.\n");
            ntfs_attr_put_search_ctx(ctx);
            goto err_out;
        }
    ...
    {
        err = fs_err;
        fs_log_trace01("Failed to open $ATTRIBUTE_LIST attribute.\n");
        goto err_out;
    }
    if (ntfs_attr_truncate(na, ni->attr_list_size + entry_len))
    {
        err = fs_err;
        fs_log_trace01("$ATTRIBUTE_LIST resize failed.\n");
        goto err_out;
    }
