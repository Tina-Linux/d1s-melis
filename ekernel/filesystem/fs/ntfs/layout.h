...
/*
 * ntfs block device information.
 * just support 512 byte sector size device.
 */
...
typedef enum
{
    ...
    COLLATION_RULES_SPACE_FILLER  = 0xffffffff,
} COLLATION_RULES;
...
typedef enum
{
    ...
    ATTR_DEF_ALWAYS_LOG = const_cpu_to_le32(0x80), /* Always log
                    ...
                    resident. */
    ATTR_DEF_SPACE_FILLER  = 0xffffffff,
} ATTR_DEF_FLAGS;
...
typedef enum
{
    ...
    ATTR_FLG_FILL       = const_cpu_to_le16(0xffff),
...
typedef struct
{
    ntfschar name[1];   /* The name of the volume in Unicode. */
...
typedef struct
{
    u8 data[1];     /* The file's data contents. */
...
typedef enum
{
    ...
    QUOTA_FLAG_SPACE_FILLER  = 0xffffffff,
} QUOTA_FLAGS;
