...
static void (*rt_thread_delete_hook)(rt_thread_t thread);
...
void rt_thread_delete_sethook(void (*hook)(rt_thread_t thread))
{
    rt_thread_delete_hook = hook;
}
#endif
...
void rt_thread_exit(void)
{
    ...
    RT_OBJECT_HOOK_CALL(rt_thread_delete_hook, (thread));
    /* switch to next task */
    rt_schedule();
}
                                ...
                                rt_uint32_t       tick)
{
    ...
    if(thread != rt_thread_idle_gethandler())
    {
#endif
        /* init thread stack */
...
#ifdef CONFIG_RISCV
    // initialzie fpu status.
    rt_memset(&thread->fdext_ctx, 0x00, sizeof(fpu_context_t));
#endif
    /* priority init */
    ...
    if (rt_thread_self())
    {
        rt_memcpy(thread->user_data, rt_thread_self()->user_data, sizeof(thread->user_data));
    }
    else
    {
        rt_memset(thread->user_data, 0, sizeof(thread->user_data));
    }
...
rt_err_t rt_thread_startup(rt_thread_t thread)
{
    ...
    if (rt_thread_self() != RT_NULL && preemptible())
    {
        /* do a scheduling */
        rt_schedule();
    }
