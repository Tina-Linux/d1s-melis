/*
 * linux-4.9/drivers/media/platform/sunxi-vin/vin-isp/isp_server/isp_tuning/isp_tuning.c
 *
...
int isp_params_parse(struct hw_isp_device *isp, struct isp_param_config *params, HW_U8 ir, int sync_mode)
{
	...
	if (ctx->sensor_info.fps_fixed >= ISP_FAST_CONVERGENCE_FPS) {
		...
		if ((isp->id == 0 && sensor_isp_cfg->sign == 0xAA66AA66) ||
		(isp->id != 0 && sensor_isp_cfg->sign == 0xBB66BB66)) {
			return parser_ini_info(params, ctx->sensor_info.name,
				ctx->sensor_info.sensor_width, ctx->sensor_info.sensor_height,
				ctx->sensor_info.fps_fixed, sensor_isp_cfg->wdr_mode, ir, sync_mode, ctx->isp_id);
		} else {
			ISP_WARN("wdr_mode is fault, use sensor_info wdr_mode\n");
			return parser_ini_info(params, ctx->sensor_info.name,
				ctx->sensor_info.sensor_width, ctx->sensor_info.sensor_height,
...
#endif
int isp_tuning_read_from_bin(struct hw_isp_device *isp, int *value)
{
	...
	idx = value[1];
	size = value[2];
	if (flag == SET_BIN_TEST_3A_TUNING) {
		param = (char*)&tuning->params.isp_test_settings;
		memcpy(&param[idx * 400], &value[3], size);
	} else if (flag == SET_BIN_ISO_TRIGER) {
		tuning->params.isp_iso_settings.triger.sharp_triger = value[3];
#ifdef USE_ENCPP
		tuning->params.isp_iso_settings.triger.encpp_sharp_triger = value[4];
		tuning->params.isp_iso_settings.triger.encoder_denoise_triger = value[5];
#endif
		tuning->params.isp_iso_settings.triger.denoise_triger = value[6];
		tuning->params.isp_iso_settings.triger.black_level_triger = value[7];
		tuning->params.isp_iso_settings.triger.dpc_triger = value[8];
		tuning->params.isp_iso_settings.triger.defog_value_triger = value[9];
		tuning->params.isp_iso_settings.triger.pltm_dynamic_triger = value[10];
		tuning->params.isp_iso_settings.triger.brightness_triger = value[11];
		tuning->params.isp_iso_settings.triger.gcontrast_triger = value[12];
		tuning->params.isp_iso_settings.triger.cem_triger = value[13];
		tuning->params.isp_iso_settings.triger.tdf_triger = value[14];
		tuning->params.isp_iso_settings.triger.color_denoise_triger = value[15];
		tuning->params.isp_iso_settings.triger.ae_cfg_triger = value[16];
		tuning->params.isp_iso_settings.triger.gtm_cfg_triger = value[17];
		tuning->params.isp_iso_settings.triger.lca_cfg_triger = value[18];
		tuning->params.isp_iso_settings.triger.wdr_cfg_triger = value[19];
		tuning->params.isp_iso_settings.triger.cfa_triger = value[20];
		tuning->params.isp_iso_settings.triger.shading_triger = value[21];
	} else if (flag == SET_BIN_ISO_OTHER) {
		param = (char*)&tuning->params.isp_iso_settings.isp_lum_mapping_point[0];
		memcpy(&param[idx * 400], &value[3], size);
	} else if (flag == SET_BIN_TUNING_UPDATE) {
		ISP_PRINT("update bin params\n");
		return isp_tuning_update(isp);
	} else {
		return -1;
	}
...
int isp_tuning_update(struct hw_isp_device *isp)
{
	...
	if (tuning->ctx == NULL)
		return -1;
	hal_mutex_lock(tuning->mutex);
	tuning->ctx->isp_ini_cfg = tuning->params;
	hal_mutex_unlock(tuning->mutex);
			...
			const struct isp_param_config *params)
{
	...
	tuning = hal_malloc(sizeof(struct isp_tuning));
	if (tuning == NULL) {
		ISP_ERR("tuning malloc fail\n");
		return NULL;
	}
