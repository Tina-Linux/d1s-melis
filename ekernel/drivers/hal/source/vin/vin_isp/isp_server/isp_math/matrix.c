/*
 * linux-4.9/drivers/media/platform/sunxi-vin/vin-isp/isp_server/isp_math/matrix.c
 *
...
int check_input(double* x, double*y, char* c)
{
	...
	if (x == y) {
		printk("Error in %s: input equals output \n", c);
		out=1;
	}
	return out;
}
...
Matrix *create_matrix(int lines, int columns)
{
	...
	Matrix *a = hal_malloc(sizeof(struct Matrix));
	if (a == NULL) {
		printf("a malloc fail\n");
		return NULL;
	}
	...
	// set matrix's value
	a->value = hal_malloc(lines * sizeof(double *));
	if (a->value == NULL) {
		printf("a->value malloc fail\n");
		hal_free(a);
		return NULL;
	}
	memset(a->value, 0, lines * sizeof(double *));

	for (i = 0; i < lines; i++) {
		a->value[i] = hal_malloc(columns * sizeof(double));
		if (a->value[i] == NULL) {
			printf("a->value[i] malloc fail\n");
			return NULL;
		}
		memset(a->value[i], 0, columns * sizeof(double));
	}
