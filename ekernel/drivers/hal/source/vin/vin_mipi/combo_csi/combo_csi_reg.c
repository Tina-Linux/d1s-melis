...
unsigned long cmb_csi_top_base_addr;
unsigned long cmb_csi_phy_base_addr[VIN_MAX_MIPI];
unsigned long cmb_csi_port_base_addr[VIN_MAX_MIPI];

int cmb_csi_set_top_base_addr(unsigned long addr)
{
	cmb_csi_top_base_addr = addr;
...
int cmb_csi_set_phy_base_addr(unsigned int sel, unsigned long addr)
{
	if (sel > VIN_MAX_MIPI - 1)
		return -1;
	cmb_csi_phy_base_addr[sel] = addr;
...
int cmb_csi_set_port_base_addr(unsigned int sel, unsigned long addr)
{
	if (sel > VIN_MAX_MIPI - 1)
		return -1;
	cmb_csi_port_base_addr[sel] = addr;
