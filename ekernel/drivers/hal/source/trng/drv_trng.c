...
static int sunxi_trng_clk_init(bool enable)
{
    ...
    if (enable)
    {
        ...
        if (ret != HAL_CLK_STATUS_OK)
            hal_log_err("TRNG clock enable failed.\n");
    }
    else
    {
        clk = hal_clock_get(clk_type, clk_id);
        ...
        if (ret != HAL_CLK_STATUS_OK)
            hal_log_err("TRNG clock disable failed.\n");
        hal_clock_put(clk);
    }
