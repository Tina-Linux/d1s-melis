...
static s32 __get_offset(int i)
{
	...
	ret = Hal_Cfg_GetKeyValue(main_name, sub_key, (u32 *)&value, sizeof(value));
	if (ret < 0) {
...
s32 tv_init(void)
{
	...
	ret = Hal_Cfg_GetKeyValue(main_name, "interface",
					&interface, 1);
	if (ret < 0)
	...
	devm_extcon_dev_register(&pdev->dev, extcon_dev[sel]);
		...
		for (i = 0; i < DAC_COUNT; i++) {
			...
			ret = Hal_Cfg_GetKeyValue(main_name, sub_key,
						   &value, 4);
...
int tv_probe(void)
{
	...
	tv_power_enable_mask = 0;
	if (Hal_Cfg_GetKeyValue(main_name, "tv_power", (int32_t *)str, sizeof(str))) {
