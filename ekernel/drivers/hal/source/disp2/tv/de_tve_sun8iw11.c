...
static void *tve_reg_base[TVE_DEVICE_NUM];
static void *tve_top_reg_base[TVE_TOP_DEVIVE_NUM];
	...
	for (i = 0; i < TVE_DAC_NUM; i++) {
		if (dac_info[sel][i]) {
			...
			TVE_TOP_WUINT32(TVE_TOP_020 + 0x020*i,
					(i<<4)|((sel+1)<<0));
			*((unsigned int *)(0x0560002c)) = 0x023a;
...
s32 tve_low_set_tv_mode(u32 sel, enum disp_tv_mode mode, u32 *cali)
{
	u32 deflick  = 0;
	u32 reg_sync = 0;
	printf("tve_reg_base[%u] : %p \n", sel, tve_reg_base[sel]);
	...
	case DISP_TV_MOD_PAL:
		...
		*((unsigned int *)(0x0560400c)) = 0x02004000;
...
unsigned char tve_low_get_sid(u32 index)
{
	unsigned char efuse[TVE_DAC_NUM] = {0};
	char tv_name[20] = {0};
	...
	if (hal_efuse_read(tv_name, efuse, 128))
