...
void di_dev_set_reg_base(void *reg_base)
{
...
bool di_dev_is_finish(void)
{
	struct di_reg *reg = di_dev_get_reg_base();

	return reg->status.bits.finish_flag ? true : false;
}
...
static void di_dev_set_mdflag(struct di_client *client)
{
	...
	if (md_buf) {
		reg->mdflag_add.dwval = __va_to_pa((unsigned long)md_buf->vir_addr);
