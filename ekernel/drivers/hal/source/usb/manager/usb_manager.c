...
static void usb_device_scan_thread(void *pArg)
{
	...
	/*safe exit*/
	while (1) {
		if (!kthread_stop(scan_thread_handle))
			break;
		hal_msleep(3);
	}
#endif
...
static int32_t usb_script_parse(usb_cfg_t *cfg)
{
	...
	if (cfg->port.detect_type == USB_DETECT_TYPE_VBUS_ID) {
		...
		ret = Hal_Cfg_GetKeyValue(SET_USB0, KEY_USB_ID_GPIO, (int32_t *)&gpio_set,
		...
		/*get det vbus gpio */
		memset(&gpio_set, 0x00, sizeof(gpio_set));
		ret = Hal_Cfg_GetKeyValue(SET_USB0, KEY_USB_DETVBUS_GPIO, (int32_t *)&gpio_set,
					  sizeof(user_gpio_set_t) >> 2);
		...
		memset(&gpio_set, 0x00, sizeof(gpio_set));
		ret = Hal_Cfg_GetKeyValue(SET_USB0, KEY_USB_DRVVBUS_GPIO, (int32_t *)&gpio_set,
					  sizeof(user_gpio_set_t) >> 2);
...
int hal_usb_manager_init(void)
{
	...
	if (g_usb_cfg.port.port_type == USB_PORT_TYPE_DEVICE) {
		...
		scan_thread_handle = kthread_create(usb_device_scan_thread,
						    &g_usb_cfg,
						    "usb-device-scan",
		...
		scan_thread_handle = kthread_create(usb_host_scan_thread,
						    &g_usb_cfg,
						    "usb-host-scan",
						    HAL_THREAD_STACK_SIZE,
						    HAL_THREAD_PRIORITY_SYS);
		...
		if (g_usb_cfg.port.detect_mode == USB_DETECT_MODE_THREAD) {
			scan_thread_handle = kthread_create(usb_hardware_scan_thread,
							    &g_usb_cfg,
							    "usb-hardware-scan",
							    HAL_THREAD_STACK_SIZE,
							    HAL_THREAD_PRIORITY_SYS);
