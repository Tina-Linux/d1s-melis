...
// #define HCD_DEFER_RH_REGISTER(hcd) ((hcd)->flags & (1U << HCD_FLAG_DEFER_RH_REGISTER))
	 ...
	 */
// #define HCD_INTF_AUTHORIZED(hcd) ((hcd)->flags & (1U << HCD_FLAG_INTF_AUTHORIZED))
	...
	hal_workqueue *giveback_workqueue;
	hal_work giveback_workqueue_task;
	struct list_head giveback_urb_list;
	hal_spinlock_t giveback_urb_list_lock;
...
struct hc_driver {
	...
	const char	*product_desc;	/* product/vendor string */
	u32		hcd_priv_size;	/* size of private data */

	/* irq handler */
	hal_irqreturn_t (*irq)(struct usb_hcd *hcd);
...
#define	HCD_BH		0x0100		/* URB complete in BH context */
	...
	s32 (*suspend)(struct usb_hcd *hcd);	 // called after all devices were suspended

	/* called after entering D0 (etc), before resuming the hub */
	// int	(*pci_resume)(struct usb_hcd *hcd, bool hibernated);
	s32 (*resume)(struct usb_hcd *hcd);	 // called before any devices get resumed
	...
	s32 (*hub_notify)(struct usb_hcd *, u32 event);  // such as HUB_EVNET_PLUGIN_COMPLETE
...
// extern void usb_wakeup_notification(struct usb_device *hdev, unsigned int portnum);
...
struct usb_tt {
	...
	hal_work		clear_work;
	hal_workqueue		*hub_tt_work_queue;
};
...
#define NS_TO_US(ns)		DIV_ROUND_UP(ns, 1000L) /* convert nanoseconds to microseconds, rounding up */
