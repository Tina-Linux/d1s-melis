...
static void mscLun_Inquiry_free(__mscLun_t *mscLun)
{
	...
	dma_free_coherent(mscLun->Inquiry);
#else
...
#if ENABLE_DMA_ALLOC_COHERENT
		mscLun->Inquiry = dma_alloc_coherent(SCSI_MAX_INQUERY_LEN);
#else
...
static void mscDevScanThread(void *p_arg)
{
	...
	/*safe exit*/
	while (1) {
		if (!kthread_stop(mscDev->DevScanThreadId))
			break;
		hal_msleep(3);
	}
#endif
			   ...
			   const struct usb_device_id *table_item)
{
	...
	/* mscDev scan thread */
	mscDev->DevScanThreadId = kthread_run((void *)mscDevScanThread,
					      (void *)mscDev,
					      "mscDevScanThread");
	...
	return 0;
error7:
	kthread_stop(mscDev->DevScanThreadId);
error6:
	kthread_stop(mscDev->MainThreadId);
error5:
error4:
