...
typedef struct __mscDev {
	struct usb_device *pusb_dev; /* mscDev 对应的Public USB Device   */
	...
	hal_thread_t MainThreadId;    /* 主线程ID                     */
	hal_thread_t DevScanThreadId; /* 主线程ID                     */
	hal_thread_t MediaChangeId;   /* media change处理线程ID       */
...
typedef struct __mscLun {
	...
	unsigned char *Inquiry; /* 存放inquire命令获得的设备信息        */
