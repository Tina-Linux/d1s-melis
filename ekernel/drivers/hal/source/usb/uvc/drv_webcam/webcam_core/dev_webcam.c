...
static __s32 webcam_irq_handle(__u32 *addr0, __u32 *addr1, __u32 *addr2)
{
	...
	if (pWebcamDevNode->pts_flag == 0) {
		...
		if (pWebcamDevNode->CB_GetPts) {
			...
			if ((__u32)(pWebcamDevNode->pts) - pts
			    > webcam_mode_all.webcam_mode[valid_mode_idx].frame_period * (1.2)) {
	...
	if (frame_id != -1)  //在free中请求buffer成功
	{
		...
		pWebcamDevNode->full2->insert_element(pWebcamDevNode->full2,
						      pWebcamDevNode->frame_id_last);
...
__s32 webcam_check_mode_valid(void)
{
	...
	for (i = 0; i < webcam_mode_all.number; i++) {
		hal_log_err("mode%d:%dx%d@%d#color_fmt=%d buf_scmode=%d\n", i,
			    webcam_mode_all.webcam_mode[i].size.width,
			    webcam_mode_all.webcam_mode[i].size.height,
			    webcam_mode_all.webcam_mode[i].frame_rate,
			    (__u32)webcam_mode_all.webcam_mode[i].color_format,
			    webcam_mode_all.webcam_mode[i].webcam_buf_scrab_mode);
		...
		if ((webcam_mode_all.webcam_mode[i].color_format == webcam_mode.color_format)
		    ...
		    && (webcam_mode_all.webcam_mode[i].webcam_buf_scrab_mode
			== webcam_mode.webcam_buf_scrab_mode)) {
...
__s32 DEV_WEBCAM_Ioctrl(__hdle hWebcam, __u32 cmd, __s32 aux, void *pbuffer)
{
	...
	case DRV_WEBCAM_CMD_SET_FRAME_QUEUE: {
		...
		for (i = 0; i < pFrameQueue->num; i++) {  //现在num就等于WEBCAM_BUFFER_NUM，为下一步修改做准备
			// WEBCAM_BUFFER_NUM
			memcpy(&pWebcamDevNode->webcam_frame[i],
			       &pFrameQueue->webcam_frame_array[i], sizeof(__webcam_frame_t));
		}
		ret = EPDK_OK;
		break;
	}
	case DRV_WEBCAM_CMD_CAPTURE_ON: {
		...
		for (i = 0; i < pWebcamDevNode->webcam_frame_num; i++) {
			...
			pWebcamDevNode->webcam_frame[i].src_rect.width
			    = pWebcamDevNode->webcam_frame[i].dst_rect.width
			    = webcam_mode.size.width;
			pWebcamDevNode->webcam_frame[i].src_rect.height
			    = pWebcamDevNode->webcam_frame[i].dst_rect.height
			    = webcam_mode.size.height;
			...
			if (EPDK_OK != pWebcamDevNode->free2->insert_element(
				pWebcamDevNode->free2, pWebcamDevNode->webcam_frame[i].frame_id)) {
		...
		if (pWebcamDevNode->tran_cb != NULL) {
			...
			if (frame_id != -1) {  //在free中请求buffer成功
				...
				pWebcamDevNode->tran_cb(pWebcamDevNode->webcam_frame[index].addr[0],
							(__u32)NULL, (__u32)NULL);
	...
	case DRV_WENCAM_CMD_WEBCAM_INSERT: {
		...
		} else if (pWebcamDevNode->capture == CAPTURE_REMOVE
			   && pWebcamDevNode->tran_cb != NULL
			   && pWebcamDevNode->used_addr0 != (__u32)NULL) {
			hal_log_info("DRV_WENCAM_CMD_WEBCAM_INSERT 2\n");
			pWebcamDevNode->tran_cb(pWebcamDevNode->used_addr0, (__u32)NULL,
						(__u32)NULL);
			pWebcamDevNode->capture = CAPTURE_ON_AND_CALLBACK;
		}
...
static __s32 transport(__u32 addr0, __u32 addr1, __u32 addr2)
{
	...
	hal_log_info("usb:(%x,%x,%x),(%x,%x,%x), %x,%x\n", addr0, addr1, addr2,
		     UVCDev->frame_buf0, UVCDev->frame_buf1, UVCDev->frame_buf2,
		     (unsigned int)UVCDev->active_buf, (unsigned int)g_usbWebcam->uvc_irq);
	if ((void *)addr0 != NULL) {
...
int DEV_WEBCAM_Ioctrl(struct devfs_node *node, int cmd, void *pbuffer);
int DEV_WEBCAM_Open(struct devfs_node *node)
{
	__webcam_drv_t *p_webcam_drv = NULL;
	UVCDev_t *UVCDev = NULL;

	hal_log_info("DEV_WEBCAM_Open\n");

	p_webcam_drv = container_of(node, __webcam_drv_t, webcam_dev_entry);
	UVCDev = p_webcam_drv->UVCDev;
	g_usbWebcam = UVCDev->Extern;
	usbWebcam_v4l2_init(UVCDev);
#if 0
	DEV_WEBCAM_Ioctrl(node, DRV_WENCAM_CMD_GET_IRQ_CB, (void *)(&g_usbWebcam->uvc_irq));
	UVCDev->exchange_buf = g_usbWebcam->uvc_irq;
	DEV_WEBCAM_Ioctrl(node, DRV_WENCAM_CMD_SET_TRANSPORT_CB, (void *)transport);
	DEV_WEBCAM_Ioctrl(node, DRV_WENCAM_CMD_WEBCAM_INSERT, 0);
#endif
	//usbWebcam_v4l2_capture_on(UVCDev);

	return 1;
}

int DEV_WEBCAM_Close(struct devfs_node *node)
{
	__webcam_drv_t *p_webcam_drv = NULL;
	UVCDev_t *UVCDev = NULL;
...
ssize_t DEV_WEBCAM_Read(struct devfs_node *node, uint32_t addr, uint32_t size, void *data)
{
	return 0;
}

ssize_t DEV_WEBCAM_Write(struct devfs_node *node, uint32_t addr, uint32_t size, const void *data)
{
	return size;
}
...
int DEV_WEBCAM_Ioctrl(struct devfs_node *node, int cmd, void *pbuffer)
{
	...
	case VIDIOC_STREAMOFF:
		ret = uvc_v4l2_do_ioctl(UVCDev->v4l2_handle, cmd, pbuffer);
		break;
	case DRV_WEBCAM_CMD_INIT: {
		hal_log_err("webcam buffer number = %d\n", WEBCAM_BUFFER_NUM);
		for (i = 0; i < WEBCAM_BUFFER_NUM; i++) {
	...
	case DRV_WENCAM_CMD_GET_IRQ_CB: {
		...
		hal_log_info("DRV_WENCAM_CMD_GET_IRQ_CB, 0x%p\n", pWebcamDevNode->irq_cb);
		ret = EPDK_OK;
		*cb = pWebcamDevNode->irq_cb;
		hal_log_err("DRV_WENCAM_CMD_GET_IRQ_CB[%X]\n", pbuffer);
		break;
