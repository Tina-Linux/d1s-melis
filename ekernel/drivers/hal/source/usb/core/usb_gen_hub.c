			 ...
			 int retry_counter)
{
	...
	for (i = 0; (i < GET_DESCRIPTOR_TRIES) && (_dev_is_present(hub, port1) == 1);
		...
		if (USE_NEW_SCHEME(retry_counter)) {
			...
			buf = dma_alloc_coherent(buff_size);
#else
...
#if ENABLE_DMA_ALLOC_COHERENT
	hub->status = dma_alloc_coherent(sizeof(*hub->status));
#else
...
#if ENABLE_DMA_ALLOC_COHERENT
	hub->descriptor = dma_alloc_coherent(sizeof(*hub->descriptor));
#else
...
#if ENABLE_DMA_ALLOC_COHERENT
	hub->status_urb_ubff = dma_alloc_coherent(STATUS_URB_BUFF_LEN);
#else
...
static void hub_disconnect(struct usb_interface *intf)
{
	...
	if (hub->descriptor) {
#if ENABLE_DMA_ALLOC_COHERENT
		dma_free_coherent((void *)(hub->descriptor));
#else
	...
	if (hub->status) {
#if ENABLE_DMA_ALLOC_COHERENT
		dma_free_coherent((void *)(hub->status));
#else
	...
	if (hub->status_urb_ubff) {
#if ENABLE_DMA_ALLOC_COHERENT
		dma_free_coherent((void *)(hub->status_urb_ubff));
#else
