...
static int hub_port_suspend(struct usb_hub *hub, int port1, struct usb_device *udev)
{
	...
	if (udev->actconfig
	    ...
	    && (udev->actconfig->desc.bmAttributes & USB_CONFIG_ATT_WAKEUP) != 0) {
 ...
 */
static int __usb_suspend_device(struct usb_device *udev, int port1, pm_message_t state)
{
	int status;
	...
	if (udev->actconfig) {
		...
		for (i = 0; i < udev->actconfig->desc.bNumInterfaces; i++) {
			...
			if (driver->suspend) {
				...
				if (intf->dev.power.power_state != state || status)
					dev_err(&intf->dev,
	...
	if (!udev->parent) {
		...
		status = hub_port_suspend(hdev_to_hub(udev->parent), port1, udev);
...
static int finish_port_resume(struct usb_device *udev)
{
	 ...
	 */
	usb_set_device_state(udev, udev->actconfig ? USB_STATE_CONFIGURED : USB_STATE_ADDRESS);
...
static int hub_port_resume(struct usb_hub *hub, int port1, struct usb_device *udev)
{
	...
	status = clear_port_feature(hub->hdev, port1, USB_PORT_FEAT_SUSPEND);
	...
	if (status) {
		...
		msleep(25);
#define LIVE_FLAGS (USB_PORT_STAT_POWER | USB_PORT_STAT_ENABLE | USB_PORT_STAT_CONNECTION)
		/* Virtual root hubs can trigger on GET_PORT_STATUS to
		 * stop resume signaling.  Then finish the resume
		 * sequence.
		 */
		devstatus = portchange = 0;
		status = hub_port_status(hub, port1, &devstatus, &portchange);

		if (status < 0 || (devstatus & LIVE_FLAGS) != LIVE_FLAGS
		    || (devstatus & USB_PORT_STAT_SUSPEND) != 0) {
			dev_dbg(hub->intfdev, "port %d status %04x.%04x after resume, %d\n", port1,
				portchange, devstatus, status);
		} else {
			/* TRSMRCY = 10 msec */
			msleep(10);
...
int usb_resume_device(struct usb_device *udev)
{
	...
	if (!udev->parent) {
		...
		if (status == 0) {
			...
			udev->dev.power.power_state = PMSG_ON;
			status = hub_resume(udev->actconfig->interface[0]);
		}
	} else if (udev->state == USB_STATE_SUSPENDED) {
		// NOTE this fails if parent is also suspended...
		status = hub_port_resume(hdev_to_hub(udev->parent), port1, udev);
	} else {
		status = 0;
	}
	...
	for (port1 = 1; port1 <= hdev->maxchild; port1++) {
		...
		if (status == 0) {
			if (portchange & USB_PORT_STAT_C_SUSPEND) {
				clear_port_feature(hdev, port1, USB_PORT_FEAT_C_SUSPEND);
...
s32 usb_suspend_device(struct usb_device *virt_dev)
{
	return 0;
}

s32 usb_resume_device(struct usb_device *virt_dev)
{
	return 0;
}
