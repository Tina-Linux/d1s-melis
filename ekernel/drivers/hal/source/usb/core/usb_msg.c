 ...
 */
int usb_control_msg(struct usb_device *dev, u32 pipe, u8 request, u8 requesttype,
		    u16 value, u16 index, void *data, u16 size, s32 timeout)
{
	int ret = 0;
	struct usb_ctrlrequest *dr = NULL;
#if ENABLE_DMA_ALLOC_COHERENT
	dr = dma_alloc_coherent(sizeof(struct usb_ctrlrequest));
#else
	dr = hal_malloc_align(sizeof(struct usb_ctrlrequest), CACHELINE_LEN);
#endif
	if (!dr) {
	...
	EHCI_DEBUG_PRINTF("req:%s(%d), req_type:0x%X",
				request == USB_REQ_GET_STATUS ? "GET_STATUS" :
				request == USB_REQ_CLEAR_FEATURE ? "CLEAR_FEATURE" :
				request == USB_REQ_SET_FEATURE ? "SET_FEATURE" :
				request == USB_REQ_SET_ADDRESS ? "SET_ADDRESS" :
				request == USB_REQ_GET_DESCRIPTOR ? "GET_DESCRIPTOR" :
				request == USB_REQ_SET_CONFIGURATION ? "SET_CONFIGURATION" :
				request == USB_REQ_SET_INTERFACE ? "SET_INTERFACE" :
				request == USB_REQ_SET_INTERFACE ? "SET_INTERFACE" : "??",
				request,
				requesttype);
	...
	if (dr) {
#if ENABLE_DMA_ALLOC_COHERENT
		dma_free_coherent((void *)dr);
#else
		hal_free_align((void *)dr);
#endif
		dr = NULL;
	} else {
 ...
 */
/* 带重试的ctrl传输，的desc */
s32 usb_get_descriptor(struct usb_device *dev, u8 type, u8 index, void *buf, s32 size)
{
	int i = 0;
	int result = 0;
...
int usb_string(struct usb_device *dev, int index, char *buf, u32 size)
{
	...
	buf[0] = 0;
#if ENABLE_DMA_ALLOC_COHERENT
	tbuf = dma_alloc_coherent(256);
#else
	tbuf = hal_malloc_align(256, CACHELINE_LEN);
#endif
 ...
 */
static void usb_enable_endpoint(struct usb_device *dev, struct usb_host_endpoint *ep)
{
...
s32 usb_set_configuration(struct usb_device *virt_father_dev, s32 configuration)
{
	int i = 0, ret = 0;
	struct usb_host_config *virt_confg = NULL;
	...
	if (virt_confg) {
		...
		if (nintf > USB_MAX_VIRT_SUB_DEV_NR) {
			__err("PANIC : Set_Conifg : too many interface,now we only support %d interfaces",
				USB_MAX_VIRT_SUB_DEV_NR);
			return -EINVAL;
		}
...
free_interfaces:
	...
	if (!virt_confg) {
		...
		for (i = 0; i < nintf; ++i) {
			...
			virt_confg->interface[i] = intf = new_interfaces[i];
		...
		for (i = 0; i < nintf; ++i) {
			struct usb_interface *intf = virt_confg->interface[i];
			__inf("[usbh core]: adding sub dev  (config #%d, interface %d)",
			      configuration, intf->altsetting[0].desc.bInterfaceNumber);
...
#if 0
s32 usb_get_device_descriptor(struct usb_device *dev, u32 size)
{
	...
	desc = malloc(sizeof(struct usb_device_descriptor), USB_MEM_FILE_TRIGGER,
...
s32 usb_get_device_descriptor(struct usb_device *dev, u32 size)
{
	struct usb_device_descriptor desc __attribute__((aligned(64)));
	s32 ret = 0;
...
int usb_get_status(struct usb_device *dev, s32 type, s32 target, void *data)
{
	s32 ret = 0;
#if ENABLE_DMA_ALLOC_COHERENT
	u16 *status = dma_alloc_coherent(sizeof(*status));
#else
	u16 *status = hal_malloc_align(sizeof(*status), CACHELINE_LEN);
#endif
	...
	*(u16 *)data = *status;
	...
	if (status) {
#if ENABLE_DMA_ALLOC_COHERENT
		dma_free_coherent(status);
#else
		hal_free_align(status);
#endif
		status = NULL;
	} else {
		__err("ERR: parameter is NULL, can't free");
...
int usb_set_interface(struct usb_device *dev, int interface, int alternate)
{
	...
	if (manual) {
		...
		for (i = 0; i < alt->desc.bNumEndpoints; i++) {
			...
			u32 pipe = __create_pipe(dev, USB_ENDPOINT_NUMBER_MASK & epaddr)
