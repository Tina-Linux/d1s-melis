...
static int _usb_parse_endpoint_desc(int cfgno,
				    int inum,
				    int asnum,
...
skip_to_next_endpoint_or_interface_descriptor:
	i = _find_next_desc(buffer, size, USB_DT_ENDPOINT, USB_DT_INTERFACE, NULL);
	return buffer - buffer0 + i;
}
				  ...
				  int size)
{
	...
	for ((buffer2 = buffer, size2 = size); size2 > 0;
	...
	for (i = 0; i < interface_nr; ++i) {
		...
		len = sizeof(struct usb_host_interface) * j;
...
/* 获得confg描述符，并parser之 */
int usb_get_all_config_desc_and_parser(struct usb_device *dev)
{
	int result      = -ENOMEM;
	...
	buffer = dma_alloc_coherent(USB_DT_CONFIG_SIZE);
#else
	buffer = hal_malloc_align(USB_DT_CONFIG_SIZE, CACHELINE_LEN);
#endif
	...
	for (cur_cfg_nr = 0; cur_cfg_nr < ncfg; cur_cfg_nr++) {
		...
		result = usb_get_descriptor(dev, USB_DT_CONFIG, cur_cfg_nr, buffer,
					    USB_DT_CONFIG_SIZE);
...
#if ENABLE_DMA_ALLOC_COHERENT
		bigbuffer = dma_alloc_coherent(length);
#else
		bigbuffer = hal_malloc_align(length, CACHELINE_LEN);
#endif
		...
		if (result < 0) {
			hal_log_err("PANIC : [ hub] : unable to read config index %d "
				    "descriptor/%s\n", cur_cfg_nr, "all");
#if ENABLE_DMA_ALLOC_COHERENT
			dma_free_coherent(bigbuffer);
#else
			hal_free_align(bigbuffer);
#endif
			bigbuffer = NULL;
			goto err;
		}
...
#if ENABLE_DMA_ALLOC_COHERENT
				dma_free_coherent(dev->rawdescriptors[i]);
#else
				hal_free_align(dev->rawdescriptors[i]);
#endif
