...
{
	volatile uint32_t *status = (uint32_t *)(sunxi_uart_port[uart_port] + UART_USR);
...
static void uart_pinctrl_init(uart_port_t uart_port)
{
	...
	switch (uart_port)
	{
		...
		case UART_3:
			...
			break;
		case UART_4:
			hal_gpio_pinmux_set_function(UART4_TX, UART4_GPIO_FUNCTION);//TX
			hal_gpio_pinmux_set_function(UART4_RX, UART4_GPIO_FUNCTION);//RX
			break;
		case UART_5:
			hal_gpio_pinmux_set_function(UART5_TX, UART5_GPIO_FUNCTION);//TX
			hal_gpio_pinmux_set_function(UART5_RX, UART5_GPIO_FUNCTION);//RX
			break;
		default:
...
static void uart_pinctrl_uninit(uart_port_t uart_port)
{
	...
	switch (uart_port)
	{
		...
		case UART_3:
			...
			break;
		case UART_4:
			hal_gpio_pinmux_set_function(UART4_TX, GPIO_MUXSEL_DISABLED);//TX
			hal_gpio_pinmux_set_function(UART4_RX, GPIO_MUXSEL_DISABLED);//RX
			break;
		case UART_5:
			hal_gpio_pinmux_set_function(UART5_TX, GPIO_MUXSEL_DISABLED);//TX
			hal_gpio_pinmux_set_function(UART5_RX, GPIO_MUXSEL_DISABLED);//RX
			break;
		default:
			UART_ERR("[uart%d] not support \n", uart_port);
			break;
