...
int main_sample_aec_o(int argc, char *argv[])
{
    int result = 0;
...
//    pthread_create(&stContext.mAiSaveDataTid, NULL, getFrameThread, &stContext);
	...
	aloge("zjx_chk_vol:%d",stContext.mConfigPara.volume);
...
FINSH_FUNCTION_EXPORT_ALIAS(main_sample_aec_o, __cmd_sample_aec_o, test mpp aec_o);
