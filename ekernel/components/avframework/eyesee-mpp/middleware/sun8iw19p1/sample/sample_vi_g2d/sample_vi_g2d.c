...
//./ekernel/drivers/hal/source/g2d_rcq/g2d_driver.h
...
/** For some scenes that do not need to be displayed, you can disable this macro. */
...
static ERRORTYPE loadSampleViG2dConfig(SampleViG2dConfig *pConfig, const char *pConfPath)
{
    ...
    strcpy(pConfig->mStoreDir, "/mnt/E");
    ...
    if(pConfPath != NULL)
    {
        ...
        /* src pic format */
        char *pStrSrcPixelFormat = (char*)GetConfParaString(&stConfParser, SAMPLE_VIG2D_KEY_SRC_PIC_FORMAT, NULL);
        if(!strcmp(pStrSrcPixelFormat, "nv21"))
        {
            pConfig->mSrcPicFormat = MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420;
        }
        else if(!strcmp(pStrSrcPixelFormat, "nv12"))
        {
            pConfig->mSrcPicFormat = MM_PIXEL_FORMAT_YUV_SEMIPLANAR_420;
        }
        else
        {
            aloge("fatal error! conf file src_pic_format[%s] is unsupported", pStrSrcPixelFormat);
            pConfig->mSrcPicFormat = MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420;
        }
        /* dst pic format */
        char *pStrDstPixelFormat = (char*)GetConfParaString(&stConfParser, SAMPLE_VIG2D_KEY_DST_PIC_FORMAT, NULL);
        if(!strcmp(pStrDstPixelFormat, "nv21"))
        {
            pConfig->mDstPicFormat = MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420;
        }
        else if(!strcmp(pStrDstPixelFormat, "nv12"))
        {
            pConfig->mDstPicFormat = MM_PIXEL_FORMAT_YUV_SEMIPLANAR_420;
        }
        else if(!strcmp(pStrDstPixelFormat, "rgb888"))
        {
            pConfig->mDstPicFormat = MM_PIXEL_FORMAT_RGB_888;
        }
        else
        {
            aloge("fatal error! conf file dst_pic_format[%s] is unsupported", pStrDstPixelFormat);
            pConfig->mDstPicFormat = MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420;
        }
    ...
    alogd("dev_num=%d, frameRate=%d, srcPixFmt=0x%x, dstPixFmt=0x%x, dropFrameNum=%d,srcSize=[%dx%d], srcRect=[%d,%d,%dx%d],"
       ...
       pConfig->mDevNum, pConfig->mFrameRate, pConfig->mSrcPicFormat, pConfig->mDstPicFormat, pConfig->mDropFrameNum, 
...
static int saveSrcPicture(SampleViCapS *pViCapCtx, VIDEO_FRAME_INFO_S *pFrameInfo)
{
    int ret = 0;
    ...
    char strFilePath[MAX_FILE_PATH_SIZE+64];

    snprintf(strFilePath, MAX_FILE_PATH_SIZE+64, "%s/src_pic[%d].%s", pContext->mConfigPara.mStoreDir, pViCapCtx->mRawStoreNum, strPixFmt);
    ...
    if(fpPic != NULL)
    {
        ...
        for(i=0; i<3; i++)
        {
            if ((pFrameInfo->VFrame.mpVirAddr[i] != NULL) && (0 != yuvSize[i]))
            {
                /** The frame is come from vipp, can NOT flush in here. */
        ...
        fclose(fpPic);
        alogd("store src frame in file[%s]", strFilePath);
        ret = 0;
    }
    else
    {
...
static int saveDstPicture(SampleViCapS *pViCapCtx, VIDEO_FRAME_INFO_S *pFrameInfo)
{
    int ret = 0;
    SampleViG2dContext *pContext = (SampleViG2dContext*)pViCapCtx->mpContext;
    //make file name, e.g., /mnt/extsd/pic[0].NV21M
    ...
    snprintf(strFilePath, MAX_FILE_PATH_SIZE+64, "%s/dst_pic[%d].%s", pContext->mConfigPara.mStoreDir, pViCapCtx->mRawStoreNum, strPixFmt);
    ...
    if(fpPic != NULL)
    {
        ...
        fclose(fpPic);
        alogd("store dst frame in file[%s]", strFilePath);
        ret = 0;
    }
    else
    {
...
static void *GetCSIFrameThread(void *pArg)
{
    ...
    while (1)
    {
        ...
        //get frame from vipp.
        if ((ret = AW_MPI_VI_GetFrame(pViCapCtx->mDev, pViCapCtx->mChn, &stSrcFrameInfo, 200)) != 0)
        {
        ...
        if(0 == ret)
        {
            /* save src and dst frame */
            if(pViCapCtx->mRawStoreNum < pContext->mConfigPara.mDstStoreCount)
            {
                if(0 == i%pContext->mConfigPara.mDstStoreInterval)
                {
                    ret = saveSrcPicture(pViCapCtx, &stSrcFrameInfo);
                    if(ret != 0)
                    {
                        aloge("fatal error! save src picture fail[%d]", ret);
                    }
                    ret = saveDstPicture(pViCapCtx, pDstFrameInfo);
                    if(ret != 0)
                    {
                        aloge("fatal error! save dst picture fail[%d]", ret);
                    }
        ...
        VIDEO_FRAME_INFO_S *pFrameInfo = pContext->mpFrameManager->GetReadyFrame(pContext->mpFrameManager, 200);
        if(NULL != pFrameInfo)
        {
            ret = pContext->mpFrameManager->UsingFrameDone(pContext->mpFrameManager, pFrameInfo);
            if(ret != 0)
            {
...
static int SampleViG2d_G2dConvert_G2dConvertVideoFrame(SampleViG2d_G2dConvert *pThiz, VIDEO_FRAME_INFO_S* pSrcFrameInfo, VIDEO_FRAME_INFO_S* pDstFrameInfo)
{
    ...
    if (G2D_FORMAT_YUV420UVC_V1U1V0U0 != eSrcFormat)
    {
        alogd("eSrcFormat %d != 40 !!!!!!!!!!!!!!!!!!!!!!", eSrcFormat);
    }
    ...
    alogv("debug g2d[0x%x]: dst virAddr[0x%x][0x%x], phyAddr[0x%x][0x%x], size[%dx%d], src virAddr[0x%x][0x%x], phyAddr[0x%x][0x%x], size[%dx%d]", pThiz->mG2dFd,
       pDstFrameInfo->VFrame.mpVirAddr[0], pDstFrameInfo->VFrame.mpVirAddr[1], pDstFrameInfo->VFrame.mPhyAddr[0], pDstFrameInfo->VFrame.mPhyAddr[1],
       pDstFrameInfo->VFrame.mWidth, pDstFrameInfo->VFrame.mHeight,
       pSrcFrameInfo->VFrame.mpVirAddr[0], pSrcFrameInfo->VFrame.mpVirAddr[1], pSrcFrameInfo->VFrame.mPhyAddr[0], pSrcFrameInfo->VFrame.mPhyAddr[1],
       pSrcFrameInfo->VFrame.mWidth, pSrcFrameInfo->VFrame.mHeight);
        ...
        if(pManager->mBufWidth * pManager->mBufHeight > 0)
        {
            switch(pManager->mPixelFormat)
            {
                ...
                case MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420:
                {
                    ...
                    alogd("[0]PhyAddr:%x, VirAddr:%x, size:%d", pNode->VFrame.VFrame.mPhyAddr[0], pNode->VFrame.VFrame.mpVirAddr[0], pManager->mBufWidth*pManager->mBufHeight);
                    ...
                    alogd("[1]PhyAddr:%x, VirAddr:%x, size:%d", pNode->VFrame.VFrame.mPhyAddr[1], pNode->VFrame.VFrame.mpVirAddr[1], pManager->mBufWidth*pManager->mBufHeight/2);
                    break;
                }
                ...
                case MM_PIXEL_FORMAT_RGB_888:
                {
                    ret = AW_MPI_SYS_MmzAlloc_Cached(&pNode->VFrame.VFrame.mPhyAddr[0], &pNode->VFrame.VFrame.mpVirAddr[0], pManager->mBufWidth*pManager->mBufHeight*3);
                    if(ret != SUCCESS)
                    {
                        pNode->VFrame.VFrame.mPhyAddr[0] = 0;
                        pNode->VFrame.VFrame.mpVirAddr[0] = NULL;
...
{
    SampleViG2dContext* pContext = (SampleViG2dContext*) pThreadData;
    int ret;
    ...
    prctl(PR_SET_NAME, (unsigned long)"sample_vi_g2d_DetectInput", 0, 0, 0);
    while(1)
    {
...
//    }
    ...
    if (ret != 0)
    {
        aloge("fatal error! pthread create fail[0x%x]", ret);
        goto _err1;
    }
    ...
    pContext->mpFrameManager = constructSampleViG2d_VideoFrameManager(5, pContext->mConfigPara.mDstPicFormat, pContext->mConfigPara.mDstWidth, pContext->mConfigPara.mDstHeight);
    ...
    pContext->mViCapCtx.mViAttr.format.pixelformat = map_PIXEL_FORMAT_E_to_V4L2_PIX_FMT(pContext->mConfigPara.mSrcPicFormat); // V4L2_PIX_FMT_NV21M; 
    ...
    pContext->mViCapCtx.mTimeout = 2000;
    ...
    if (0 == ret)
    {
        ...
        alogd("vipp[%d]virChn[%d]: get csi frame thread id=[0x%lx]", pContext->mViCapCtx.mDev, pContext->mViCapCtx.mChn, pContext->mViCapCtx.mThreadId);
    }
    else
    {
    ...
    ret = AW_MPI_VO_EnableChn(pContext->mVoDisplayCtx.mVoLayer, pContext->mVoDisplayCtx.mVOChn);
    if(ret != SUCCESS)
    {
    ...
    if (0 == ret)
    {
        ...
        alogd("pthread create display threadId[0x%lx]", pContext->mVoDisplayCtx.mThreadId);
    }
    else
    {
    ...
    alogd("VoDisplay threadId[0x%lx] exit[%d], pValue[%p]", pContext->mVoDisplayCtx.mThreadId, ret, pValue);
    pContext->mVoDisplayCtx.mbThreadExistFlag = false;
#endif

    ret = pthread_join(pContext->mViCapCtx.mThreadId, (void*)&pValue);
    alogd("ViCap threadId[0x%lx] exit[%d], pValue[%p]", pContext->mViCapCtx.mThreadId, ret, pValue);
    ...
    ret = AW_MPI_VO_DisableChn(pContext->mVoDisplayCtx.mVoLayer, pContext->mVoDisplayCtx.mVOChn);
    if(ret != SUCCESS)
    {
    ...
    ret = AW_MPI_VI_DestoryVipp(pContext->mViCapCtx.mDev);
    if(ret != SUCCESS)
    {
...
_err5:
    ...
    if(ret != 0)
    {
        aloge("fatal error! ViCap threadId[0x%lx] join fail[%d]", pContext->mViCapCtx.mThreadId, ret);
    }
_err4:
    destructSampleViG2d_G2dConvert(pContext->mViCapCtx.mpG2dConvert);
    pContext->mViCapCtx.mpG2dConvert = NULL;
...
FINSH_FUNCTION_EXPORT_ALIAS(main_sample_vi_g2d, __cmd_sample_vi_g2d, "test g2d rotate,resize,crop");
