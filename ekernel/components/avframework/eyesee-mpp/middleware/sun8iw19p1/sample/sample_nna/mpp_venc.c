...
static ERRORTYPE MPPCallbackWrapper(void *cookie, MPP_CHN_S *pChn, MPP_EVENT_TYPE event, void *pEventData)
{
    ...
    VENC_Params  *pVENC = (VENC_Params *) cookie;
    switch (event)
    {
    case MPP_EVENT_RELEASE_VIDEO_BUFFER:
        if (pFrame != NULL)
        {
            //AW_MPI_VI_ReleaseFrame(0, 0, pFrame);
			//printf("\n\n--s-MPP_EVENT_RELEASE_VIDEO_BUFFER\n\n");
        }
        break;
...
int create_venc(VENC_Params* pVENCParams, VENC_CHN chn, int type)
{
    ...
    if (PT_H264 == mVencChnAttr.VeAttr.Type)
    {
        ...
        default:
            ...
            mVencChnAttr.RcAttr.mAttrH264Cbr.mMaxQp = 51;
            mVencChnAttr.RcAttr.mAttrH264Cbr.mMinQp = 1;
            break;
        }
    }
