...
/*MPI VI*/
static int aw_vipp_creat(VI_DEV ViDev, VI_ATTR_S *pstAttr)
{
    int ret = -1;
...
static int aw_virvi_creat(VI_DEV ViDev, VI_CHN ViCh, void *pAttr)
{
    int ret = -1;
...
static int aw_vipp_destory(VI_DEV ViDev)
{
    int ret = -1;
...
static int aw_virvi_destory(VI_DEV ViDev, VI_CHN ViCh)
{
    int ret = -1;
...
/*MPI ISE*/
static int aw_iseport_creat(ISE_GRP IseGrp, ISE_CHN IsePort, ISE_CHN_ATTR_S *PortAttr)
{
    int ret = -1;
...
static int aw_iseport_destory(ISE_GRP IseGrp, ISE_CHN IsePort)
{
    int ret = -1;
...
static int aw_isegroup_creat(ISE_GRP IseGrp, ISE_GROUP_ATTR_S *pGrpAttr)
{
    int ret = -1;
...
static int aw_isegroup_destory(ISE_GRP IseGrp)
{
    int ret = -1;
...
static ERRORTYPE loadSampleVirvi2Fish2VencConfig(SampleVirvi2Fish2VencConfig *pConfig, const char *conf_path)
{
    ...
    pConfig->VencChnConfig[0].DestFrameRate = 60;
    pConfig->VencChnConfig[0].DestBitRate = 8*1024*1024;
    strcpy(pConfig->VencChnConfig[0].OutputFilePath, "/mnt/E/AW_FishEncoderVideoChn0.H264");
...
int main_sample_virvi2fish2venc(int argc, char *argv[])
{
...
FINSH_FUNCTION_EXPORT_ALIAS(main_sample_virvi2fish2venc, __cmd_sample_virvi2fish2venc, test mpp virvi2fish2venc);
