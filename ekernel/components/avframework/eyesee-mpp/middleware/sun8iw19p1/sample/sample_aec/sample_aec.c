...
int initSampleAecFrameManager(SampleAecFrameManager *pFrameManager, int nFrameNum, SampleAecConfig *pConfigPara)
{
    ...
    else if (MOD_ID_AI == pChn->mModId)
    {
        ...
        switch(event)
        {
            case MPP_EVENT_CAPTURE_AUDIO_DATA:
            {
                ...
                alogd("AIChannel transport pcm:%d bytes in tunnel-mode", pDataInfo->mLen);
                break;
            }
            default:
...
{
    int ret = 0;
    strcpy(pConfig->mPcmSrcPath, "/mnt/E/sample_aoref_8000_ch1_bit16_aec_30s.wav");
    strcpy(pConfig->mPcmDstPath, "/mnt/E/ai_cap.wav");
...
static void* SampleDetectInputThread(void* pThreadData)
{
    ...
    prctl(PR_SET_NAME, (unsigned long)"sample_aec_DetectInput", 0, 0, 0);
    while(1)
    {
...
int main_sample_aec(int argc, char *argv[])
{
    int result = 0;
    void *pValue = NULL;
...
//    }
    ...
    else
    {
        ...
        if(stContext.mConfigPara.mSampleRate != nSampleRate
            || stContext.mConfigPara.mChannelCnt != nChnNum
            || stContext.mConfigPara.mBitWidth != nBitsPerSample)
        {
            aloge("fatal error! sample aec param [%d][%d][%d]is not match!", stContext.mConfigPara.mChannelCnt, stContext.mConfigPara.mSampleRate, stContext.mConfigPara.mBitWidth);
        }
    }
    //init mpp system
    ...
    AW_MPI_SYS_Init();
    //init frame manager
    initSampleAecFrameManager(&stContext.mFrameManager, 5, &stContext.mConfigPara);
...
//    pthread_create(&stContext.mAiSaveDataTid, NULL, getFrameThread, &stContext);
    ...
    if(0 == result)
    {
        alogd("pthread create send_pcm_ao_thread[0x%lx]!", stContext.mSendPcmAoTid);
    }
    else
    {
    ...
    if(0 == result)
    {
        alogd("sendPcmAoTid[0x%lx] is joined!", stContext.mSendPcmAoTid);
    }
    else
    {
        aloge("fatal error! sendPcmAoTid[0x%lx] join fail[%d]!", stContext.mSendPcmAoTid, result);
    }
    result = pthread_join(stContext.mAiSaveDataTidPcm, NULL);
    if(0 == result)
    {
        alogd("aiSaveDataTidPcm[0x%lx] is joined!", stContext.mAiSaveDataTidPcm);
    }
    else
    {
        aloge("fatal error! aiSaveDataTidPcm[0x%lx] join fail[%d]!", stContext.mAiSaveDataTidPcm, result);
    }
    ...
    AW_MPI_AO_Disable(stContext.mAIODev);
...
_exit:
    ...
    alogd("sample_aec exit[%d]", result);
    //log_quit();
    return result;
}
FINSH_FUNCTION_EXPORT_ALIAS(main_sample_aec, __cmd_sample_aec, test mpp aec);
