...
#define VENC_DEFAULT_SRC_SIZE   1080
#define VENC_DEFAULT_DST_SIZE   1080

#define VENC_DEFAULT_BITRATE  (10*1024*1024)
#define VENC_DEFAULT_FRAMERATE  30

#define VENC_DEFAULT_SRC_PIXFMT    MM_PIXEL_FORMAT_YUV_PLANAR_420
#define VENC_DEFAULT_DST_PIXFMT    MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420
#define VENC_DEFAULT_ENCODER   PT_H264

#define VENC_DEFAULT_YUV_SRC_PATH  "/mnt/extsd/sample_venc/1080p.yuv"
#define VENC_DEFAULT_YUV_DST_PATH  "/mnt/extsd/sample_venc/1080p.mov"
...
static SAMPLE_VENC_PARA_S *pVencPara;
#if 0
...
static ERRORTYPE initVencPara(void)
{
    if (pVencPara != NULL)
    {
       alogd("para has been init");
       return SUCCESS;
    }
    ...
    pVencPara->mConfigPara.srcSize  = VENC_DEFAULT_SRC_SIZE;
    if (pVencPara->mConfigPara.srcSize == 3840)
    {
        pVencPara->mConfigPara.srcWidth = 3840;
        pVencPara->mConfigPara.srcHeight= 2160;
    }
    else if (pVencPara->mConfigPara.srcSize == 1080)
    {
        pVencPara->mConfigPara.srcWidth = 1920;
        pVencPara->mConfigPara.srcHeight= 1080;
    }
    else if(pVencPara->mConfigPara.srcSize == 720)
    {
        pVencPara->mConfigPara.srcWidth = 1280;
        pVencPara->mConfigPara.srcHeight= 720;
    }

    pVencPara->mConfigPara.srcPixFmt = VENC_DEFAULT_SRC_PIXFMT;

    pVencPara->mConfigPara.dstSize  = VENC_DEFAULT_DST_SIZE;
    if (pVencPara->mConfigPara.dstSize == 3840)
    {
        pVencPara->mConfigPara.dstWidth = 3840;
        pVencPara->mConfigPara.dstHeight= 2160;
    }
    else if (pVencPara->mConfigPara.dstSize == 1080)
    {
        pVencPara->mConfigPara.dstWidth = 1920;
        pVencPara->mConfigPara.dstHeight= 1080;
    }
    else if (pVencPara->mConfigPara.dstSize == 720)
    {
        pVencPara->mConfigPara.dstWidth = 1280;
        pVencPara->mConfigPara.dstHeight= 720;
    }
    pVencPara->mConfigPara.dstPixFmt = VENC_DEFAULT_DST_PIXFMT;

    pVencPara->mConfigPara.mVideoEncoderFmt = VENC_DEFAULT_ENCODER;
    pVencPara->mConfigPara.mField = VIDEO_FIELD_FRAME;
    pVencPara->mConfigPara.mVideoBitRate = VENC_DEFAULT_BITRATE;
    pVencPara->mConfigPara.mVideoFrameRate = VENC_DEFAULT_FRAMERATE;
    //pVencPara->mConfigPara.maxKeyFrame = VENC_DEFAULT_FRAMERATE;

    strcpy(pVencPara->mConfigPara.intputFile, VENC_DEFAULT_YUV_SRC_PATH);
    strcpy(pVencPara->mConfigPara.outputFile, VENC_DEFAULT_YUV_DST_PATH);
...
static ERRORTYPE loadConfigPara(SAMPLE_VENC_PARA_S *pVencPara, char *conf_path)
{
    ...
    pVencPara->mConfigPara.srcSize = GetConfParaInt(&mConf, VENC_CFG_SRC_SIZE, 0);
    if (pVencPara->mConfigPara.srcSize == 3840)
    {
        pVencPara->mConfigPara.srcWidth  = 3840;
        pVencPara->mConfigPara.srcHeight = 2160;
    ...
    else if (pVencPara->mConfigPara.srcSize == 480)
    {
        pVencPara->mConfigPara.srcWidth  = 640;
        pVencPara->mConfigPara.srcHeight = 480;
    }

    ptr = (char *)GetConfParaString(&mConf, VENC_CFG_DST_FILE_STR, NULL);
    strcpy(pVencPara->mConfigPara.outputFile, ptr);
    ptr = (char *)GetConfParaString(&mConf, VENC_CFG_DST_ENCODER, NULL);
    if (!strcmp(ptr, "H.264"))
    {
        alogd("h.264 encoder");
        pVencPara->mConfigPara.mVideoEncoderFmt = PT_H264;
    }
    else if (!strcmp(ptr, "H.265"))
    {
        alogd("h.265 encoder");
        pVencPara->mConfigPara.mVideoEncoderFmt = PT_H265;
    }
    else if (!strcmp(ptr, "MJPEG"))
    {
        alogd("mjpeg encoder");
        pVencPara->mConfigPara.mVideoEncoderFmt = PT_MJPEG;
    }
    else
    {
    ...
    pVencPara->mConfigPara.srcPixFmt = MM_PIXEL_FORMAT_YUV_PLANAR_420;
    ptr = (char *)GetConfParaString(&mConf, VENC_CFG_SRC_PIXFMT, NULL);
    if (ptr != NULL)
    {
        if (!strcmp(ptr, "nv21"))
        {
            pVencPara->mConfigPara.srcPixFmt = MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420;
        }
        else if (!strcmp(ptr, "aw_afbc"))
        {
            pVencPara->mConfigPara.srcPixFmt = MM_PIXEL_FORMAT_YUV_AW_AFBC;
        }
    }

    pVencPara->mConfigPara.dstSize = GetConfParaInt(&mConf, VENC_CFG_DST_SIZE, 0);
    alogd("get dst_size: %dp ", pVencPara->mConfigPara.dstSize);
    if (pVencPara->mConfigPara.dstSize == 3840)
    {
        pVencPara->mConfigPara.dstWidth  = 3840;
        pVencPara->mConfigPara.dstHeight = 2160;
    ...
    else if (pVencPara->mConfigPara.dstSize == 480)
    {
        pVencPara->mConfigPara.dstWidth  = 640;
        pVencPara->mConfigPara.dstHeight = 480;
    }

    pVencPara->mConfigPara.mEncUseProfile = GetConfParaInt(&mConf, VENC_CFG_DST_ENC_PROFILE, 0);
    ...
    alogd("enc_fmt=%d,profile=%d, rc_mode=%d, src_pix_fmt=%d", pVencPara->mConfigPara.mVideoEncoderFmt, \
        pVencPara->mConfigPara.mEncUseProfile, pVencPara->mConfigPara.mRcMode, pVencPara->mConfigPara.srcPixFmt);
    alogd("src=%s, dst=%s, src_size=%d, dst_size=%d, frate=%d, brate=%d, rotate=%d", pVencPara->mConfigPara.intputFile, \
        pVencPara->mConfigPara.outputFile, pVencPara->mConfigPara.srcSize, pVencPara->mConfigPara.dstSize, \
        pVencPara->mConfigPara.mVideoFrameRate, pVencPara->mConfigPara.mVideoBitRate, \
        pVencPara->mConfigPara.rotate);
...
static ERRORTYPE configVencChnAttr(SAMPLE_VENC_PARA_S *pVencPara)
{
    ...
    //pVencPara->mVencChnAttr.VeAttr.MaxKeyInterval = pVencPara->mConfigPara.mVideoMaxKeyItl;
    ...
    //pVencPara->mVencChnAttr.VeAttr.PixelFormat = pVencPara->mConfigPara.dstPixFmt;
    ...
    {
        pVencPara->mVencChnAttr.VeAttr.PixelFormat = MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420;
    }
    ...
    if (PT_H264 == pVencPara->mVencChnAttr.VeAttr.Type)
    {
        ...
        //pVencPara->mVencChnAttr.VeAttr.AttrH264e.Profile = VENC_H264ProfileHigh;
        ...
        case 1:
            ...
            pVencPara->mVencChnAttr.RcAttr.mAttrH264Vbr.mMovingTh = 20;
            pVencPara->mVencChnAttr.RcAttr.mAttrH264Vbr.mQuality = 5;
            break;
        case 2:
    ...
    else if (PT_H265 == pVencPara->mVencChnAttr.VeAttr.Type)
    {
        ...
        case 1:
            ...
            pVencPara->mVencChnAttr.RcAttr.mAttrH265Vbr.mMovingTh = 20;
            pVencPara->mVencChnAttr.RcAttr.mAttrH265Vbr.mQuality = 5;
            break;
        case 2:
...
static void writeSpsPpsHead(SAMPLE_VENC_PARA_S *pVencPara)
{
    ...
    if (pVencPara->mConfigPara.mVideoEncoderFmt == PT_H264)
    {
        ...
        AW_MPI_VENC_GetH264SpsPpsInfo(pVencPara->mVeChn, &pH264SpsPpsInfo);
    ...
    else if (pVencPara->mConfigPara.mVideoEncoderFmt == PT_H265)
    {
        ...
        AW_MPI_VENC_GetH265SpsPpsInfo(pVencPara->mVeChn, &H265SpsPpsInfo);
...
static void *saveFromEncThread(void *pThreadData)
{
    ...
    while (!pVencPara->mOverFlag)
    {
        ...
        ret = AW_MPI_VENC_GetStream(pVencPara->mVeChn, &vencFrame, 100);
        if (SUCCESS == ret)
        {
...
int main(int argc, char** argv)
{
    ...
    if (initVencPara() != SUCCESS)
    {
        aloge("something wrong in init");
        return -1;
    }
    ...
    ret = vencBufMgrCreate(IN_FRAME_BUF_NUM, pVencPara->mConfigPara.srcSize, &pVencPara->mInBuf_Q, isAwAfbc);
    if (ret != 0)
    {
