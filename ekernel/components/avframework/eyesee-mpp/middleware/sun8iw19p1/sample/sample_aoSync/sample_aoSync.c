...
static ERRORTYPE loadSampleAOSyncConfig(SampleAOSyncConfig *pConfig, const char *conf_path)
{
    ...
    alogd("param: pcmFilePath[%s], sampleRate[%d], channelCount[%d], bitWidth[%d], frameSize[%d], aoVolume[%d], TestDuration[%d], ParseWavHeaderEnable[%d]",
...
int main_sample_aoSync(int argc, char *argv[])
{
    int result = 0;
    ...
    int rett = pthread_create(&stContext.mSendThdId, NULL, sendAudioFrameThread, &stContext);
    if (rett || !stContext.mSendThdId)
    {
    ...
    pthread_join(stContext.mSendThdId, (void*) &rett);
...
FINSH_FUNCTION_EXPORT_ALIAS(main_sample_aoSync, __cmd_sample_aoSync, test mpp ao sync);
