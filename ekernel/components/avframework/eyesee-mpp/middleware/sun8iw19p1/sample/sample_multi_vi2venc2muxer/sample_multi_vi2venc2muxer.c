 ...
 ******************************************************************************
  ...
  Created       : 2020/5/15
  Last Modified : 2021-06-04 15:19:54
  Description   :
  Function List :
  History       :
******************************************************************************/
...
static void* SampleDetectInputThread(void* pThreadData)
{
    ...
    prctl(PR_SET_NAME, (unsigned long)"sample_multi_vi2venc2muxer_DetectInput", 0, 0, 0);
    while(1)
    {
...
static int loadSampleMultiVi2venc2muxerConfig(SampleMultiVi2venc2muxerContext *pThiz, const char *conf_path)
{
    ...
    //vipp configure
    pThiz->mConfigPara.mVipp0 = 0;
    pThiz->mConfigPara.mVipp0Format = MM_PIXEL_FORMAT_YUV_AW_LBC_2_5X;
    pThiz->mConfigPara.mVipp0CaptureWidth = 1280;
    pThiz->mConfigPara.mVipp0CaptureHeight = 720;
    pThiz->mConfigPara.mVipp0Framerate = 60;

    pThiz->mConfigPara.mVipp1 = 1;
    ...
    strcpy(pThiz->mConfigPara.mVideoAFile, "/mnt/E/720p.mp4");
    pThiz->mConfigPara.mVideoAFileCnt = 3;
    pThiz->mConfigPara.mVideoAFramerate = 30;
    pThiz->mConfigPara.mVideoABitrate = 2*1000*1000;
    pThiz->mConfigPara.mVideoAWidth = 1280;
    pThiz->mConfigPara.mVideoAHeight = 720;
    ...
    strcpy(pThiz->mConfigPara.mVideoBFile, "/mnt/E/640p.mp4");
    ...
    strcpy(pThiz->mConfigPara.mVideoCFile, "/mnt/E/timelapse_first.mp4");
    ...
    strcpy(pThiz->mConfigPara.mVideoDFile, "/mnt/E/timelapse_second.mp4");
    ...
    strcpy(pThiz->mConfigPara.mVideoEFileJpeg, "/mnt/E/720p.jpeg");
    pThiz->mConfigPara.mVideoEFileJpegCnt = 3;
    pThiz->mConfigPara.mVideoEWidth = 1280;
    pThiz->mConfigPara.mVideoEHeight = 720;
    pThiz->mConfigPara.mVideoEEncoderType = PT_JPEG;
    pThiz->mConfigPara.mVideoEPhotoInterval = 30;
    pThiz->mConfigPara.mTestDuration = 40;
    ...
    if(conf_path != NULL)
    {
        ...
        //vipp configure
        pThiz->mConfigPara.mVipp0 = GetConfParaInt(&stConf, SAMPLE_MULTI_VI2VENC2MUXER_VIPP0, 0);
        ...
        pThiz->mConfigPara.mVipp1 = GetConfParaInt(&stConf, SAMPLE_MULTI_VI2VENC2MUXER_VIPP1, 0);
        ...
        //params of every video encode line
        pThiz->mConfigPara.mVideoAVipp = GetConfParaInt(&stConf, SAMPLE_MULTI_VI2VENC2MUXER_VIDEOA_VIPP, 0);
        ...
        pThiz->mConfigPara.mVideoBVipp = GetConfParaInt(&stConf, SAMPLE_MULTI_VI2VENC2MUXER_VIDEOB_VIPP, 0);
        ...
        pThiz->mConfigPara.mVideoCVipp = GetConfParaInt(&stConf, SAMPLE_MULTI_VI2VENC2MUXER_VIDEOC_VIPP, 0);
        ...
        pThiz->mConfigPara.mVideoDVipp = GetConfParaInt(&stConf, SAMPLE_MULTI_VI2VENC2MUXER_VIDEOD_VIPP, 0);
        ...
        pThiz->mConfigPara.mVideoEVipp = GetConfParaInt(&stConf, SAMPLE_MULTI_VI2VENC2MUXER_VIDEOE_VIPP, 0);
...
SampleMultiVi2venc2muxerContext *constructSampleMultiVi2venc2muxerContext(void)
{
...
_err1:
    ret = AW_MPI_VI_DestoryVipp(pVipp->mViDev);
    if (ret != SUCCESS)
    {
...
static int configVencChnAttr(Vi2venc2muxerContext *pRecCtx)
{
    ...
    pRecCtx->mVencRcParam.product_mode = (int)VENC_PRODUCT_NORMAL_MODE;
    pRecCtx->mVencRcParam.sensor_type = VENC_ST_DIS_WDR;
        ...
        case PT_H264:
        {
            switch (pRecCtx->mVideoRcMode)
            {
                case 1:
                {
                    ...
                    pRecCtx->mVencRcParam.ParamH264Vbr.mQpInit = 30;
                    pRecCtx->mVencRcParam.ParamH264Vbr.mbEnMbQpLimit = 0;
                    pRecCtx->mVencRcParam.ParamH264Vbr.mMovingTh = 20;
                    pRecCtx->mVencRcParam.ParamH264Vbr.mQuality = 5;
                    pRecCtx->mVencRcParam.ParamH264Vbr.mIFrmBitsCoef = 15;
                    pRecCtx->mVencRcParam.ParamH264Vbr.mPFrmBitsCoef = 10;
                    break;
                }
                case 2:
                {
                ...
                default:
                {
                    ...
                    pRecCtx->mVencRcParam.ParamH264Cbr.mMinQp = 1;
                    pRecCtx->mVencRcParam.ParamH264Cbr.mMaxPqp = 50;
                    pRecCtx->mVencRcParam.ParamH264Cbr.mMinPqp = 10;
                    pRecCtx->mVencRcParam.ParamH264Cbr.mQpInit = 30;
                    pRecCtx->mVencRcParam.ParamH264Cbr.mbEnMbQpLimit = 0;
                    break;
                }
            }
            break;
        ...
        case PT_H265:
        {
            switch (pRecCtx->mVideoRcMode)
            {
                case 1:
                {
                    ...
                    pRecCtx->mVencRcParam.ParamH265Vbr.mQpInit = 30;
                    pRecCtx->mVencRcParam.ParamH265Vbr.mbEnMbQpLimit = 0;
                    pRecCtx->mVencRcParam.ParamH265Vbr.mMovingTh = 20;
                    pRecCtx->mVencRcParam.ParamH265Vbr.mQuality = 5;
                    pRecCtx->mVencRcParam.ParamH265Vbr.mIFrmBitsCoef = 15;
                    pRecCtx->mVencRcParam.ParamH265Vbr.mPFrmBitsCoef = 10;
                    break;
                }
                case 2:
                {
                ...
                default:
                {
                    ...
                    pRecCtx->mVencRcParam.ParamH265Cbr.mQpInit = 30;
                    pRecCtx->mVencRcParam.ParamH265Cbr.mbEnMbQpLimit = 0;
                    break;
                }
            }
            break;
...
static ERRORTYPE configMuxGrpAttr(Vi2venc2muxerContext *pRecCtx)
{
    ...
    pRecCtx->mMuxGrpAttr.mHeight = pRecCtx->mVideoHeight;
    pRecCtx->mMuxGrpAttr.mWidth = pRecCtx->mVideoWidth;
    pRecCtx->mMuxGrpAttr.mVideoFrmRate = pRecCtx->mVideoFramerate*1000;
    pRecCtx->mMuxGrpAttr.mVideoEncodeType = pRecCtx->mVideoEncoderType;
...
static int prepareRecord(Vi2venc2muxerContext *pRecCtx)
{
    ...
    if (pRecCtx->mVideoEncoderType == PT_H264)
    {
        ...
        AW_MPI_MUX_SetH264SpsPpsInfo(pRecCtx->mMuxGrp, &H264SpsPpsInfo);
    }
    else if(pRecCtx->mVideoEncoderType == PT_H265)
    {
        ...
        AW_MPI_MUX_SetH265SpsPpsInfo(pRecCtx->mMuxGrp, &H265SpsPpsInfo);
    }
    else
    {
...
int takePicture(TakeJpegContext *pJpegCtx)
{
    ...
    if(SUCCESS == AW_MPI_VENC_GetStream(pJpegCtx->mVeChn, &stVencStream, 5000))
    {
        ...
        char strNameBuf[MAX_FILE_PATH_SIZE+16] = {0};
...
int main_sample_multi_vi2venc2muxer(int argc, char** argv)
{
    ...
    {
        alogd("take picture threadId:0x%lx", pContext->mTakeJpegCtx.mTakePictureThreadId);
    }
    //wait exit.
    ...
    alogd("take picture pthread[0x%lx] join ret[%d], pValue[%p]", pContext->mTakeJpegCtx.mTakePictureThreadId, result, pValue);
    //stop vipp
    for(i=0; i<MAX_VIPP_DEV_NUM; i++)
    {
        if(pContext->mVippConfigs[i].mbValid)
    ...
    alogd("bye, sample_multi_vi2venc2muxer");
    //log_quit();
    return result;
...
FINSH_FUNCTION_EXPORT_ALIAS(main_sample_multi_vi2venc2muxer, __cmd_sample_multi_vi2venc2muxer, test multiple video encode);
