 ...
 ******************************************************************************
  ...
  Created       : 2017/1/5
  Last Modified : 2021-07-16 12:09:12
...
static ERRORTYPE loadSampleVirViConfig(SampleVirViConfig *pConfig, const char *conf_path)
{
    ...
    pConfig->mJpegWidth = pConfig->PicWidth;
    pConfig->mJpegHeight = pConfig->PicHeight;
    pConfig->mJpegStoreCount = 2;
    pConfig->mJpegStoreInterval = 60;
    strcpy(pConfig->mStoreDirectory, "/mnt/F");
    ...
    if(conf_path != NULL)
    {
        ...
        pConfig->mJpegWidth = GetConfParaInt(&stConfParser, SAMPLE_VirVi_JpegWidth, 0);
        pConfig->mJpegHeight = GetConfParaInt(&stConfParser, SAMPLE_VirVi_JpegHeight, 0);
...
int saveJpegPicture(VirVi_Cap_S *pCap, VIDEO_FRAME_INFO_S *pFrameInfo)
{
    ...
    unsigned int nVbvBufSize;

    if (0 == pContext->mConfigPara.mJpegWidth || 0 == pContext->mConfigPara.mJpegHeight)
    {
        pContext->mConfigPara.mJpegWidth = pFrameInfo->VFrame.mWidth;
        pContext->mConfigPara.mJpegHeight = pFrameInfo->VFrame.mHeight;
    }

    nVbvBufSize = ALIGN((pContext->mConfigPara.mJpegWidth*pContext->mConfigPara.mJpegHeight*3/2 /2) \
                    +(pContext->mConfigPara.mJpegWidth*pContext->mConfigPara.mJpegHeight/3), 1024);
    ...
    pCap->mChnAttr.VeAttr.AttrJpeg.PicWidth = pContext->mConfigPara.mJpegWidth;
    pCap->mChnAttr.VeAttr.AttrJpeg.PicHeight = pContext->mConfigPara.mJpegHeight;
...
static void *GetCSIFrameThread(void *pArg)
{
    ...
    while (pContext->mConfigPara.GetFrameCount != j)
    {
        ...
        if (i % 60 == 0)
        {
            ...
            /*
             *printf("%s line %d,times %d. y-addr 0x%lx, uv-addr 0x%lx.\n", \
             *        __func__, __LINE__, i, (unsigned long)pCap->pstFrameInfo.VFrame.mpVirAddr[0], (unsigned long)pCap->pstFrameInfo.VFrame.mpVirAddr[1]);
             */
#if 1
            FILE *fd = NULL;
            char filename[128];
            sprintf(filename, "/mnt/F/%dx%d_%d.yuv",
