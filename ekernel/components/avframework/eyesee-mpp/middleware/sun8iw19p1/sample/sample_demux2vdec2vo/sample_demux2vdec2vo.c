 ...
 ******************************************************************************
  ...
  Created       : 2016/11/4
 *  Last Modified:  2020-05-27 16:28:45
  Description   :
  Function List :
  History       :
******************************************************************************/
...
static SAMPLE_DEMUX2VDEC2VO_S *gpDemux2Vdec2VoData = NULL;

static void printMemoryContent(void *pMem, int nSize)
{
    int i = 0;
    int nIntSize = nSize/4;
    int *ptr = (int*)pMem;
    for(i=0; i<nIntSize; i++)
    {
        if(i%16 == 0)
        {
            printf("\n");
...
static SAMPLE_DEMUX2VDEC2VO_S* InitDemux2Vdec2VoData(void)
{
    SAMPLE_DEMUX2VDEC2VO_S *pDemux2Vdec2VoData = (SAMPLE_DEMUX2VDEC2VO_S* )malloc(sizeof(SAMPLE_DEMUX2VDEC2VO_S));
    ...
    pDemux2Vdec2VoData->mClockChn = 0xAA;
...
static ERRORTYPE loadConfigPara(SAMPLE_DEMUX2VDEC2VO_S *pDemux2Vdec2VoData, const char *conf_path)
{
    ...
    strcpy(pDemux2Vdec2VoData->mConfigPara.srcFile, "/mnt/E/test.mp4");
    ...
    pDemux2Vdec2VoData->mConfigPara.mDisplayX = 60;
    pDemux2Vdec2VoData->mConfigPara.mDisplayY = 0;
    pDemux2Vdec2VoData->mConfigPara.mDisplayWidth = 360;
...
//        {
//            int i = 0;
//            int nIntSize = sizeof(SAMPLE_DEMUX2VDEC2VO_S)/4;
//            int *ptr = (int*)pDemux2Vdec2VoData;
//            for(i=0; i<nIntSize; i++)
//            {
//                if(i%16 == 0)
//                {
...
static ERRORTYPE prepare(SAMPLE_DEMUX2VDEC2VO_S *pDemux2Vdec2VoData)
{
    ...
    if (DemuxMediaInfo.mVideoNum >0)
    {
        ...
        if (ret == SUCCESS)
        {
            alogd("bind clock[%d] & demux", pDemux2Vdec2VoData->mClockChn);
...
static void handle_exit(int signo)
{
    ...
    if(NULL != gpDemux2Vdec2VoData)
    {
        cdx_sem_up(&gpDemux2Vdec2VoData->mSemExit);
    }
}
...
int main_sample_demux2vdec2vo(int argc, char** argv)
{
    int result = -1;
    int ret = 0;
    SAMPLE_DEMUX2VDEC2VO_S *pContext = InitDemux2Vdec2VoData();
    if (NULL == pContext)
    {
        return -1;
    }
    gpDemux2Vdec2VoData = pContext;
    ...
    if(argc > 1)
    {
        ...
        pConfFilePath = pContext->mCmdLinePara.mConfigFilePath;
    }
    else
    {
        pConfFilePath = NULL;
    ...
    if (loadConfigPara(pContext, pConfFilePath) != SUCCESS)
    {
    ...
    pContext->srcFd = open(pContext->mConfigPara.srcFile, O_RDONLY);
    if (pContext->srcFd < 0)
    {
        aloge("ERROR: cannot open video src file %s.", pContext->mConfigPara.srcFile);
        goto err_out_0;
    }
    ...
    if (pContext->mConfigPara.seekTime > 0)
    {
        seekto(pContext);
    }
    if (start(pContext) != SUCCESS)
    {
...
err_out_2:
    ...
    if (pContext->mVoLayer >= 0)
    {
        if (pContext->mVoChn >= 0)
        {
        ...
        usleep(50*1000);
        pContext->mVoLayer = MM_INVALID_CHN;
    ...
    {
        int ret = AW_MPI_VDEC_DestroyChn(pContext->mVdecChn);
        alogd("ret = %d", ret);
    }
    if (pContext->mDmxChn >= 0)
    {
        AW_MPI_DEMUX_DestroyChn(pContext->mDmxChn);
    }
    if (pContext->mClockChn >= 0)
    {
        AW_MPI_CLOCK_DestroyChn(pContext->mClockChn);
    }
err_out_1:
    //exit mpp system
    AW_MPI_SYS_Exit();

    close(pContext->srcFd);
err_out_0:
...
FINSH_FUNCTION_EXPORT_ALIAS(main_sample_demux2vdec2vo, __cmd_sample_demux2vdec2vo, test mpp demux2vdec2vo);
