 ...
 ******************************************************************************
  ...
  Created       : 2016/11/4
 *  Last Modified:  2021-05-12 11:06:19
  Description   :
  Function List :
  History       :
******************************************************************************/
...
{    
    ...
    if ((ret = AW_MPI_VI_GetFrame(VIDEV, inCh, &pstFrameInfo, 5000)) != 0)
    {
        ...
        while(mChn < VENC_MAX_CHN_NUM){
            ...
            {
                bSuccessFlag = true;
                __log("create venc channel[%d] success!", mChn);
                break;
            }
        ...
        {
//#if 0
            printf("%s line %d.addr = 0x%lx.\n", __func__, __LINE__, (unsigned long)mJpegThumbBuf.ThumbAddrVir);
