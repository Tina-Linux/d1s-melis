...
int AW_MPI_ISP_GetTemperature(ISP_DEV IspDev)
{
    ...
    aloge("fatal error! not implement.");
    return -1;
}

AW_U32 AW_MPI_ISP_GetAwbGainSum(ISP_DEV IspDev, AW_S32 *awb_rgain_sum, AW_S32 *awb_bgain_sum)
{
	if (awb_rgain_sum == NULL || awb_bgain_sum == NULL) {
		aloge("Invalid ptr for awb_gain_sum\n");
		return -1;
	}

	isp_get_awb_gain_sum(IspDev, awb_rgain_sum, awb_bgain_sum);
