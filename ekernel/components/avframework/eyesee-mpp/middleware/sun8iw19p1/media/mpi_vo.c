...
ERRORTYPE AW_MPI_VO_Enable(VO_DEV VoDev)
{
    ...
    int disp_type;
    int tv_mode;
    if(0 == hwd_get_disp_type(&disp_type, &tv_mode))
    {
        pVODevInfo->mVoPubAttr.enIntfType = map_disp_output_type_to_VO_INTF_TYPE_E((disp_output_type)disp_type);
        pVODevInfo->mVoPubAttr.enIntfSync = map_disp_tv_mode_to_VO_INTF_SYNC_E((disp_tv_mode)tv_mode);
    }
    else
    {
