...
#define SOUND_MIXER_AUDIOCODEC   "audiocodec"
#define SOUND_MIXER_SNDDAUDIO0   "snddaudio0"

#define SOUND_CARD_SNDDAUDIo0   "snddaudio0"
...
static ERRORTYPE audioHw_AI_AnsProcess(void *pThreadData, AUDIO_FRAME_S *pFrm)
{
...
#if (MPPCFG_ANS_LIB == OPTION_ANS_LIBRARY_WEBRTC)
        WebRtcSpl_AnalysisQMF(tmp_near_buffer,tmp_ans_shInL,tmp_ans_shInH,(int32_t*)pCap->filter_state1,(int32_t*)pCap->filter_state12);
        if (0 == WebRtcNs_Process(pCap->ans_int ,tmp_ans_shInL,tmp_ans_shInH ,tmp_ans_shOutL , tmp_ans_shOutH))
        {
            WebRtcSpl_SynthesisQMF(tmp_ans_shOutL,tmp_ans_shOutH,processed_frm_ptr,(int32_t*)pCap->Synthesis_state1,(int32_t*)pCap->Synthesis_state12);
        }
#elif (MPPCFG_ANS_LIB == OPTION_ANS_LIBRARY_LSTM)
    ...
    { 
        ...
        {
            aloge("ans_fatal_err_buff_left:%d-%d",near_buff_offset,pCap->in_buff_data_remain_len_ans);
        }
    ...
    if(pCap->out_buff_data_remain_len_ans >= pCap->mCfg.chunkSize*sizeof(short))
    {
        ...
        {
            aloge("ans_fatal_error:%d-%d",(int)pCap->mCfg.chunkSize*sizeof(short),pFrm->mLen);
        }
        ...
        if(pCap->out_buff_data_remain_len_ans > pCap->mCfg.chunkSize*sizeof(short))
        {
            aloge("ans_fatal_err_out_buff_data_mov:%d-%d",(int)pCap->mCfg.chunkSize*sizeof(short),pCap->out_buff_data_remain_len_ans);
        }
        else
        {
...
//        aloge("zjx_tbin:%d-%d",pCap->tmp_pcm_in_size,pCap->tmp_pcm_ref_size); 
    ...
    if(pCap->out_buff_data_remain_len >= pCap->mCfg.chunkSize*sizeof(short))
    {
        ...
        if(pFrm->mLen != pCap->mCfg.chunkSize*sizeof(short))
        {
            aloge("aec_fatal_error:%ld-%d",pCap->mCfg.chunkSize*sizeof(short),pFrm->mLen);
        }
...
//        aloge("zjx_tbo:%d-%d-%d",pCap->tmp_pcm_out_size,pFrm->mLen,pCap->mCfg.chunkSize*sizeof(short));
        ...
        if(pCap->out_buff_data_remain_len > pCap->mCfg.chunkSize*sizeof(short))
        {
            aloge("aec_fatal_err_out_buff_data_mov:%ld-%d",pCap->mCfg.chunkSize*sizeof(short),pCap->out_buff_data_remain_len);
        }
        else
        {
...
static void *audioHw_AI_CapThread(void *pThreadData)
{
    ...
    { 
        ...
        if(pCap->mAttr.ai_aec_en)
        {
            ...
            {
                alogd("aec_to_start_second_cap:%lld",CDX_GetTimeUs());
                pthread_attr_t attr; 
		...
		if(pCap->mAttr.ai_agc_en)
		{
			...
			agc_m_process(&pCap->mAttr.ai_agc_cfg, pCap->pCapBuf,pCap->ai_agc_tmp_buff,pCap->mCfg.chunkBytes/2);
