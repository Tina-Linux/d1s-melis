...
static VENC_CHN_MAP_S* VENC_CHN_MAP_S_Construct(void)
{
...
ERRORTYPE AW_MPI_VENC_EnableBinImage(VENC_CHN VeChn, VencBinImageParam *pParam)
{
    if(!(VeChn >= 0 && VeChn < VENC_MAX_CHN_NUM))
    {
        aloge("fatal error! invalid VeChn[%d]!", VeChn);
    ...
    ERRORTYPE ret;
    ret = COMP_SetConfig(pChn->mEncComp, COMP_IndexVendorVencEnableBinImage, (void*)pParam);
    return ret;
}

ERRORTYPE AW_MPI_VENC_GetBinImageData(VENC_CHN VeChn, VENC_BinImageBuf *pImageBuf)
{
    if(!(VeChn >= 0 && VeChn < VENC_MAX_CHN_NUM))
    {
        aloge("fatal error! invalid VeChn[%d]!", VeChn);
    ...
    ERRORTYPE ret;
    ret = COMP_GetConfig(pChn->mEncComp, COMP_IndexVendorVencBinImageData, (void*)pImageBuf);
    return ret;
}

ERRORTYPE AW_MPI_VENC_EnableMvInfo(VENC_CHN VeChn, BOOL bEnable)
{
    if(!(VeChn >= 0 && VeChn < VENC_MAX_CHN_NUM))
    {
        aloge("fatal error! invalid VeChn[%d]!", VeChn);
    ...
    ERRORTYPE ret;
    ret = COMP_SetConfig(pChn->mEncComp, COMP_IndexVendorVencEnableMvInfo, (void*)&bEnable);
    return ret;
}

ERRORTYPE AW_MPI_VENC_GetMvInfoData(VENC_CHN VeChn, VENC_MvInfoBuf *pMvInfoBuf)
{
    if(!(VeChn >= 0 && VeChn < VENC_MAX_CHN_NUM))
    {
        aloge("fatal error! invalid VeChn[%d]!", VeChn);
    ...
    ERRORTYPE ret;
    ret = COMP_GetConfig(pChn->mEncComp, COMP_IndexVendorVencMvInfoData, (void*)pMvInfoBuf);
    return ret;
}
