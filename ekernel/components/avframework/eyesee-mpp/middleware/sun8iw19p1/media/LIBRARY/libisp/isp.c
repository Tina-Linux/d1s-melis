...
//	isp_sensor_otp_init(isp);
	...
	// Load data to isp before open it.
	isp_reg_save_init(&isp_ctx[dev_id]);

	// Set exp to sensor before run isp.
	ae_result = &(isp_ctx[dev_id].ae_entity_ctx.ae_result);
	exp_gain.exp_val = ae_result->sensor_set.ev_set_curr.ev_sensor_exp_line;
	exp_gain.gain_val = ae_result->sensor_set.ev_set_curr.ev_analog_gain >> 4;
	isp_sensor_set_pre_exp_gain(isp, &exp_gain);
...
int isp_exit(int dev_id)
{
	...
	isp_reg_save_exit(&isp_ctx[dev_id]);
	isp_tuning_exit(isp);
...
int isp_run(int dev_id)
{
	...
	ret = pthread_create(&media_params.isp_tid[dev_id], &attr, __isp_thread, isp);
	if(ret != 0)
...
HW_S32 isp_set_saved_ctx(int dev_id)
{
    ...
    //isp_log_save_exit(&isp_ctx[dev_id]);
    //isp_stat_save_exit(&isp_ctx[dev_id]);
    ret = isp_ctx_save_exit(&isp_ctx[dev_id]);
    if(ret != 0)
    {
        ISP_ERR("isp%d ctx save fail[%d]", dev_id, ret);
    }
    ret = isp_reg_save_exit(&isp_ctx[dev_id]);
    if(ret != 0)
    {
        ISP_ERR("isp%d reg save fail[%d]", dev_id, ret);
    }
    //isp_tuning_exit(isp);
    //isp_ctx_algo_exit(&isp_ctx[dev_id]);
    //isp_dev_close(&media_params, dev_id);
...
#if 0
HW_S32 vin_isp_set_ae_lv(int dev_id, unsigned int ev_lv)
{
	struct hw_isp_device *isp;
	struct isp_lib_context *ctx;
	...
	if (ev_lv > 325 || ev_lv < 0) {
		printf("ev_lv error \n");
		return -1;
	}
	printf("set ev_lv --> %d\n");
	ctx->ae_entity_ctx.ae_result.sensor_set.ev_set_curr.ev_lv = ev_lv;
