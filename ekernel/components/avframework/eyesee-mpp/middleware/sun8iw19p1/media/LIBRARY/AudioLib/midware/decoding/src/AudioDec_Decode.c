...
static int ShowAbsBits(void *buf, int Len, void *mem)
{
    Ac320FileRead *FileInfo = (Ac320FileRead *)mem;
    AudioDecoderContextLib *pAudioDecodeLib  = NULL;
    int     Retlen = 0;
...
#else
    alib_loge("fatal error! why use dynamic lib[%s] in static_lib_mode?", libname);
    return NULL;
#endif
}
...
static int InitCodec(AudioDecoderContextLib* pAudioDecodeLib, InitCodecInfo* initcodecinfo)
{
    ...
    {
        DECLARE_ADEC_INIT_FUNC(ADEC_G711A_INIT_FUNC);
        DECLARE_ADEC_EXIT_FUNC(ADEC_G711A_EXIT_FUNC);
        pAudioDecodeLib->pCedarCodec->init = &ADEC_G711A_INIT_FUNC;
        pAudioDecodeLib->pCedarCodec->exit = &ADEC_G711A_EXIT_FUNC;
    }
    else if(0 == strcmp(initcodecinfo->init, STRING(ADEC_G711U_INIT_FUNC)))
    {
        DECLARE_ADEC_INIT_FUNC(ADEC_G711U_INIT_FUNC);
        DECLARE_ADEC_EXIT_FUNC(ADEC_G711U_EXIT_FUNC);
        pAudioDecodeLib->pCedarCodec->init = &ADEC_G711U_INIT_FUNC;
        pAudioDecodeLib->pCedarCodec->exit = &ADEC_G711U_EXIT_FUNC;
    }
    else if(0 == strcmp(initcodecinfo->init, STRING(ADEC_G726_INIT_FUNC)))
    {
        DECLARE_ADEC_INIT_FUNC(ADEC_G726_INIT_FUNC);
        DECLARE_ADEC_EXIT_FUNC(ADEC_G726_EXIT_FUNC);
        //pAudioDecodeLib->pCedarCodec->init = &ADEC_G726_INIT_FUNC;
        //pAudioDecodeLib->pCedarCodec->exit = &ADEC_G726_EXIT_FUNC;
    }
    else
    {
...
*/
        ...
        */
        else if((pAudioDecodeLib->pCedarCodec->flag & AD_CEDAR_CTRL_FLAG_BUILD_AACHDR) && !(pAudioDecodeLib->nAudioInfoFlags & ADEC_DISABLE_AAC_PACKING))
        {
            if(pExtraData)
            {
    ...
    if(pAudioStreamInfo->eCodecFormat == AUDIO_CODEC_FORMAT_G726)
    {
        pBsInFor->g726_enc_law = pAudioStreamInfo->g726_enc_law;
        pBsInFor->bitrate = pAudioStreamInfo->g726_src_bit_rate;
    }
    pBsInFor->firstflag = 0;
    pAudioDecodeLib->pCedarAudioDec->FileReadInfo = &pAudioDecodeLib->DecFileInfo;
    pAudioDecodeLib->pCedarAudioDec->DecoderCommand = &pAudioDecodeLib->pInternal;
               ...
               int*          nOffset)
{
    ...
    *nOffset = pAudioDecodeLib->DecFileInfo.FileWritingOpsition;
    ...
    if((pAudioDecodeLib->pCedarCodec->flag & AD_CEDAR_CTRL_FLAG_BUILD_AACHDR) && !(pAudioDecodeLib->nAudioInfoFlags & ADEC_DISABLE_AAC_PACKING))
    {
        ...
        else
        {
            ...
            *ppBuf = pAudioDecodeLib->DecFileInfo.bufWritingPtr + pAudioDecodeLib->Streambuffer.CedarAbsPackHdrLen; //holo for update fill
            nLengthToEnd2 = pAudioDecodeLib->DecFileInfo.bufStart + pAudioDecodeLib->DecFileInfo.BufToTalLen - (unsigned char*)(*ppBuf);
                                          ...
                                          int nOffset)
{
    ...
    if(pAudioDecodeLib->DecFileInfo.bufWritingPtr + nWriteLenth >= pAudioDecodeLib->DecFileInfo.bufStart + pAudioDecodeLib->DecFileInfo.BufToTalLen)
    {
        int nBufferLen = pAudioDecodeLib->DecFileInfo.bufStart + pAudioDecodeLib->DecFileInfo.BufToTalLen - pAudioDecodeLib->DecFileInfo.bufWritingPtr;
...
#endif
        pAudioDecodeLib->DecFileInfo.bufWritingPtr = pAudioDecodeLib->DecFileInfo.bufWritingPtr + nWriteLenth - pAudioDecodeLib->DecFileInfo.BufToTalLen;
    }
    else
    {
#ifdef __AD_CEDAR_DBG_WRITEOUT_BITSTREAM
