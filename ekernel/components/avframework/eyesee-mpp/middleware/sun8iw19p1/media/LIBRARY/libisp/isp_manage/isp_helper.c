...
int isp_reg_save_init(struct isp_lib_context *ctx)
{
	FILE *file_fd = NULL;
	char fdstr[50] = {0};
	...
	sprintf(fdstr, "%s/isp%d_%d_%d_%d_reg_saved.bin", ISP_PATH, ctx->isp_id, ctx->sensor_info.sensor_width,
		ctx->sensor_info.sensor_height, (int)ctx->sensor_info.fps_fixed);
	file_fd = fopen(fdstr, "rb");
	...
	if (file_fd == NULL) {
		...
		fread(&read_len, sizeof(int), 1, file_fd);
		if (data_len != read_len) {
			ISP_WARN("%s size %d != isp_tbl size %d!\n", fdstr, read_len, data_len);
		} else {
			fread(ctx->load_reg_base, ISP_LOAD_DRAM_SIZE, 1, file_fd);
			ISP_PRINT("get isp_tbl from %s success!!!\n", fdstr);
		}
	}
	fclose(file_fd);
...
int isp_reg_save_exit(struct isp_lib_context *ctx)
{
	FILE *file_fd = NULL;
	char fdstr[50] = {0};
	int data_len = ISP_LOAD_DRAM_SIZE;
	...
	if (file_fd == NULL) {
		...
		fwrite(&data_len, sizeof(int), 1, file_fd);
		fwrite(ctx->load_reg_base, ISP_LOAD_DRAM_SIZE, 1, file_fd);
		ISP_LIB_LOG(ISP_LOG_ISP, "save isp_tbl to %s success!!!\n", fdstr);
	}
	fclose(file_fd);
	return 0;
open_fail:
