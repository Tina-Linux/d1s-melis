...
void __isp_awb_run(struct isp_lib_context *isp_gen)
{
	...
	struct isp_dg_gain *dg = &isp_gen->module_cfg.gain_offset_cfg.gain;
	...
	if (!isp_gen->isp_ini_cfg.isp_test_settings.wdr_en || isp_gen->ae_entity_ctx.ae_param->nor_cmd_mode) {
		if (isp_gen->otp_enable == 1) {
			isp_gen->module_cfg.wb_gain_cfg.wb_gain.r_gain = isp_awb_cxt->awb_result.wb_gain_output.r_gain*isp_gen->wb_golden_ratio[0];
			isp_gen->module_cfg.wb_gain_cfg.wb_gain.gr_gain = isp_awb_cxt->awb_result.wb_gain_output.gr_gain*isp_gen->wb_golden_ratio[1];
			isp_gen->module_cfg.wb_gain_cfg.wb_gain.gb_gain = isp_awb_cxt->awb_result.wb_gain_output.gb_gain*isp_gen->wb_golden_ratio[1];
			isp_gen->module_cfg.wb_gain_cfg.wb_gain.b_gain = isp_awb_cxt->awb_result.wb_gain_output.b_gain*isp_gen->wb_golden_ratio[2];
		} else {
	...
	if (isp_ini_cfg->isp_test_settings.linear_en &&
		...
		dg->r_gain = ((HW_U32)dg->r_gain * (HW_U32)isp_awb_cxt->awb_result.wb_gain_output.r_gain + 128) >> 8;
		dg->b_gain = ((HW_U32)dg->b_gain * (HW_U32)isp_awb_cxt->awb_result.wb_gain_output.b_gain + 128) >> 8;
	}
#endif
...
#endif
#if (ISP_VERSION == 520 || ISP_VERSION == 521)
	if (isp_gen->pltm_entity_ctx.pltm_result.pltm_algo_sel == PLTM_ALGO_1) {
		isp_gen->module_cfg.mode_cfg.hist_sel = 0;//AFTER_WDR;
		isp_gen->stats_ctx.hist.last_cnr_cnt = 0;
		isp_gen->stats_ctx.hist.last_wdr_cnt = 0;
		isp_gen->stats_ctx.hist.last_cnr_lum = 0;
		isp_gen->stats_ctx.hist.last_wdr_lum = 0;
	}
#endif
...
#endif
	modify the manual to control new_dnr_update
*/
#if (ISP_VERSION == 520 || ISP_VERSION == 521)
	  if (param->isp_test_settings.wdr_en && isp_gen->sensor_info.wdr_mode == 2) {/* CMD_WDR */
	    isp_gen->ae_entity_ctx.ae_param->use_dnr_update = !param->isp_test_settings.manual_en;
	    if (isp_gen->ae_entity_ctx.ae_param->use_dnr_update)
	      ISP_PRINT("use new dnr optimization\n");
	  } else {
	    isp_gen->ae_entity_ctx.ae_param->use_dnr_update = 0;
	  }
#endif
