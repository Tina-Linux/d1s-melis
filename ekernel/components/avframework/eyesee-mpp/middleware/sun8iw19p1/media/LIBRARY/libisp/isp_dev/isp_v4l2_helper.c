...
int v4l2_subdev_open(struct media_entity_param *entity)
{
	if (entity->fd != -1)
		return 0;

	ISP_PRINT("[DBG]: %s line %d, devname %s, entity %s.\n",\
		__func__, __LINE__, entity->devname, entity->info.name);
...
void v4l2_subdev_close(struct media_entity_param *entity)
{
	if (entity->fd == -1)
		return;
...
int v4l2_get_control(struct media_entity_param *entity, unsigned int id,
			    int *value)
{
...
int v4l2_set_control(struct media_entity_param *entity, unsigned int id,
			    int *value)
{
	struct v4l2_control ctrl;
	int ret;
...
int v4l2_get_controls(struct media_entity_param *entity, unsigned int count,
...
int v4l2_set_controls(struct media_entity_param *entity, unsigned int count,
			     struct v4l2_ext_control *ctrls)
{
	struct v4l2_ext_controls controls;
	int ret;
