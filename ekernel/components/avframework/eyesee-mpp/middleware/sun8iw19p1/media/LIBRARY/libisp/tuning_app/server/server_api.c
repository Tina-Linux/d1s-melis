...
static int client_handle(int sock_fd)
{
	...
	case SOCK_TYPE_REGOPT:
		...
		snprintf(name,sizeof(name),"register_thread");
		break;
	case SOCK_TYPE_AELV:
		thread_func_ptr = &sock_handle_aelv_thread;
		snprintf(name,sizeof(name),"aelv_thread");
		break;
	case SOCK_TYPE_SET_INPUT:
		thread_func_ptr = &sock_handle_set_input_thread;
		snprintf(name,sizeof(name),"set_input_thread");
		break;
	case SOCK_TYPE_RAW_FLOW:
		thread_func_ptr = &sock_handle_raw_flow_thread;
		snprintf(name,sizeof(name),"raw_flow_thread");
		break;
	case SOCK_TYPE_ISP_VERSION:
		thread_func_ptr = &sock_handle_isp_version_thread;
	    snprintf(name,sizeof(name),"isp_version_thread");
		break;
	case SOCK_TYPE_PREVIEW_VENCODE:
#ifdef ISPTOOL_VENCODE
		thread_func_ptr = &sock_handle_preview_vencode_thread;
		break;
#else
		LOG("App in board does't have preview vencode feature, please rebuild App with ISPTOOL_VENCODE macro define\n");
		break;
#endif
#if 1
	case SOCK_CMD_EXIT:
	...
	// 3. add work
	ret = add_work(thread_func_ptr, (void *)sock_fd,name);
	...
	if (SERVER_FLAG_INIT_YES == g_server_init_flag) {
		while (1) {
			...
			if (client_socket >= 0) {
			    ...
			    printf("client_socket %d %d %d %d %d\n",client_socket,errno,ETIMEDOUT,EAGAIN,EWOULDBLOCK,EINTR);
				if (!SHOULD_TRY_AGAIN()) {
