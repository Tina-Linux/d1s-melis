	...
	case V4L2_EVENT_CTRL:
		for(i = 0; i < array_size(isp_cid_array); i++) {
			if (isp_cid_array[i].id == event.id)
				ISP_DEV_LOG(ISP_LOG_ISP, "name is %s, value is 0x%lx\n", isp_cid_array[i].name, event.u.ctrl.value);
		}
...
{
	struct media_entity_param *entity = NULL;
...
static int isp_stat_req_buffers(struct hw_isp_device *isp)
{
	...
	if (isp->buffer == NULL)
		return -ENOMEM;
	memset(isp->buffer, 0x00, config.buf_size);
	return 0;
}
...
{
	struct media_entity_param *entity = NULL, *head = NULL;
	struct media_device *mdev = NULL;
	char name[32];
	//int isp_id = 0;
...
int isp_dev_start(struct hw_isp_device *isp)
{
	...
	isp->ops->monitor_fd(isp->id, isp->subdev.fd, HW_ISP_EVENT_READ, isp_subdev_handle_event, isp);
	...
	isp->ops->monitor_fd(isp->id, isp->stat.fd, HW_ISP_EVENT_READ, isp_stat_process_buffer, isp);
...
int isp_sensor_set_pre_exp_gain(struct hw_isp_device *isp, struct sensor_exp_gain *exp_gain)
{
	int ret;
	static struct sensor_exp_gain exp_gain_save[HW_ISP_DEVICE_NUM];
	...
	ret = ioctl(isp->sensor.fd, VIDIOC_VIN_SENSOR_P_EXP_GAIN, exp_gain);
	if (ret < 0)
		ISP_ERR("%s set exp and gain failed: %s (%d).\n",
			isp->sensor.info.name, strerror(errno), errno);

	ISP_DEV_LOG(ISP_LOG_SUBDEV, "set pre sensor exp gain, exp_val: %d, gain_val: %d, r_gain: %d, b_gain: %d.\n",
		exp_gain->exp_val, exp_gain->gain_val, exp_gain->r_gain, exp_gain->b_gain);
	return ret;
}
