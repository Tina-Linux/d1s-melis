 ...
 *        {
 *                ac_enc[i] = g711a_data[i];
 *        }
 *        alaw_expand(g711a_bytes, ac_enc, amp);
 *        return g711a_bytes * 2;
 *}
 */

//int g711u_dec(unsigned char * g711u_enc, short * as_dec_out, int N_sample)
/*
 ...
 *        {
 *                ac_enc[i] = g711u_data[i];
 *        }
 *        ulaw_expand(g711u_bytes, ac_enc, amp);
 *        return g711u_bytes * 2;
 *}
 */
...
{
	short ac_enc[2048] = { 0 };
	alaw_compress(len, amp, ac_enc);

	for (int i = 0; i < len; i++)
	{
		g711_data[i] = ac_enc[i];
	}
	return len;
}
