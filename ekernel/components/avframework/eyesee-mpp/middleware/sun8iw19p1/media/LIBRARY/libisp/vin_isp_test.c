...
rt_device_t dispfh;
unsigned long arg[4];
static struct disp_layer_config config;
...
static void disp_exit(void)
{
	...
	arg[2] = 1;
	arg[3] = 0;
	dispfh->control(dispfh, DISP_LAYER_SET_CONFIG, (void *)arg);
...
static int disp_init(int width, int height)
{
	...
	arg[3] = 0;
	dispfh->control(dispfh, DISP_LAYER_GET_CONFIG, (void *)arg);
	config.info.fb.format = DISP_FORMAT_YUV420_P;
	config.info.mode  = LAYER_MODE_BUFFER;
	config.enable = 1;

	screen_wdith = dispfh->control(dispfh, DISP_GET_SCN_WIDTH, (void *)arg);
	screen_height = dispfh->control(dispfh, DISP_GET_SCN_HEIGHT, (void *)arg);

	if (screen_wdith < width || screen_height < height) {
		printf("warn:input size is larger than screen size\n");
	}
...
static int disp_set_addr(u32 width, u32 height, u32 addr0, u32 addr1, u32 addr2)
{
	...
	arg[2] = 1;
	arg[3] = 0;
	return dispfh->control(dispfh, DISP_LAYER_SET_CONFIG, (void *)arg);
}
...
int cmd_vin_isp_test(int argc __attribute__((__unused__)), char *argv[] __attribute__((__unused__)))
{
	int ret, n = 0;
	int ch_num = 0;
	int width = 640;
...
int cmd_vin_preview(int argc, char *argv[])
{
    char *argvv[10] =
    {
	"vin_preview",
    ...
    cmd_vin_isp_test(9, argvv);

    return 0;
}

FINSH_FUNCTION_EXPORT_ALIAS(cmd_vin_isp_test, __cmd_vin_isp_test, vin isp test);
FINSH_FUNCTION_EXPORT_ALIAS(cmd_vin_preview, __cmd_vin_preview, vin_preview);
