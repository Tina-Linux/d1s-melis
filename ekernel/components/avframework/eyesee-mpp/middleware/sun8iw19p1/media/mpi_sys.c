 ...
 ******************************************************************************
  ...
  Created       : 2016/05/23
  Last Modified : 2021-06-24 14:26:48
...
static void SYS_DecideBindPortIndex(MPP_CHN_S *pSrcChn, MPP_CHN_S *pDestChn, unsigned int *pSrcPortIdx, unsigned int *pDstPortIdx, MppBindControl* pBindControl)
{
    ...
    switch (pDestChn->mModId)
    {
        ...
        case MOD_ID_MUX:
        {
            if(MOD_ID_VENC == pSrcChn->mModId || MOD_ID_UVC == pSrcChn->mModId)
            {
                *pDstPortIdx = 0;
            }
            ...
            else if(MOD_ID_TENC == pSrcChn->mModId)
            {
                *pDstPortIdx = 2;
            }
            else if(MOD_ID_DEMUX == pSrcChn->mModId)
            {
                if (pBindControl == NULL || pBindControl->eDomain == COMP_PortDomainAudio)
                {
                    *pDstPortIdx = 1;
...
static ERRORTYPE ion_iommu_prepare(void)
{
...
static ERRORTYPE ion_iommu_over(void)
{
    if(gSysManager.mbVeInitFlag)
    {
...
ERRORTYPE AW_MPI_SYS_Init(void)
{
    ...
    if (0 != ret) 
    {
        aloge("AW_MPI_VI_Init failed, call vi_exit.");
        AW_MPI_VI_Exit();
        return FAILURE;
    }
    alogd("ISP init");
