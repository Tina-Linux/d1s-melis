        ...
        PARAM_IN void* pComponentConfigStructure)
{
    ...
    switch (nIndex)
    {
        ...
        {
            AUDIO_SAMPLE_RATE_E *rate = (AUDIO_SAMPLE_RATE_E*)pComponentConfigStructure;
            if(*rate != pChnData->mVqeCfg.s32WorkSampleRate)
            {
                if((*rate <= 48000) && (*rate >= 8000))
                {
                    pChnData->mVqeCfg.s32WorkSampleRate = *rate;
                }
            }
            break;
        }
        ...
        {
            pChnData->mVqeCfg.s32WorkSampleRate = AUDIO_SAMPLE_RATE_BUTT;
            break;
        }
...
static ERRORTYPE AOChannel_ComponentDeInit(PARAM_IN COMP_HANDLETYPE hComponent)
{
    ...
    {
        free(pChnData->tmpHandleBuf);
        pChnData->tmpHandleBuf = NULL;
    }
...
//    }
    ...
    pChnData->tmpBufSize = 64/*128*/*1024;
...
//    pthread_attr_setschedparam(&attr, &param);
    ...
    err = pthread_create(&pChnData->mThreadId, &attr, AOChannel_ComponentThread, pChnData);
    if (err) {
        eError = FAILURE;
        goto ERR_EXIT6;
    }
...
PROCESS_MESSAGE:
        ...
        ret = audioHw_AO_FillPcmRingBuf(pChnData->mMppChnInfo.mDevId, tmpPcmFrame.mpAddr, tmpPcmFrame.mLen);
        if (SUCCESS == ret)
        {
