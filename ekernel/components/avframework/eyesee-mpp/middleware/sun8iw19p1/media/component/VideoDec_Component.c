...
static ERRORTYPE CompFrameInit(VDecCompOutputFrame *pCompFrame, VideoPicture *pPicture, int nRotation)
{
    ...
    *pCompFrame->mpPicture = *pPicture;
    ...
    int nUSize = 0;
    int nVSize = 0;
    aloge("fatal error! not support ion alloc now.");
    #if 0
...
{
    aloge("fatal error! not support ion free.");
    if(pCompFrame->mpPicture)
    {
...
{
#if (AWCHIP == AW_V5 || AWCHIP == AW_V316 || AWCHIP == AW_V459)
    ...
    if(pVideoDecData->mbUseCompFrame)
    {
        ...
        if(pVideoDecData->mG2DHandle >= 0)
        {
            if(0 == pVideoDecData->mCompFrameBufferThreadId)
            {
                ...
                alogd("create VideoDec rotate frame buffer threadId[0x%lx]", pVideoDecData->mCompFrameBufferThreadId);
                message_t msg;
...
ERRORTYPE VideoDecSetIonFd(PARAM_IN COMP_HANDLETYPE hComponent, PARAM_IN int nIonFd)
{
    ...
    aloge("fatal error! dup is not implement.");
...
_TryToRequestVbs:
    ...
    {// request buffer success
        ...
        list_move_tail(&pEntry->mList, &pInputData->mUsedVbsList);
        //notify VideoDec_InputThread 
        if (pInputData->mbWaitUsedVbsFlag)
        {
            pInputData->mbWaitUsedVbsFlag = FALSE;
            message_t msg;
            msg.command = VDecComp_InputData_UsedVbsAvailable;
            put_message(&pInputData->cmd_queue, &msg);
        }
        eError = SUCCESS;
    }
    else 
    {// request buffer failed, return fail immediately.
        eError = ERR_VDEC_BUF_FULL;
    }
...
ERRORTYPE VideoDecComponentInit(PARAM_IN COMP_HANDLETYPE hComponent)
{
    ...
    alogd("create VideoDec comp threadId[0x%lx]", pVideoDecData->thread_id);
