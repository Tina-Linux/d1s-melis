 ...
 ******************************************************************************
  ...
  Created       : 2016/08/14
 *  Last Modified:  2021-04-22 09:28:07
...
static int AudioEncLibInit(AUDIOENCDATATYPE *pAudioEncData)
{
    ...
    char *support_format[] = {"aac", "lpcm", "pcm", "adpcm", "mp3", "g711a", "g711u", "g726", "other"};
...
ERRORTYPE AudioEncGetFirstAudioFrmPts(
        PARAM_IN COMP_HANDLETYPE hComponent,
        PARAM_OUT long long* a_pts)
{
    ...
    *a_pts = pAudioEncData->first_audio_pts; 
        ...
        PARAM_IN  COMP_BUFFERHEADERTYPE* pBuffer)
{
    ...
    if ((pAudioEncData->mOutputPortTunnelFlag==TRUE && pBuffer->nInputPortIndex==pAudioEncData->sInPortDef.nPortIndex) ||
        (pAudioEncData->mOutputPortTunnelFlag==FALSE))
    {
        ...
        {
            pAudioEncData->first_audio_pts = pFrm->tmp_pts;
        }

        //pthread_mutex_lock(&pAudioEncData->mInputPcmMutex);
        int ret = WriteAudioStreamBuffer(pAudioEncData->pCedarA, pAudioBuf, bufSize);
...
ERRORTYPE AudioEncComponentInit(PARAM_IN COMP_HANDLETYPE hComponent)
{
    ...
    pComp->GetFristAudioFrmPts = AudioEncGetFirstAudioFrmPts;
    ...
    alogd("create AudioEnc threadId[0x%lx]", pAudioEncData->thread_id);
    EXIT: return eError;
}
