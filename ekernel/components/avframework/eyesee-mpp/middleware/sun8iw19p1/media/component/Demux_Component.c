...
#define MAX_USE_BUFFER_NUM  (6)
#define PER_BLOCK_SIZE   (1024*64*4)

typedef struct _BUFFER_MNG
{
    int mPktListNodeCnt;
    struct list_head idlePktList;
...
typedef struct _DEMUXDATATYPE
{
    ...
    int64_t                     max_send_packet_time_video;
    int64_t                     max_send_packet_time_audio;
...
ERRORTYPE DemuxComponentInit(PARAM_IN COMP_HANDLETYPE hComponent)
{
    ...
    alogd("create Demux threadId[0x%lx]", pDemuxData->thread_id);
#if DEBUG_SAVE_VIDEO_STREAM
...
PROCESS_MESSAGE:
        ...
        // Get system time
        int64_t system_time;
        if (pDemuxData->mOutputPortTunnelFlag)
        {
            if(hnd_clock_comp)
            {
