...
ERRORTYPE DoVideoIseAddPort(PARAM_IN COMP_HANDLETYPE hComponent, int iseGrpId, int iseChnId, IseChnAttr *pIseChnAttr)
{
    ...
    if (ERR_ISE_UNEXIST == eRet)
    {
        ...
        if (ISEMODE_ONE_FISHEYE == pVideoIseData->mIseGrpAttr.iseMode)
        {
            ...
            width = pChnAttr_fish_cfg->out_chroma_pitch[iseChnId];
            height = pChnAttr_fish_cfg->out_h[iseChnId];
            ...
            pVideoIseData_fish_gdc_cfg = &pVideoIseData->mIseChnAttr.mode_attr.mFish.ise_gdc_cfg;
            width = pChnAttr_fish_gdc_cfg->undistImage.width;
            height = pChnAttr_fish_gdc_cfg->undistImage.height;
        ...
        else if (ISEMODE_TWO_FISHEYE == pVideoIseData->mIseGrpAttr.iseMode)
        {
            ...
            width = pChnAttr_dfish_cfg->out_chroma_pitch[iseChnId];
			height = pChnAttr_dfish_cfg->out_h[iseChnId];
        ...
        else if (ISEMODE_TWO_ISE == pVideoIseData->mIseGrpAttr.iseMode)
        {
            ...
            if (0 == iseChnId)
            {
                width = pIseChnAttr->pChnAttr->mode_attr.mIse.ise_cfg.pano_chroma_pitch;
                height = pIseChnAttr->pChnAttr->mode_attr.mIse.ise_cfg.pano_h;
            }
            else if (1 <= iseChnId && iseChnId <= 3)
            {
                width = pChnAttr_ise_cfg->scalar_chroma_pitch[iseChnId - 1];
                height = pChnAttr_ise_cfg->scalar_h[iseChnId - 1];
        ...
        {
            pEntry->mpOutBuf = (ISEOutBuf_t *)malloc(sizeof(ISEOutBuf_t));
            memset(pEntry->mpOutBuf, 0, sizeof(ISEOutBuf_t));
            INIT_LIST_HEAD(&pEntry->mpOutBuf->mList);
            if (ISEMODE_ONE_FISHEYE == pVideoIseData->mIseGrpAttr.iseMode
                    || ISEMODE_TWO_FISHEYE == pVideoIseData->mIseGrpAttr.iseMode )  //add ISEMODE_TWO_FISHEYE!!
            {
#if (MPPCFG_ISE_GDC == OPTION_ISE_GDC_ENABLE)
                {
                // Y data, Phy & Vir Addr
                ...
                if(pIseChnAttr->pChnAttr->mode_attr.mFish.ise_gdc_cfg.rectPara.warpMode == Warp_BirdsEye)
                {

                    width = pIseChnAttr->pChnAttr->mode_attr.mFish.ise_gdc_cfg.birdsEyeImage.width;
                    height = pIseChnAttr->pChnAttr->mode_attr.mFish.ise_gdc_cfg.birdsEyeImage.height;
                    ldc_width = pIseChnAttr->pChnAttr->mode_attr.mFish.ise_gdc_cfg.undistImage.width;
                    ldc_height = pIseChnAttr->pChnAttr->mode_attr.mFish.ise_gdc_cfg.undistImage.height;
                    stAttr.mLen = ALIGN(width,16)*ALIGN(height,16)+ALIGN(ldc_width,16)*ALIGN(ldc_height,16);
                    alogd("===>>> Warp_BirdsEye ISE GDC out image  ionallcomem birdsEyeImage:%d*%d + undistImage:%d*%d!!!",width,height,ldc_width,ldc_height);
                }
                else
                {
                    width = pIseChnAttr->pChnAttr->mode_attr.mFish.ise_gdc_cfg.undistImage.width;
                    height = pIseChnAttr->pChnAttr->mode_attr.mFish.ise_gdc_cfg.undistImage.height;
                    stAttr.mLen = ALIGN(width,16)*ALIGN(height,16);
                    alogd("===>>> ISE GDC out image ionallcomem W*H=%d*%d!!!",width,height);
                }
                stAttr.mAlign = 0;
                stAttr.mIonHeapType = IonHeapType_CARVEOUT;
                stAttr.mbSupportCache = 0;
                pEntry->mpOutBuf->mIseOut.out_luma_mmu_Addr[iseChnId] =  ion_allocMem_extend(&stAttr);
                if (NULL == pEntry->mpOutBuf->mIseOut.out_luma_mmu_Addr[iseChnId])
                {
                 eRet = FAILURE;
                 aloge("Add ise chn%d, Palloc pano_luma failed.",iseChnId);
                }
                memset(pEntry->mpOutBuf->mIseOut.out_luma_mmu_Addr[iseChnId], 0x0, stAttr.mLen);
                pEntry->mpOutBuf->mIseOut.out_luma_phy_Addr[iseChnId] =
                (unsigned int)ion_getMemPhyAddr(pEntry->mpOutBuf->mIseOut.out_luma_mmu_Addr[iseChnId]);
                pEntry->mpOutBuf->VFrame.VFrame.mpVirAddr[0] = pEntry->mpOutBuf->mIseOut.out_luma_mmu_Addr[iseChnId];
                pEntry->mpOutBuf->VFrame.VFrame.mPhyAddr[0] = pEntry->mpOutBuf->mIseOut.out_luma_phy_Addr[iseChnId];
                // U + V data, Phy & Vir Addr
                memset(&stAttr, 0, sizeof(IonAllocAttr));
                if(pIseChnAttr->pChnAttr->mode_attr.mFish.ise_gdc_cfg.rectPara.warpMode == Warp_BirdsEye)
                {
                    stAttr.mLen = (ALIGN(width,16)*ALIGN(height,16))/2+(ALIGN(ldc_width,16)*ALIGN(ldc_height,16))/2;
                }
                else
                {
                    stAttr.mLen = (ALIGN(width,16)*ALIGN(height,16))/2;
                }
                stAttr.mAlign = 0;
                stAttr.mIonHeapType = IonHeapType_CARVEOUT;
                stAttr.mbSupportCache = 0;
                pEntry->mpOutBuf->mIseOut.out_chroma_u_mmu_Addr[iseChnId] =  ion_allocMem_extend(&stAttr);
                if (NULL == pEntry->mpOutBuf->mIseOut.out_chroma_u_mmu_Addr[iseChnId])
                {
                 eRet = FAILURE;
                 aloge("Add ise chn x, Palloc pano_chroma failed.\r\n");
                }
                memset(pEntry->mpOutBuf->mIseOut.out_chroma_u_mmu_Addr[iseChnId], 0x0, stAttr.mLen);
                pEntry->mpOutBuf->mIseOut.out_chroma_u_phy_Addr[iseChnId] =
                (unsigned int)ion_getMemPhyAddr(pEntry->mpOutBuf->mIseOut.out_chroma_u_mmu_Addr[iseChnId]);
                pEntry->mpOutBuf->VFrame.VFrame.mpVirAddr[1] = pEntry->mpOutBuf->mIseOut.out_chroma_u_mmu_Addr[iseChnId];
                pEntry->mpOutBuf->VFrame.VFrame.mPhyAddr[1] = pEntry->mpOutBuf->mIseOut.out_chroma_u_phy_Addr[iseChnId];
                }
#else
                {
                 alogd("===>>> ISE MO out image ionallcomem W %d*H %d!!!",width,height);
                 // Y data, Phy & Vir Addr
                 IonAllocAttr stAttr;
                 memset(&stAttr, 0, sizeof(IonAllocAttr));
                 stAttr.mLen = ALIGN(width,16)*ALIGN(height,16);
            ...
            else if (ISEMODE_TWO_ISE == pVideoIseData->mIseGrpAttr.iseMode)
            {
                ...
                if(pEntry->mIseChn == 0)
                {
                    IonAllocAttr stAttr;
                    memset(&stAttr, 0, sizeof(IonAllocAttr));
                    stAttr.mLen = width * height;
                    stAttr.mAlign = 0;
                    stAttr.mIonHeapType = IonHeapType_CARVEOUT;
                    stAttr.mbSupportCache = 0;
                    pEntry->mpOutBuf->mStiOut.pano_luma_mmu_Addr = ion_allocMem_extend(&stAttr);
                    ...
                    (unsigned int)ion_getMemPhyAddr(pEntry->mpOutBuf->mStiOut.pano_luma_mmu_Addr);
                    pEntry->mpOutBuf->VFrame.VFrame.mpVirAddr[0] = pEntry->mpOutBuf->mStiOut.pano_luma_mmu_Addr;
                    pEntry->mpOutBuf->VFrame.VFrame.mPhyAddr[0] = pEntry->mpOutBuf->mStiOut.pano_luma_phy_Addr;
                    // U + V data, Phy & Vir Addr
                    memset(&stAttr, 0, sizeof(IonAllocAttr));
                    stAttr.mLen = width * height / 2;
                    stAttr.mAlign = 0;
                    stAttr.mIonHeapType = IonHeapType_CARVEOUT;
                    stAttr.mbSupportCache = 0;
                    pEntry->mpOutBuf->mStiOut.pano_chroma_mmu_Addr =
                            ion_allocMem_extend(&stAttr);
                ...
                else if(pEntry->mIseChn == 1 || pEntry->mIseChn == 2 || pEntry->mIseChn == 3)
                {
                    IonAllocAttr stAttr;
                    memset(&stAttr, 0, sizeof(IonAllocAttr));
                    stAttr.mLen = width * height;
                    ...
                    stAttr.mIonHeapType = IonHeapType_CARVEOUT;
                    stAttr.mbSupportCache = 0;
                    pEntry->mpOutBuf->mStiOut.scalar_luma_mmu_Addr[pEntry->mIseChn-1] =
                            ion_allocMem_extend(&stAttr);
                    ...
                    (unsigned int)ion_getMemPhyAddr(pEntry->mpOutBuf->mStiOut.scalar_luma_mmu_Addr[pEntry->mIseChn-1]);
                    pEntry->mpOutBuf->VFrame.VFrame.mpVirAddr[0] = pEntry->mpOutBuf->mStiOut.scalar_luma_mmu_Addr[pEntry->mIseChn-1];
                    pEntry->mpOutBuf->VFrame.VFrame.mPhyAddr[0] = pEntry->mpOutBuf->mStiOut.scalar_luma_phy_Addr[pEntry->mIseChn-1];
                    // U + V data, Phy & Vir Addr
                    memset(&stAttr, 0, sizeof(IonAllocAttr));
                    stAttr.mLen = width * height / 2;
                    stAttr.mAlign = 0;
                    stAttr.mIonHeapType = IonHeapType_CARVEOUT;
                    stAttr.mbSupportCache = 0;
                    pEntry->mpOutBuf->mStiOut.scalar_chroma_mmu_Addr[pEntry->mIseChn-1] =
                            ion_allocMem_extend(&stAttr);
            ...
            pEntry->mpOutBuf->mIseOutId = (i << 8) | 0xFFFF0000;
            VIDEO_FRAME_INFO_S *pVFrameInfo = &pEntry->mpOutBuf->VFrame;
            pVFrameInfo->mId = i;
            pVFrameInfo->VFrame.mWidth = width;
            pVFrameInfo->VFrame.mHeight = height;
            //pVFrameInfo->VFrame.mField = VIDEO_FIELD_TOP;
            ...
            alogd("Port[%d]data_index=%d: phy:y=%#x,uv=%#x; vir:y=%p,uv=%p,pixel format:%d",
                ...
                pEntry->mpOutBuf->VFrame.VFrame.mPhyAddr[0],
                pEntry->mpOutBuf->VFrame.VFrame.mPhyAddr[1],
                pEntry->mpOutBuf->VFrame.VFrame.mpVirAddr[0],
                pEntry->mpOutBuf->VFrame.VFrame.mpVirAddr[1],
                pVFrameInfo->VFrame.mPixelFormat);
            list_move_tail(&pEntry->mpOutBuf->mList, &pEntry->mIdleOutYUVList);
        }
    ...
    if (!list_empty(&pVideoIseData->mValidChnAttrList))
    {
        list_for_each_entry_safe(pEntry, pTmp, &pVideoIseData->mValidChnAttrList, mList)
        {
            if (pEntry->mIseChn == iseChnId)
            {
                ...
                {
                    list_move_tail(&pEntry->mReadyOutYUVList, &pEntry->mIdleOutYUVList);
                }
                if (!list_empty(&pEntry->mUsedOutYUVList))
                {
                    list_move_tail(&pEntry->mUsedOutYUVList, &pEntry->mIdleOutYUVList);
                }*/
                ...
                pEntry_1 = NULL,pTmp_1 = NULL;
                if (!list_empty(&pEntry->mIdleOutYUVList))
                {
                    list_for_each_entry_safe(pEntry_1, pTmp_1, &pEntry->mIdleOutYUVList, mList)
                    {
                        if (NULL != pEntry_1->VFrame.VFrame.mpVirAddr[0])
                        {
                            ion_freeMem(pEntry_1->VFrame.VFrame.mpVirAddr[0]);
                            pEntry_1->mIseOut.out_luma_mmu_Addr[iseChnId] = NULL;
                            pEntry_1->VFrame.VFrame.mpVirAddr[0] = NULL;
                            pEntry_1->VFrame.VFrame.mPhyAddr[0] = 0;
                        }
                        if (NULL != pEntry_1->VFrame.VFrame.mpVirAddr[1])
                        {
                            ion_freeMem(pEntry_1->VFrame.VFrame.mpVirAddr[1]);
                            pEntry_1->mIseOut.out_chroma_u_mmu_Addr[iseChnId] = NULL;
                            pEntry_1->VFrame.VFrame.mpVirAddr[1] = NULL;
                            pEntry_1->VFrame.VFrame.mPhyAddr[1] = 0;
                        }
                        cnt++;
                        list_del(&pEntry_1->mList);
                        if(pEntry_1 != NULL){
                            free(pEntry_1);
                        }
                    }
                    ...
                    list_for_each_entry(pEntry, &pVideoIseData->mValidChnAttrList, mList)
                    {
                            ...
                            (2 == pEntry->mIseChn) || (3 == pEntry->mIseChn))
                        {
                            ...
                            if (!list_empty(&(pEntry->mIdleOutYUVList))) // copy item to ise proc out buf
                            {
                                ...
                                if(pVideoIseData->mIseChnAttr.mode_attr.mFish.ise_gdc_cfg.rectPara.warpMode == Warp_BirdsEye)
                                {
                                    /*when the warpMode == Warp_BirdsEye,there have tow output image BirdsEye+LDC*/
                                    width = pVideoIseData->mIseChnAttr.mode_attr.mFish.ise_gdc_cfg.birdsEyeImage.width;
                                    height = pVideoIseData->mIseChnAttr.mode_attr.mFish.ise_gdc_cfg.birdsEyeImage.height;
...
#else
                                width = pVideoIseData->mIseChnAttr.mode_attr.mFish.ise_cfg.out_luma_pitch[pEntry->mIseChn];
                                ...
                                ise_procout.out_luma_mmu_Addr[pEntry->mIseChn] = (*pOutBuf[pEntry->mIseChn]).mIseOut.out_luma_mmu_Addr[pEntry->mIseChn];
                                ise_procout.out_chroma_u_mmu_Addr[pEntry->mIseChn] = (*pOutBuf[pEntry->mIseChn]).mIseOut.out_chroma_u_mmu_Addr[pEntry->mIseChn];
                                ise_procout.out_luma_phy_Addr[pEntry->mIseChn] = (*pOutBuf[pEntry->mIseChn]).mIseOut.out_luma_phy_Addr[pEntry->mIseChn];
                                ise_procout.out_chroma_u_phy_Addr[pEntry->mIseChn] = (*pOutBuf[pEntry->mIseChn]).mIseOut.out_chroma_u_phy_Addr[pEntry->mIseChn];
#if (MPPCFG_ISE_GDC == OPTION_ISE_GDC_ENABLE)
                                if(pVideoIseData->mIseChnAttr.mode_attr.mFish.ise_gdc_cfg.rectPara.warpMode == Warp_BirdsEye)
                                {
                                    ise_procout_ldc.out_luma_mmu_Addr[pEntry->mIseChn] = (*pOutBuf[pEntry->mIseChn]).mIseOut.out_luma_mmu_Addr[pEntry->mIseChn]+width*height;
                                    ise_procout_ldc.out_chroma_u_mmu_Addr[pEntry->mIseChn] = (*pOutBuf[pEntry->mIseChn]).mIseOut.out_chroma_u_mmu_Addr[pEntry->mIseChn]+width*height/2;
                                    ise_procout_ldc.out_luma_phy_Addr[pEntry->mIseChn] = (*pOutBuf[pEntry->mIseChn]).mIseOut.out_luma_phy_Addr[pEntry->mIseChn]+width*height;
                                    ise_procout_ldc.out_chroma_u_phy_Addr[pEntry->mIseChn] = (*pOutBuf[pEntry->mIseChn]).mIseOut.out_chroma_u_phy_Addr[pEntry->mIseChn]+width*height/2;
                                }
#endif
                                if(pVideoIseData->mIseChnAttr.mode_attr.mFish.ise_gdc_cfg.rectPara.warpMode == Warp_BirdsEye)
                                {
                                    result = ion_flushCache(ise_procout.out_luma_mmu_Addr[pEntry->mIseChn],width*height + ldc_width*ldc_height);
                                    if(result < 0)
                                            aloge("ion flush cache failed!\n");
                                    result = ion_flushCache(ise_procout.out_chroma_u_mmu_Addr[pEntry->mIseChn],((width*height)>>1)+((ldc_width*ldc_height)>>1));
                                    if(result < 0)
                                            aloge("ion flush cache failed!\n");
                                }
                                else
                    ...
                    list_for_each_entry(pEntry, &pVideoIseData->mValidChnAttrList, mList)
                    {
                           ...
                           (2 == pEntry->mIseChn) || (3 == pEntry->mIseChn))
                        {
                            ...
                            if (!list_empty(&(pEntry->mIdleOutYUVList))) // copy item to ise proc out buf
                            {
                                ...
                                ise_procout.out_luma_phy_Addr[pEntry->mIseChn] = (*pOutBuf[pEntry->mIseChn]).mIseOut.out_luma_phy_Addr[pEntry->mIseChn];
                                ise_procout.out_chroma_u_phy_Addr[pEntry->mIseChn] = (*pOutBuf[pEntry->mIseChn]).mIseOut.out_chroma_u_phy_Addr[pEntry->mIseChn];
                                width = pVideoIseData->mIseChnAttr.mode_attr.mDFish.ise_cfg.out_luma_pitch[pEntry->mIseChn];
                ...
                if (!list_empty(&(pVideoIseData->mValidChnAttrList)))
                {
                    list_for_each_entry(pEntry, &pVideoIseData->mValidChnAttrList, mList)
                    {
                        ...
                        if (0 == pEntry->mIseChn)
                        {
                            ...
                            {
                                pOutBuf[pEntry->mIseChn] = list_first_entry(&(pEntry->mIdleOutYUVList), ISEOutBuf_t, mList);
                                ise_procout.pano_luma_mmu_Addr = (*pOutBuf[pEntry->mIseChn]).mStiOut.pano_luma_mmu_Addr;
                                ise_procout.pano_chroma_mmu_Addr = (*pOutBuf[pEntry->mIseChn]).mStiOut.pano_chroma_mmu_Addr;
                                ise_procout.pano_luma_phy_Addr = (*pOutBuf[pEntry->mIseChn]).mStiOut.pano_luma_phy_Addr;
                                ise_procout.pano_chroma_phy_Addr = (*pOutBuf[pEntry->mIseChn]).mStiOut.pano_chroma_phy_Addr;
                                width = pVideoIseData->mIseChnAttr.mode_attr.mIse.ise_cfg.pano_luma_pitch;
                                height = pVideoIseData->mIseChnAttr.mode_attr.mIse.ise_cfg.pano_h;
                           ...
                           (2 == pEntry->mIseChn) || (3 == pEntry->mIseChn))
                        {
                            ...
                            {
                                pOutBuf[pEntry->mIseChn] = list_first_entry(&(pEntry->mIdleOutYUVList), ISEOutBuf_t, mList);
                                ise_procout.scalar_luma_mmu_Addr[pEntry->mIseChn-1] = (*pOutBuf[pEntry->mIseChn]).mStiOut.scalar_luma_mmu_Addr[pEntry->mIseChn-1];
                                ise_procout.scalar_chroma_mmu_Addr[pEntry->mIseChn-1] = (*pOutBuf[pEntry->mIseChn]).mStiOut.scalar_chroma_mmu_Addr[pEntry->mIseChn-1];
                                ise_procout.scalar_luma_phy_Addr[pEntry->mIseChn-1] = (*pOutBuf[pEntry->mIseChn]).mStiOut.scalar_luma_phy_Addr[pEntry->mIseChn-1];
                                ise_procout.scalar_chroma_phy_Addr[pEntry->mIseChn-1] = (*pOutBuf[pEntry->mIseChn]).mStiOut.scalar_chroma_phy_Addr[pEntry->mIseChn-1];
                                width = pVideoIseData->mIseChnAttr.mode_attr.mIse.ise_proccfg.scalar_luma_pitch[pEntry->mIseChn-1];
