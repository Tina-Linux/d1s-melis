...
static ERRORTYPE RPCMGetDuration_l(PARAM_IN COMP_HANDLETYPE hComponent, PARAM_OUT RPCMDuration *pRPCMDuration)
{
    ...
    if(!list_empty(&manager->mUsingPacketList))
    {
        ...
        {
            if (CODEC_TYPE_VIDEO == pPacketEntry->mStreamType)
            {
        ...
        if (nUsingLastPts >= 0 && nUsingBeginPts >= 0)
        {
    ...
    if(!list_empty(&manager->mReadyPacketList))
    {
        ...
        if (nReadyBeginPts >= 0 && nReadyLastPts >= 0)
        {
...
static ERRORTYPE RPCMIncreaseDataBufList(RsPacketCacheManager *pThiz, int nSize)
{
    ...
    //    goto _err0;
    //}
    //pDB->mpBuffer = memalign(4096, nSize);
...
ERRORTYPE RPCMRefPacket(PARAM_IN COMP_HANDLETYPE hComponent, PARAM_OUT int nId)
{
    ERRORTYPE eError = SUCCESS;
    int    nFindFlag = 0;
    RsPacketCacheManager *manager = (RsPacketCacheManager*)hComponent;
...
Time: 2018/4/2
*******************************************************************************/
static ERRORTYPE RPCMReleasePacket_l(PARAM_IN COMP_HANDLETYPE hComponent, PARAM_OUT int nId)
{
    RsPacketCacheManager *manager = (RsPacketCacheManager*)hComponent;
    int nFindFlag = 0;
...
Time: 2018/4/2
*******************************************************************************/
static ERRORTYPE RPCMReleaseReadyPacket_l(PARAM_IN COMP_HANDLETYPE hComponent, PARAM_OUT int nId)
{
    RsPacketCacheManager *manager = (RsPacketCacheManager*)hComponent;
    int nFindFlag     = 0;
    BOOL nReleaseFlag = FALSE;
...
static ERRORTYPE RPCMReleasePacket(PARAM_IN COMP_HANDLETYPE hComponent, PARAM_OUT int nId)
{
    RsPacketCacheManager *manager = (RsPacketCacheManager*)hComponent;
    ERRORTYPE eError = SUCCESS;
