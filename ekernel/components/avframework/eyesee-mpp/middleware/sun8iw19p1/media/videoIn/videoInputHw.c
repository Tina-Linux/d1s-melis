...
BOOL checkRegionPositionValid(RGN_ATTR_S *pRgnAttr, RGN_CHN_ATTR_S *pRgnChnAttr)
{
    ...
    switch(pRgnAttr->enType)
    {
        ...
        default:
        {
            aloge("fatal error! v316 don't support region type:%d!", pRgnAttr->enType);
            bValid = FALSE;
            break;
        }
    }
    ...
    switch(pRgnChnAttr->enType)
    {
        ...
        default:
        {
            aloge("fatal error! v459 don't support region type:%d!", pRgnChnAttr->enType);
            bValid = FALSE;
            break;
        }
    }
...
ERRORTYPE videoInputHw_IspAe_GetISOSensitive(int *pvipp_id, int *value)
{
	...
	/*
        ...
        ISP_PRINT("****video_get_control value[%d]****\n", *value);
        if (1 == *value) {
            *value = 0;
            return SUCCESS;
        }
    ...
    *value = *value + 1;
#endif
	if (isp_get_attr_cfg(isp_id, ISP_CTRL_ISO_AGAIN, value) < 0) {
		return FAILURE;
	}
    return SUCCESS;
}
...
#define VIN_ALIGN_WIDTH 16 //ref to ./lichee/linux-4.9/drivers/media/platform/sunxi-vin/platform/platform_cfg.h
...
void * VideoInputHw_CapThread(void *pThreadData)
{
	...
	prctl(PR_SET_NAME, (unsigned long)"VICaptureThread", 0, 0, 0);
    ...
    while ( 1 ) {
		...
		if (0 == videoInputHw_GetData(&vipp_id, &pstFrameInfo, nMilliSec)) { /* success : get yuv data */
            ...
            if (iDropFrameNum > 0) {
                ...
                if(0 == iDropFrameNum)
                {
                    alogd("drop %d frames done!", attr.drop_frame_num);
                }
                continue;
            }
