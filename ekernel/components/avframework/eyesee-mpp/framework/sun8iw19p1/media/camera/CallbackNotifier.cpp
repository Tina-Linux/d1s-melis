                    ...
                    const struct isp_exif_attribute* pIspExif)
{
    ...
    if (isMessageEnabled(CAMERA_MSG_RAW_IMAGE) && (!isThumbnailPic))
    {
        ...
        alogd("takePicture: Raw size %d", bufsize);
...
//    else
    ...
    if(NULL == mpJpegenc)
    {
        ...
        //ion_memGetTotalSize();
        unsigned int minVbvBufSize = (picWidth * picHeight * 3/2 /2) + (picWidth*picHeight/3);
        unsigned int vbvBufSize = (picWidth * picHeight * 3/2 /10 * mPictureNum) + (picWidth*picHeight/3);
