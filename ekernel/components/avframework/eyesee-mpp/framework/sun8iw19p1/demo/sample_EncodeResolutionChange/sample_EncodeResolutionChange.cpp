...
status_t SampleEncodeResolutionChangeContext::loadConfig()
{
    ...
    if(ConfigFilePath.empty())
    {
        ...
        mConfigPara.mEncodeFolderPath = "/mnt/E/sample_EncodeResolutionChange_Files";
...
{
    SampleEncodeResolutionChangeContext* pContext = (SampleEncodeResolutionChangeContext*) pThreadData;
    int ret;
    ...
    prctl(PR_SET_NAME, (unsigned long)"demo_EncodeResolutionChange_DetectInput", 0, 0, 0);
    while(1)
    {
...
int main_demo_EncodeResolutionChange(int argc, char *argv[])
{
    int result = 0;
    int ret;
    void *pValue = NULL;
    ...
    strFilePath = pContext->mConfigPara.mEncodeFolderPath + '/' + pContext->mConfigPara.mFilePath0;
    nFallocateLen = 0;
    //nFallocateLen = 100*1024*1024;
    //pContext->mpRecorder->addOutputFormatAndOutputSink(MEDIA_FILE_FORMAT_MP4, (char*)strFilePath.c_str(), nFallocateLen, false);
    ...
    pContext->mpRecorder->setVideoFrameRate(pContext->mConfigPara.mDstFrameRate);
    ...
    pContext->mpRecorder->setVEncAttr(&stVEncAttr);
    pContext->mpRecorder->setVideoEncoder(pContext->mConfigPara.mEncodeType);
    alogd("setEncodeVideoSize=[%dx%d]", pContext->mConfigPara.mEncodeWidth0, pContext->mConfigPara.mEncodeHeight0);
    pContext->mpRecorder->setVideoSize(pContext->mConfigPara.mEncodeWidth0, pContext->mConfigPara.mEncodeHeight0);
    alogd("setIDRFrameInterval=[%d]", pContext->mConfigPara.mIDRFrameInterval);
    pContext->mpRecorder->setVideoEncodingIFramesNumberInterval(pContext->mConfigPara.mIDRFrameInterval);
    pContext->mpRecorder->enableVideoEncodingPIntra(pContext->mConfigPara.mbVideoEncodingPIntra);

    alogd("setVideoRCMode=[%d]", pContext->mConfigPara.mVideoRCMode);
    pContext->mpRecorder->setVideoEncodingRateControlMode(pContext->mConfigPara.mVideoRCMode);
    if(EyeseeLinux::EyeseeRecorder::VideoRCMode_CBR == pContext->mConfigPara.mVideoRCMode)
    {
        ...
        pContext->mpRecorder->setVEncBitRateControlAttr(stRcAttr);
    }
    else if(EyeseeLinux::EyeseeRecorder::VideoRCMode_FIXQP == pContext->mConfigPara.mVideoRCMode)
    {
        int IQp = pContext->mConfigPara.mQp1;
        int PQp = pContext->mConfigPara.mQp2;
        EyeseeRecorder::VEncBitRateControlAttr stRcAttr;
        stRcAttr.mVEncType = pContext->mConfigPara.mEncodeType;
        stRcAttr.mRcMode = EyeseeRecorder::VideoRCMode_FIXQP;
        switch(pContext->mConfigPara.mEncodeType)
        {
            case PT_H264:
            {
    ...
    else if(EyeseeLinux::EyeseeRecorder::VideoRCMode_ABR == pContext->mConfigPara.mVideoRCMode)
    {
        int minIQp = pContext->mConfigPara.mQp1;
        int maxIQp = pContext->mConfigPara.mQp2;
        EyeseeRecorder::VEncBitRateControlAttr stRcAttr;
        stRcAttr.mVEncType = pContext->mConfigPara.mEncodeType;
        stRcAttr.mRcMode = EyeseeRecorder::VideoRCMode_ABR;
        switch(pContext->mConfigPara.mEncodeType)
        {
            case PT_H264:
            {
    ...
    pContext->mpRecorder->getGopAttr(&GopAttr);
    ...
    pContext->mpRecorder->setGopAttr(&GopAttr);

    if(pContext->mConfigPara.mbAdvanceRef)
    {
        VENC_PARAM_REF_S RefParam;
        pContext->mpRecorder->getRefParam(&RefParam);
        RefParam.Base = pContext->mConfigPara.nBase;
        RefParam.Enhance = pContext->mConfigPara.nEnhance;
        RefParam.bEnablePred = (BOOL)pContext->mConfigPara.bRefBaseEn;
        pContext->mpRecorder->setRefParam(&RefParam);
    }
    ...
    if(pContext->mConfigPara.mbEnableSmartP)
    {
        ...
        pContext->mpRecorder->setVideoEncodingSmartP(&smartParam);
    }
    ...
    pContext->mpRecorder->SetVideoEncodingIntraRefresh(&stIntraRefresh);
    
    pContext->mpRecorder->setCaptureRate(pContext->mConfigPara.mTimeLapseFps);

    //pContext->mpRecorder->enableVideoEncodingLongTermRef(pContext->mConfigPara.mbLongTermRef);
    //if(pContext->mConfigPara.mbLongTermRef)
    //{
    //    alogd("had open longtermref!");
   // }
   // else
   //// {
   //     alogd("had close longtermref!");
   // }

    pContext->mpRecorder->enableHorizonFlip(pContext->mConfigPara.mbHorizonfilp);
    pContext->mpRecorder->enableAdaptiveIntraInp(pContext->mConfigPara.mbAdaptiveintrainp);
    pContext->mpRecorder->enable3DNR(pContext->mConfigPara.mb3dnr);
    pContext->mpRecorder->enableColor2Grey(pContext->mConfigPara.mbcolor2grey);
    pContext->mpRecorder->enableNullSkip(pContext->mConfigPara.mbNullSkipEnable);
    ...
    pContext->mpRecorder->setVencSuperFrameConfig(&stSuperFrameConfig);
    ...
    if(!pContext->mConfigPara.mFilePath1.empty())
    {
        ...
        //pContext->mpRecorder->addOutputFormatAndOutputSink(MEDIA_FILE_FORMAT_MP4, (char*)strFilePath.c_str(), 0, false);
        SinkParam stSinkParam;
        memset(&stSinkParam, 0, sizeof(SinkParam));
        stSinkParam.mOutputFormat = MEDIA_FILE_FORMAT_MP4;
        stSinkParam.mOutputFd = -1;
        ...
        pContext->mpRecorder->setVideoSize(pContext->mConfigPara.mEncodeWidth1, pContext->mConfigPara.mEncodeHeight1);
        alogd("setIDRFrameInterval=[%d]", pContext->mConfigPara.mIDRFrameInterval);
        pContext->mpRecorder->setVideoEncodingIFramesNumberInterval(pContext->mConfigPara.mIDRFrameInterval);
        pContext->mpRecorder->enableVideoEncodingPIntra(pContext->mConfigPara.mbVideoEncodingPIntra);
...
_err0:
    ...
    pSampleEncodeResolutionChangeContext = NULL;
    return result;
}

FINSH_FUNCTION_EXPORT_ALIAS(main_demo_EncodeResolutionChange, __cmd_demo_EncodeResolutionChange, test mpp framework recorder);
