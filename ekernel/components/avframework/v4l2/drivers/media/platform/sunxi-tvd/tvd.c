				...
				struct v4l2_format *f)
{
	...
	if (temp_sel != dev->sel) {
		...
		hal_free_irq(tvd_irq[temp_sel]);
		/* register irq again*/
		tvd_dbg("request tvd%d's irq:%d\n", dev->sel, dev->irq);
		ret = hal_request_irq(dev->irq, tvd_isr, dev->name, dev);
		if (ret != 0)
			goto OUT;
	}
...
static int tvd_open(struct file *file)
{
	...
	ret = hal_request_irq(dev->irq, tvd_isr, dev->v4l2_dev.name, dev);
...
static int tvd_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_SHARPNESS:
		c.value = tvd_get_sharpness(dev->sel);
		break;
	case V4L2_CID_HUE:
		c.value = tvd_get_hue(dev->sel);
		break;
	}
	ctrl->val = c.value;
...
static int tvd_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_SHARPNESS:
		...
		if (dev->mulit_channel_mode) {
			for (i = 0; i < tvd_total_num; ++i) {
				if (tvd_status[i].tvd_used)
					tvd_set_sharpness(i, ctrl->val);
			}

		}
		else
			tvd_set_sharpness(dev->sel, ctrl->val);
		break;
	case V4L2_CID_HUE:
		tvd_dbg("%s: V4L2_CID_SATURATION sel=%d, val=%d,\n", __func__,
			dev->sel, ctrl->val);
		if (dev->mulit_channel_mode) {
			for (i = 0; i < tvd_total_num; ++i) {
				if (tvd_status[i].tvd_used)
					tvd_set_hue(i, ctrl->val);
			}

		}
		else
			tvd_set_hue(dev->sel, ctrl->val);
		break;
...
static int __tvd_init_controls(struct v4l2_ctrl_handler *hdl)
{
	...
	v4l2_ctrl_new_std(hdl, &tvd_ctrl_ops, V4L2_CID_SHARPNESS, -32, 32, 1, 0);
	v4l2_ctrl_new_std(hdl, &tvd_ctrl_ops, V4L2_CID_HUE, 0, 256, 1, 0);
...
static int tvd_resume(struct device *d)
{
	...
	if ((!tvd_task) && (tvd_hot_plug)) {
		tvd_task = kthread_create(__tvd_detect_thread, (void *)0,
					  "tvd detect", HAL_THREAD_STACK_SIZE, HAL_THREAD_PRIORITY);
		if (IS_ERR(tvd_task)) {
			s32 err = 0;
