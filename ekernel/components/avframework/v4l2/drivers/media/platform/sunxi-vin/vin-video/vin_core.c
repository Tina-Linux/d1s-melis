...
static hal_spinlock_t vin_core_lock;
				  ...
				  unsigned int mask, int index, bool have_code)
{
	...
	int vin_fmt_size = ARRAY_SIZE(vin_formats);

	if (index >= vin_fmt_size) {
		return NULL;
	}
...
static size_t vin_status_dump(struct vin_core *vinc, char *buf, size_t size)
{
	...
	if (vinc->id == 0) {
		...
		count += snprintf(buf + count, size - count, "VIN hardware feature list:\n"
	...
	if (vinc->mipi_sel == 0xff) {
		...
		if (vinc->tdm_rx_sel == 0xff) {
			count += snprintf(buf + count, size - count, "vi%u:\n%s => csi%d => isp%d => vipp%d\n",
				vinc->id, vinc->vid_cap.pipe.sd[VIN_IND_SENSOR] ? vinc->vid_cap.pipe.sd[VIN_IND_SENSOR]->name : "(null)",
	...
	count += snprintf(buf + count, size - count,
			   ...
			   "output => width: %lu, height: %lu, fmt: %s\n"
...
static int cmd_vin_debug(int argc, char **argv)
{
	struct vin_debugfs_buffer *buf;
	int i = 0;

	buf = rt_calloc(1, sizeof(*buf));
	if (buf == NULL)
		return -ENOMEM;
...
FINSH_FUNCTION_EXPORT_ALIAS(cmd_vin_debug, __cmd_vin_debug, print vin debug info);
#endif
...
static hal_irqreturn_t vin_isr(void *priv)
{
	...
	__cpsr = hal_spin_lock_irqsave(&vin_core_lock);
	/* exception handle */
	...
	if (status.vsync_trig) {
		...
		if (cap->capture_mode != V4L2_MODE_IMAGE) {
			...
			if (list_empty(&cap->vidq_active) || cap->vidq_active.next->next == &cap->vidq_active) {
				vin_err("No active queue to serve");
				goto unlock;
			}
	...
	if (status.frame_done) {
		...
		if (rt_list_isempty(&(&buf->vb.vb2_buf.vb2_queue->done_wq)->waiting_list))
		{
...
unlock:
	hal_spin_unlock_irqrestore(&vin_core_lock, __cpsr);
...
{
	int ret;
	static const char *irqname[] = {"csic_dma0", "csic_dma1", "csic_dma2", "csic_dma3"};

	ret = hal_request_irq(vinc->irq, vin_isr,
			i < 4? irqname[i] : "csic_dmax", &vinc->vid_cap);
	if (ret) {
...
static int vin_core_probe(int id)
{
	struct vin_core *vinc = NULL;
	int ret = 0;

	vinc = sunxi_vin_get_vin_core(id);
	if (!vinc) {
		vin_err("request vinc failed!\n");
		ret = -ENOMEM;
		goto ekzalloc;
	}
	...
	sprintf(mainkey, "vind0/vinc%d", id);
	...
	sprintf(property_name, "vinc%d_rear_sensor_sel", id);
	if (script_parser_fetch(phandle, mainkey, property_name, &vinc->rear_sensor, 1))
		vinc->rear_sensor = 0;

	sprintf(property_name, "vinc%d_front_sensor_sel", id);
	if (script_parser_fetch(phandle, mainkey, property_name, &vinc->front_sensor, 1))
		vinc->front_sensor = 1;

	sprintf(property_name, "vinc%d_csi_sel", id);
	if (script_parser_fetch(phandle, mainkey, property_name, &vinc->csi_sel, 1))
		vinc->csi_sel = 0;

	sprintf(property_name, "vinc%d_mipi_sel", id);
	if (script_parser_fetch(phandle, mainkey, property_name, &vinc->mipi_sel, 1))
		vinc->mipi_sel = 0xff;

	sprintf(property_name, "vinc%d_isp_sel", id);
	if (script_parser_fetch(phandle, mainkey, property_name, &vinc->isp_sel, 1))
		vinc->isp_sel = 0;

	sprintf(property_name, "vinc%d_isp_tx_ch", id);
	if (script_parser_fetch(phandle, mainkey, property_name, &vinc->isp_tx_ch, 1))
		vinc->isp_tx_ch = 0;

	sprintf(property_name, "vinc%d_tdm_rx_sel", id);
	if (script_parser_fetch(phandle, mainkey, property_name, &vinc->tdm_rx_sel, 1))
		vinc->tdm_rx_sel = 0;
#endif

	vinc->base = sunxi_vin_get_dma_base(id);
	vinc->irq = sunxi_vin_get_dma_irq(id);
	...
	vin_log(VIN_LOG_VIDEO, "base = 0x%08x\n", vinc->base);
	vin_log(VIN_LOG_VIDEO, "irq = %d\n", vinc->irq);

	vin_irq_request(vinc, id);
...
static int vin_core_remove(int id)
{
