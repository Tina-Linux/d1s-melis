/*
 * ekernel/components/avframework/v4l2/platform/sunxi-vin/vin-csi/sunxi_csi.c
 *
...
static int __csi_pin_config(struct csi_dev *dev, int enable)
{
...
#if defined CONFIG_SOC_SUN8IW19P1 || defined CONFIG_SOC_SUN20IW1
	unsigned int func = 7;
	int i = 0;
	int num = 0;
	int *pins = NULL;
	...
	pins = sunxi_vin_get_csi_pins(dev->id);
	num = sunxi_vin_get_csi_pins_num();

	for (i = 0; i < num; i ++) {
		ret = hal_gpio_pinmux_set_function(pins[i], func);
		if (ret) {
			vin_err("set csi pins[%d] func[%d] failed\n", pins[i], func);
		}
		//hal_gpio_set_driving_level(pins[i], 1);
		//hal_gpio_set_pull(pins[i], 0);
	}
