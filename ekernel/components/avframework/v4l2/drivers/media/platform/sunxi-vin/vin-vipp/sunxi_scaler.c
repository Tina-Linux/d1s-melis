/*
 * ekernel/components/avframework/v4l2/platform/sunxi-vin/vin-vipp/sunxi_scaler.c
 *
...
int __scaler_init_subdev(struct scaler_dev *scaler)
{
	...
	rt_mutex_init(&scaler->subdev_lock, "scalersubmtx", RT_IPC_FLAG_FIFO);
...
static int scaler_resource_alloc(struct scaler_dev *scaler)
{
	...
	ret = os_mem_alloc(&scaler->vipp_reg);
	if (ret < 0) {
...
{
	os_mem_free(&scaler->vipp_reg);
}

static int scaler_probe(int id)
{
	struct scaler_dev *scaler = NULL;
	int ret = 0;

	scaler = rt_calloc(1, sizeof(struct scaler_dev));
	if (!scaler) {
		ret = -ENOMEM;
	...
	scaler->id = id;
	scaler->base = sunxi_vin_get_scaler_base(id);
	if (!scaler->base) {
		scaler->is_empty = 1;
		vin_warn("%s scaler is empty\n", __func__);
	}
...
static int scaler_remove(int id)
{
	struct scaler_dev *scaler = glb_vipp[id];
	struct v4l2_subdev *sd = &scaler->subdev;
