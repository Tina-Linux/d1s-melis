...
static hal_spinlock_t vin_video_lock;

#define VIN_MAJOR_VERSION 1
#define VIN_MINOR_VERSION 0
#define VIN_RELEASE       0

#define VIN_VERSION (((VIN_MAJOR_VERSION) << 16) + ((VIN_MINOR_VERSION) << 8) + (VIN_RELEASE))

extern struct vin_core *vin_core_gbl[VIN_MAX_DEV];

void __vin_s_stream_handle(struct rt_work *work, void *work_data)
{
	int ret = 0;
...
static void __vin_pipeline_reset_handle(struct rt_work *work)
{
	unsigned int i;
	...
	if (!cap->pipe.sd[VIN_IND_SENSOR]->entity.use_count) {
		...
		rt_mutex_release(&cap->vdev.entity.graph_obj.mdev->graph_mutex);
		rt_mutex_release(&csi->reset_lock);
		return;
	}
		      ...
		      struct vin_frame *frame, struct vin_addr *paddr)
{
	...
	if (vb->vb2_queue->memory == VB2_MEMORY_USERPTR)
		paddr->y = __va_to_pa((u32)vb->planes[0].m.userptr);
	else if (vb->vb2_queue->memory == VB2_MEMORY_MMAP)
	...
	if (frame->fmt.memplanes == 1) {
		...
		if (frame->fmt.memplanes >= 2) {
			if (vb->vb2_queue->memory == VB2_MEMORY_USERPTR)
				paddr->cb = __va_to_pa((u32)vb->planes[1].m.userptr);
			else if (vb->vb2_queue->memory == VB2_MEMORY_MMAP)
		...
		if (frame->fmt.memplanes == 3) {
			if (vb->vb2_queue->memory == VB2_MEMORY_USERPTR)
				paddr->cr = __va_to_pa((u32)vb->planes[2].m.userptr);
			else if (vb->vb2_queue->memory == VB2_MEMORY_MMAP)
...
void vin_set_next_buf_addr(struct vin_core *vinc)
{
	...
	csic_buf_addr_fifo_en(vinc->vipp_sel, 0);
#else
...
static void buffer_queue(struct vb2_buffer *vb)
{
	...
	__cpsr = hal_spin_lock_irqsave(&vin_video_lock);
...
#endif
	hal_spin_unlock_irqrestore(&vin_video_lock, __cpsr);
}
...
static int __vin_set_fmt(struct vin_core *vinc, struct v4l2_format *f)
{
	...
	ffmt = vin_find_format(&f->fmt.pix_mp.pixelformat, NULL,
					VIN_FMT_ALL, -1, false);
	if (ffmt == NULL) {
		vin_err("vin does not support this pixelformat 0x%lx\n",
					...
					struct v4l2_format *f)
{
	...
	if (f->fmt.win.bitmap) {
		...
		clip = rt_calloc(1, sizeof(struct v4l2_clip) * osd->overlay_cnt * 2);
		if (clip == NULL) {
		...
		bitmap = rt_calloc(1, bitmap_size * pix_size);
		if (bitmap == NULL) {
			vin_err("%s - Alloc of bitmap buf failed\n", __func__);
			return -ENOMEM;
		}
		if (!memcpy(bitmap, f->fmt.win.bitmap,
		...
		clip = rt_calloc(1, sizeof(struct v4l2_clip) * f->fmt.win.clipcount * 2);
		if (clip == NULL) {
			vin_err("%s - Alloc of clip mask failed\n", __func__);
			return -ENOMEM;
		}
...
static int __osd_reg_setup(struct vin_core *vinc, struct vin_osd *osd)
{
	...
	osd_cfg = rt_calloc(1, sizeof(struct vipp_osd_config));
	if (osd_cfg == NULL) {
		vin_err("%s - Alloc of osd_cfg failed\n", __func__);
		return -ENOMEM;
	}

	para = rt_calloc(1, sizeof(struct vipp_osd_para_config));
	if (para == NULL) {
...
static int vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	if (vinc->large_image == 1) {
		...
		os_mem_alloc(&vinc->ptn_cfg.ptn_buf);
...
#else
	rt_mutex_take(&cap->vdev.entity.graph_obj.mdev->graph_mutex, RT_WAITING_FOREVER);
	vin_timer_init(cap->vinc);
	ret = vin_pipeline_call(cap->vinc, set_stream, &cap->pipe, cap->vinc->stream_idx);
	if (ret < 0)
			 ...
			 struct v4l2_streamparm *parms)
{
	...
	ret = v4l2_subdev_call(vinc->vid_cap.pipe.sd[VIN_IND_SENSOR], video, g_parm, parms);
	if (ret < 0)
	{
...
static unsigned int vin_poll(struct file *file, struct rt_pollreq *wait)
{
	struct vin_core *vinc = video_drvdata(file);
	struct vin_vid_cap *cap = &vinc->vid_cap;
	//software_break();
...
static const struct v4l2_ioctl_ops vin_ioctl_ops = {
	...
	.vidioc_enum_frameintervals = NULL,
};
...
int vin_init_video(struct v4l2_device *v4l2_dev, struct vin_vid_cap *cap)
{
	...
	rt_work_init(&cap->s_stream_task, __vin_s_stream_handle, NULL);
	...
	// This lock would be hold By vb_vidq(queue) and video_device object two.
	rt_mutex_init(&cap->lock, "vincapmtx", RT_IPC_FLAG_FIFO);
...
static void vin_capture_subdev_unregistered(struct v4l2_subdev *sd)
{
	...
	if (video_is_registered(&vinc->vid_cap.vdev)) {
		...
		rt_mutex_delete(&vinc->vid_cap.lock);
	}
