...
struct sensor_exp_gain grobal_exp_gain = {0, 0, 0, 0};
...
static struct regval_list sensor_1080p30_regs[] = {
	...
	{0x0342, 0x08},//hts = 2256
	{0x0343, 0xD0},

	{0x0340, 0x10},//vts = 4344
	{0x0341, 0xF8},
...
static int sensor_g_exp(struct v4l2_subdev *sd, __s32 *value)
{
	...
	*value = info->exp;
	sensor_dbg("sensor_get_exposure = %d", info->exp);
	return 0;
}
...
static int sensor_g_gain(struct v4l2_subdev *sd, __s32 *value)
{
	struct sensor_info *info = to_state(sd);
	*value = info->gain;
	sensor_dbg("sensor_get_gain = %d", info->gain);
	return 0;
}
...
static int sensor_s_pre_exp_gain(struct v4l2_subdev *sd,
			     struct sensor_exp_gain *exp_gain)
{
	struct sensor_info *info = to_state(sd);
	...
	info->exp = grobal_exp_gain.exp_val;
	info->gain = grobal_exp_gain.gain_val;
	return 0;
}
...
static int sensor_s_hflip(struct v4l2_subdev *sd, int enable)
{
	...
	sensor_dbg("into set sensor hfilp the value:%d ", enable);
			...
			struct sensor_fps *fps)
{
	...
	sensor_dbg("------start set fps = %d--------!!!!", fps->fps);
	if (fps->fps == 0)
		fps->fps = 30;
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	sensor_dbg("0x0016 0x%x", rdval);

	sensor_read(sd, 0x0017, &rdval);
	sensor_dbg("0x0017 0x%x", rdval);
...
static long sensor_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_VIN_SENSOR_EXP_GAIN:
		...
		break;
	case VIDIOC_VIN_SENSOR_P_EXP_GAIN:
		ret = sensor_s_pre_exp_gain(sd, (struct sensor_exp_gain *)arg);
		break;
...
static int sensor_reg_init(struct sensor_info *info)
{
	...
	if (grobal_exp_gain.exp_val && grobal_exp_gain.gain_val) {
...
static int sensor_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	sensor_dbg("%s on = %d, %d*%d fps: %d code: %x", __func__, enable,
...
device_initcall(imx386_mipi_init);
