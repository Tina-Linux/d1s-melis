/*
 * ekernel/components/avframework/v4l2/platform/sunxi-vin/vin-isp/sunxi_isp.c
 *
...
static hal_spinlock_t isp_spin_lock;
extern hal_spinlock_t vin_lock;

#ifdef CONFIG_ISP_LOAD_REG_FROM_BIN
...
#if defined CONFIG_D3D_LBC_MODE
#define D3D_RAW_LBC_MODE		16 /* 0 = lossless, 12 = 1.2x, 30 = 3x(< 3x)*/
#define D3D_K_LBC_MODE			20 /* 0 = lossless, 12 = 1.2x, 20 = 2x(< 2x)*/
#else
...
static void __isp_s_sensor_stby_handle(struct rt_work *work, void *work_data)
{
...
static int isp_3d_pingpong_alloc(struct isp_dev *isp)
{
	...
	ret = os_mem_alloc(&isp->d3d_pingpong[2]);
	if (ret < 0) {
	...
	isp->d3d_pingpong[1].size = roundup(isp->mf.width, 64) * isp->mf.height * 29 / 8;
#endif
	ret = os_mem_alloc(&isp->d3d_pingpong[0]);
	if (ret < 0) {
		vin_err("isp 3d pingpong buf0 requset failed!\n");
		return -ENOMEM;
	}
	ret = os_mem_alloc(&isp->d3d_pingpong[1]);
	if (ret < 0) {
...
{
	os_mem_free(&isp->d3d_pingpong[0]);
	os_mem_free(&isp->d3d_pingpong[1]);
#if defined CONFIG_SOC_SUN8IW19P1
#if defined CONFIG_D3D_LTF_EN
	os_mem_free(&isp->d3d_pingpong[2]);
#endif
#endif
}
...
static int isp_wdr_pingpong_alloc(struct isp_dev *isp)
{
	...
	ret = os_mem_alloc(&isp->wdr_pingpong[1]);
	if (ret < 0) {
		vin_err("isp wdr pingpong buf1 requset failed!\n");
		return -ENOMEM;
	}
#endif
	ret = os_mem_alloc(&isp->wdr_pingpong[0]);
	if (ret < 0) {
...
{
#if defined CONFIG_SOC_SUN8IW19P1
	os_mem_free(&isp->wdr_pingpong[0]);
#else
	os_mem_free(&isp->wdr_pingpong[0]);
	os_mem_free(&isp->wdr_pingpong[1]);
#endif
}
...
#ifdef CONFIG_ISP_LOAD_REG_FROM_BIN
int sunxi_isp_load_reg_from_bin(struct isp_dev *isp)
{
#define ISP_PATH "/mnt/E"
	int i = 0;
	int read_len = 0;
	char fdstr[50] = {0};
	FILE *file_fd = NULL;
	...
	for (i = 0; i < VIN_MAX_DEV; i++) {
		...
		vinc = vind->vinc[i];
		if ((vinc->isp_sel == isp->id) && vinc->vid_cap.pipe.sd[VIN_IND_SENSOR])
			break;
	}

	info = container_of(vinc->vid_cap.pipe.sd[VIN_IND_SENSOR], struct sensor_info, sd);
	if (info) {
		sprintf(fdstr, "%s/isp%d_%d_%d_%d_reg_saved.bin", ISP_PATH, isp->id,
					info->current_wins->width_input,
					info->current_wins->height_input,
					info->current_wins->fps_fixed);
	} else {
		vin_warn("warning: get sensor_info failed! use default bin file.\n");
		sprintf(fdstr, "%s/isp%d_1920_1080_60_reg_saved.bin", ISP_PATH, isp->id);
	}

	file_fd = fopen(fdstr, "rb");

	if (file_fd == NULL) {
		vin_warn("open %s failed!!!\n", fdstr);
	} else {
		fread(&read_len, sizeof(int), 1, file_fd);
		if (ISP_LOAD_DRAM_SIZE != read_len) {
			vin_warn("%s size %d != isp_tbl size %d!\n", fdstr, read_len, ISP_LOAD_DRAM_SIZE);
		} else {
			fread(&isp->load_shadow[0], ISP_LOAD_DRAM_SIZE, 1, file_fd);
#if defined CONFIG_D3D
			...
			if (!strncmp("C2398_mipi", vinc->vid_cap.pipe.sd[VIN_IND_SENSOR]->name, 6)) {
				*((int *)isp->isp_load.vir_addr + 0x340/4) = 0x1f431f43;
				*((int *)isp->isp_load.vir_addr + 0x344/4) = 0x1f431f43;
				isp->bind_sensor = 1;
			}
			vin_log(VIN_LOG_ISP, "get isp_tbl from %s success!!!\n", fdstr);
		}
	}
...
static int sunxi_isp_subdev_s_stream(struct v4l2_subdev *sd, int enable)
{
...
#if defined CONFIG_ARCH_SUN8IW16P1 || defined CONFIG_SOC_SUN8IW19P1 || defined CONFIG_ARCH_SUN50IW10P1 || defined CONFIG_SOC_SUN20IW1P1
	...
	if (enable) {
		...
		if (isp->load_flag) {
			...
			vin_warn("We should load data to isp before open it, take care the first frame picture now!\n");
		}
...
#ifdef CONFIG_ISP_LOAD_REG_FROM_BIN
		sunxi_isp_load_reg_from_bin(isp);
#endif
				    ...
				    struct v4l2_subdev_format *fmt)
{
	...
	rt_mutex_take(&isp->subdev_lock, RT_WAITING_FOREVER);
	fmt->format = isp->mf;
	rt_mutex_release(&isp->subdev_lock);
	return 0;
}
...
int sunxi_isp_subdev_init(struct v4l2_subdev *sd, u32 val)
{
	...
	if (val) {
		...
		if (!isp->have_init) {
			memcpy(isp->isp_load.vir_addr, &isp_default_reg[0], ISP_LOAD_REG_SIZE);
			memcpy(isp->isp_load.vir_addr + ISP_LOAD_REG_SIZE + ISP_FE_TBL_SIZE + ISP_BAYER_TABLE_SIZE + ISP_RGB_TABLE_SIZE, &isp_default_yuv_table[0], ISP_YUV_TABLE_SIZE);
...
static int __isp_set_load_reg(struct v4l2_subdev *sd, struct isp_table_reg_map *reg)
{
	...
	isp->load_flag = 1;
	memcpy(&isp->load_shadow[0], reg->addr, reg->size);
	return 0;
}
...
static int __isp_set_table1_map(struct v4l2_subdev *sd, struct isp_table_reg_map *tbl)
{
	...
	if (!isp->use_isp)
		return 0;
	ret = memcpy(&isp->load_shadow[0] + ISP_LOAD_REG_SIZE, tbl->addr, tbl->size);
	if (!ret) {
...
static int __isp_set_table2_map(struct v4l2_subdev *sd, struct isp_table_reg_map *tbl)
{
	...
	if (!isp->use_isp)
		return 0;
	ret = memcpy(&isp->load_shadow[0] + ISP_LOAD_REG_SIZE + ISP_TABLE_MAPPING1_SIZE, tbl->addr, tbl->size);
	if (!ret) {
...
void sunxi_isp_reset(struct isp_dev *isp)
{
	...
	if (isp->sensor_lp_mode)
		rt_work_submit(&isp->s_sensor_stby_task, 0);
}
...
int __isp_init_subdev(struct isp_dev *isp)
{
	...
	rt_mutex_init(&isp->subdev_lock, "ispsubmtx", RT_IPC_FLAG_FIFO);
...
static hal_irqreturn_t isp_isr(void *priv)
{
	...
	if (bsp_isp_get_irq_status(isp->id, PARA_LOAD_PD)) {
		...
		if (isp->ptn_type) {
			hal_spin_unlock_irqrestore(&isp_spin_lock, __cpsr);
			return HAL_IRQ_OK;
		}
		bsp_isp_set_para_ready(isp->id, PARA_NOT_READY);
		if (isp->load_flag) {
			memcpy(isp->isp_load.vir_addr, &isp->load_shadow[0], ISP_LOAD_DRAM_SIZE);
			hal_dcache_clean((unsigned long)isp->isp_load.vir_addr, ISP_LOAD_DRAM_SIZE);
		}

		if (isp->bind_sensor && !isp->isp_frame_number) {
			*((int *)isp->isp_load.vir_addr + 0x340/4) = 0x1f001f00;
			*((int *)isp->isp_load.vir_addr + 0x344/4) = 0x1f001f00;
		}
		isp->isp_ob.set_cnt++;
		load_val = bsp_isp_load_update_flag(isp->id);
		if (isp->wdr_mode == ISP_DOL_WDR_MODE)
...
#if defined CONFIG_ARCH_SUN8IW12P1 || defined CONFIG_ARCH_SUN8IW17P1 || defined CONFIG_SOC_SUN8IW19P1
		isp_3d_pingpong_update(isp);
#endif
		bsp_isp_set_para_ready(isp->id, PARA_READY);
	}
...
static int isp_probe(int id)
{
	struct isp_dev *isp = NULL;
	int ret = 0;

	isp = rt_calloc(1, sizeof(struct isp_dev));
	if (!isp) {
		ret = -ENOMEM;
		goto ekzalloc;
	}
	...
	rt_work_init(&isp->s_sensor_stby_task, __isp_s_sensor_stby_handle, NULL);
	glb_isp[isp->id] = isp;
...
static int isp_remove(int id)
{
	struct isp_dev *isp = glb_isp[id];
	struct v4l2_subdev *sd = &isp->subdev;
