		       ...
		       union v4l2_ctrl_ptr ptr)
{
	...
	if (ctrl->is_ptr && !ctrl->is_string)
		return !memcpy(c->ptr, ptr.p, c->size) ?
		       -EFAULT : 0;
	...
	case V4L2_CTRL_TYPE_STRING:
		...
		return !memcpy(c->string, ptr.p_char, len + 1) ?
		       -EFAULT : 0;
	case V4L2_CTRL_TYPE_INTEGER64:
		c->value64 = *ptr.p_s64;
		break;
		       ...
		       union v4l2_ctrl_ptr ptr)
{
	...
	if (ctrl->is_ptr && !ctrl->is_string) {
		...
		ret = !memcpy(ptr.p, c->ptr, c->size) ? -EFAULT : 0;
	...
	case V4L2_CTRL_TYPE_STRING:
		...
		if (size > ctrl->maximum + 1)
			size = ctrl->maximum + 1;
		ret = !memcpy(ptr.p_char, c->string, size) ? -EFAULT : 0;
		if (!ret) {
...
int v4l2_ctrl_handler_init_class(struct v4l2_ctrl_handler *hdl,
				 unsigned nr_of_controls_hint, const char *name)
{
	hdl->lock = &hdl->_lock;
	rt_mutex_init(hdl->lock, "v4l2_ctrlmtx", RT_IPC_FLAG_FIFO);
	INIT_LIST_HEAD(&hdl->ctrls);
	INIT_LIST_HEAD(&hdl->ctrl_refs);
	hdl->nr_of_buckets = 1 + nr_of_controls_hint / 8;
	hdl->buckets = rt_calloc(hdl->nr_of_buckets, sizeof(hdl->buckets[0]));
...
//	rt_mutex_take(hdl->lock, RT_WAITING_FOREVER);
	/* Free all nodes */
			   ...
			   struct v4l2_ctrl *ctrl)
{
	...
	new_ref  = rt_malloc(sizeof(*new_ref));
	if (!new_ref)
		return handler_set_err(hdl, -ENOMEM);

	memset(new_ref, 0x00, sizeof(*new_ref));
			...
			const s64 *qmenu_int, void *priv)
{
	...
	ctrl  = rt_malloc(sizeof(*ctrl) + sz_extra);
	if (ctrl == NULL) {
		handler_set_err(hdl, -ENOMEM);
		return NULL;
	}

	memset(ctrl, 0x00, sizeof(*ctrl) + sz_extra);
			...
			u32 id, u8 _max, u64 mask, u8 _def)
{
	...
	if (type == V4L2_CTRL_TYPE_MENU)
		...
		qmenu_int = v4l2_ctrl_get_int_menu(id, (u32 *)&qmenu_int_len);
			  ...
			  bool (*filter)(const struct v4l2_ctrl *ctrl))
{
	...
	if (hdl->error)
		return hdl->error;
//	rt_mutex_take(add->lock, RT_WAITING_FOREVER);
...
{
	struct v4l2_query_ext_ctrl qec = { .id = qc->id, .type = 0 };
	int rc;
...
int v4l2_g_ext_ctrls(struct v4l2_ctrl_handler *hdl, struct v4l2_ext_controls *cs)
{
	...
	if (cs->count > ARRAY_SIZE(helper)) {
		helpers = rt_calloc(cs->count, sizeof(helper[0]));
		if (helpers == NULL)
			return -ENOMEM;
	}
			  ...
			  struct v4l2_ctrl_helper *helpers, bool set)
{
	...
	for (i = 0; i < cs->count; i++) {
		...
		if (ctrl->type == V4L2_CTRL_TYPE_INTEGER64)
			p_new.p_s64 = (void *)&cs->controls[i].value64;
		else
			p_new.p_s32 = (void *)&cs->controls[i].value;
...
{
	struct v4l2_ctrl *ctrl = v4l2_ctrl_find(hdl, control->id);
	struct v4l2_ext_control c = { .id = control->id, .size = 0 };
	int ret;
...
unsigned int v4l2_ctrl_poll(struct file *file, struct rt_pollreq *req)
{
