					  ...
					  int idx_max)
{
	...
	ent_enum->bmap = rt_calloc(idx_max / BITS_PER_LONG, sizeof(long));
	if (!ent_enum->bmap)
		return -ENOMEM;
...
//	rt_mutex_take(&link->graph_obj.mdev->graph_mutex, RT_WAITING_FOREVER);
	ret = __media_entity_setup_link(link, flags);
//	rt_mutex_release(&link->graph_obj.mdev->graph_mutex);
