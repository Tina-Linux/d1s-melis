...
#if 1
struct vb2_mem_ops {
	void		*(*alloc)(struct rt_device *dev, unsigned long attrs,
				  unsigned long size,
	...
	void		*(*get_userptr)(struct rt_device *dev, unsigned long vaddr,
					unsigned long size,
					enum dma_data_direction dma_dir);
	...
	void		*(*attach_dmabuf)(struct rt_device *dev,
					  struct dma_buf *dbuf,
					  unsigned long size,
					  enum dma_data_direction dma_dir);
	...
	int		(*mmap)(void *buf_priv, struct map_struct *map);
};
#endif
...
struct vb2_buffer {
	...
	unsigned int		memory; //enum vb2_memory
...
struct vb2_queue {
	unsigned int			type;   //enum v4l2_buf_type
	unsigned int			io_modes;   //enum vb2_io_modes
	struct rt_device		*dev;
	unsigned long			dma_attrs;  //DMA_ATTR_NO_KERNEL_MAPPING
	...
	void				*drv_priv;
	unsigned int			buf_struct_size;    //sizeof(vin_buffer)
	u32				timestamp_flags;    //V4L2_BUF_FLAG_TIMESTAMP_MONOTONIC
	gfp_t				gfp_flags;  //GFP_KERNEL
	u32				min_buffers_needed;

	/* private: internal use only */
	struct rt_mutex			mmap_lock;
	unsigned int			memory; //enum vb2_memory
	struct vb2_buffer		*bufs[VB2_MAX_FRAME];
	unsigned int			num_buffers;

	struct list_head		queued_list;    //struct vb2_buffer
	unsigned int			queued_count;

	atomic_t			owned_by_drv_count;
	struct list_head		done_list;
	rt_ubase_t			done_lock;
	rt_wqueue_t			done_wq;

	struct rt_device		*alloc_devs[VB2_MAX_PLANES];
