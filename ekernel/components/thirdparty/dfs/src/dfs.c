...
void fd_uninstall(struct dfs_fd *d, int fd)
{
    struct dfs_fdtable *fdt;
    fd -= DFS_FD_OFFSET;
    ...
    /* uinstall this fd entry */

    fdt = dfs_fdtable_get();

    if (fd < (int)fdt->maxfd && fdt->fds[fd] == d)
    {
        if (d->dup_count == 0)
        {
            rt_free(d);
...
int dupfd(struct dfs_fd *d, int startfd)
{
    struct dfs_fdtable *fdt;
    int idx;
    ...
    if (idx == fdt->maxfd && fdt->maxfd < DFS_FD_MAX)
    {
        ...
        fds = (struct dfs_fd **)rt_realloc(fdt->fds, cnt * sizeof(struct dfs_fd *));
        if (fds == NULL) goto __copy; /* return fdt->maxfd */
...
__copy:
    ...
    return idx != fdt->maxfd ? idx + DFS_FD_OFFSET : -1;

}

int dupfd2(struct dfs_fd *d, int newfd)
{
    struct dfs_fdtable *fdt;
    struct dfs_fd *new_d;
    int idx;
...
#include <finsh.h>
int list_fd(int argc, const char **argv)
{
    (void)argc;
    (void)argv;
    int index;
    ...
    rt_kprintf("fd type    ref dup magic  path\n");
    rt_kprintf("-- ------  --- --- ----- ------\n");
