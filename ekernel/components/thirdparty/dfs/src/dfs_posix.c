 ...
 */
int dupfd(struct dfs_fd *d, int fd);
int dupfd2(struct dfs_fd *d, int newfd);
void fd_uninstall(struct dfs_fd *d, int fd);
int dfs_file_truncate(struct dfs_fd *d, off_t length);
...
{
    int ret;
    ret = dfs_mount(source, target, filesystemtype, mountflags, data);
    if (ret)
    {
        rt_kprintf("mount errno: %d!\n", rt_get_errno());
    }
    return ret;
}
