...
static inline void memoryVersionInfo(void)
{
    printf("\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> memoryInfo V1.0<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n");
}
...
/*funciton begin*/
int sunxi_ion_alloc_open(MemParameter param)
{
    ...
    g_ion_alloc_context->memParam.iommuFlag = param.iommuFlag;
#endif
...
#if 1
    if(g_ion_alloc_context->memParam.iommuFlag){
	    close(g_ion_alloc_context->cedar_fd);
        g_ion_alloc_context->cedar_fd = 0;
    }
#endif
...
int sunxi_ion_alloc_palloc(MemBufInfo* bufferInfo,int /*bufferNum*/ number)
{
    if(NULL == bufferInfo || !number)
        return -1;
    ...
    for(index = 0;index < number;index++)
{
    ...
    mbufferInfo = bufferInfo +index;
    bufferSize = mbufferInfo->bufferSize;
    ...
    alloc_data.aw_len = (size_t)bufferSize;
    alloc_data.aw_align = ION_ALLOC_ALIGN ;

#if 1
    if(g_ion_alloc_context->memParam.iommuFlag)
        //if use iommu,we just don't alloc cache bufer now.
    ...
    alloc_buffer->size = bufferSize;
    alloc_buffer->fd_data.handle = fd_data.handle;
    alloc_buffer->fd_data.aw_fd = fd_data.aw_fd;

    mbufferInfo->dataFd  = alloc_buffer->fd_data.aw_fd;
    mbufferInfo->phyAddr = (void*)alloc_buffer->phy;
    mbufferInfo->virAddr = (void*)alloc_buffer->vir;

	logd("===palloc index %d ,map_addr %p, fd %d===", index,mbufferInfo[index]->virAddr, mbufferInfo[index]->dataF);

    aw_mem_list_add_tail(&alloc_buffer->i_list, &g_ion_alloc_context->list);

#if DEBUG_ION_REF==1
    cdx_use_mem += bufferSize;
    int i = 0;
    for (i = 0; i < ION_BUF_LEN; i++) {
        if (ion_buf_nodes_test[i].addr == 0
				&& ion_buf_nodes_test[i].size == 0) {
            ion_buf_nodes_test[i].addr = addr_vir;
            ion_buf_nodes_test[i].size = bufferSize;
            break;
        }
    }
...
void sunxi_ion_alloc_pfree(MemBufInfo* bufferInfo,int number)
{
    int ret,index;
...
for(index = 0;index < number;index++)
{
    ...
    mbufferInfo = bufferInfo+index;
    unsigned long addr_vir = (unsigned long)mbufferInfo->virAddr;
    if (addr_vir == 0){
		logw("current buffer index %d maybe alloc fail!\n",index);
		continue;
	}
    ...
    {
        if (tmp->vir == addr_vir) {
            logd("ion_alloc_free item phy =0x%lx vir =0x%lx, size =%d\n", \
                tmp->phy, tmp->vir, tmp->size);
            /*unmap user space*/
...
struct AwMemOpsS _allocionMemOpsS =
{
