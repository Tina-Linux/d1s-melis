...
static ERRORTYPE InitDemux2Adec2AOData(void)
{
    ...
    pDemux2Adec2AOData->mDetectInputThreadId = -1;
...
static ERRORTYPE loadConfigPara(SAMPLE_DEMUX2ADEC2AO_S *pSampleData, const char *conf_path)
{
    ...
    strcpy(pSampleData->srcFile, "/mnt/E/test.aac");
...
static ERRORTYPE createAOChn(SAMPLE_DEMUX2ADEC2AO_S *pSampleData)
{
    ...
    if(ret != SUCCESS)
    {
        aloge("fatal error! aodev[%d] enable fail!", pSampleData->mAODevId);
    }
    //create ao channel
    BOOL bSuccessFlag = FALSE;
...
static void handle_exit(void)
{
    ...
    if(NULL != pDemux2Adec2AOData)
    {
        cdx_sem_up(&pDemux2Adec2AOData->mWaitEof);
    }
}

static void* SampleDetectInputThread(void* pThreadData)
{
    SAMPLE_DEMUX2ADEC2AO_S* pDemux2Adec2AOData = (SAMPLE_DEMUX2ADEC2AO_S*) pThreadData;
    int ret;
    ...
    prctl(PR_SET_NAME, (unsigned long)"sample_demux2adec2ao_DetectInput", 0, 0, 0);
    while(1)
    {
        ...
        {
            unsigned int val = cdx_sem_get_val(&pDemux2Adec2AOData->mWaitEof);
            if(val > 0)
            {
                alogd("val[%d], main thread want to exit?", val);
            }
        }
...
int main_sample_demux2adec2ao(int argc, char** argv)
{
    int ret = 0;
    void *pValue = NULL;
    ...
    ret = pthread_create(&pDemux2Adec2AOData->mDetectInputThreadId, NULL, SampleDetectInputThread, pDemux2Adec2AOData);
    if (ret != 0)
    {
    ...
    //usleep(100*1000);
    alogd("create detect input thread:[0x%lx]", pDemux2Adec2AOData->mDetectInputThreadId);
    ...
    {
        aloge("ERROR: cannot open src file[%s]", pDemux2Adec2AOData->srcFile);
        ret = -1;
        goto _exit2;
    }
    else
    ...
    AW_MPI_AO_SetDevVolume(pDemux2Adec2AOData->mAODevId, 100);
   ...
   //exit mpp system
    ...
    ret = pthread_cancel(pDemux2Adec2AOData->mDetectInputThreadId);
    alogd("post cancel signal to pthread[0x%lx] join, ret[0x%x]", pDemux2Adec2AOData->mDetectInputThreadId, ret);
    ret = pthread_join(pDemux2Adec2AOData->mDetectInputThreadId, &pValue);
    alogd("pthread[0x%lx] join, ret[0x%x], pValue[%p]", pDemux2Adec2AOData->mDetectInputThreadId, ret, pValue);
...
_exit1:
    cdx_sem_deinit(&pDemux2Adec2AOData->mWaitEof);
_exit0:
    if(pDemux2Adec2AOData)
    {
        free(pDemux2Adec2AOData);
        pDemux2Adec2AOData = NULL;
    }
    printf("sample_demux2adec2ao exit[%d]!\n", ret);
    return ret;

}
FINSH_FUNCTION_EXPORT_ALIAS(main_sample_demux2adec2ao, __cmd_sample_demux2adec2ao, test mpp demux2adec2ao);
