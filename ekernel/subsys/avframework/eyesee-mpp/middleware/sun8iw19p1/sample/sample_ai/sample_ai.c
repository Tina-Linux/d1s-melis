...
//                pConfig->mSampleRate, pConfig->mBitWidth, pConfig->mChannelCnt, pConfig->mFrameSize, pConfig->mCapDuraSec);
    ...
    alogd("para: filePath[%s], mSampleRate[%d], mBitWidth[%d], mChannelCnt[%d], mFrameSize[%d], mCapTime[%d]s, ai_gain[%d]",
        pConfig->mPcmFilePath, pConfig->mSampleRate, pConfig->mBitWidth, pConfig->mChannelCnt, pConfig->mFrameSize, 
        pConfig->mCapDuraSec, pConfig->ai_gain);
    return ret;
}
...
int main_sample_ai(int argc, char *argv[])
{
    int result = 0;
...
FINSH_FUNCTION_EXPORT_ALIAS(main_sample_ai, __cmd_sample_ai, test mpp ai);
