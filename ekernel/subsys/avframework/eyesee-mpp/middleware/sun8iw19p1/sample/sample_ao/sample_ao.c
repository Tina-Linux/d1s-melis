...
static ERRORTYPE loadSampleAOConfig(SampleAOConfig *pConfig, const char *conf_path)
{
    ...
    alogd("param: pcmFilePath[%s], sampleRate[%d], channelCount[%d], bitWidth[%d], frameSize[%d]",
        pConfig->mPcmFilePath, pConfig->mSampleRate, pConfig->mChannelCnt, pConfig->mBitWidth, pConfig->mFrameSize);
    return SUCCESS;
}
...
int main_sample_ao(int argc, char *argv[])
{
    int result = 0;
    ...
    AW_MPI_AO_SetDevVolume(stContext.mAODev, 100);
...
FINSH_FUNCTION_EXPORT_ALIAS(main_sample_ao, __cmd_sample_ao, test mpp ao);
