    ...
    : mVideoLayerId(hlay)
{
    ...
    {
        alogw("vdec already rotate [%ld] clock-wise", nVdecInitRotation);
    }
    ...
    {
        alogw("bufWidth[%ld]!=display_width[%ld]", mWidth, mCropWidth);
    }
    if(mHeight != mCropHeight)
    {
        alogw("bufHeight[%ld]!=display_height[%ld]", mHeight, mCropHeight);
    }
