 ...
 */
static inline void __list_add(struct list_entry *new, struct list_entry *prev, struct list_entry *next)
{
    next->prev = new;
    new->next = next;
    new->prev = prev;
 ...
 */
static inline void list_add(struct list_entry *new, struct list_entry *head) { __list_add(new, head, head->next); }
 ...
 */
static inline void list_add_tail(struct list_entry *new, struct list_entry *head) { __list_add(new, head->prev, head); }
 ...
 */
static inline void __list_del_entry(struct list_entry *entry) { __list_del(entry->prev, entry->next); }
 ...
 */
static inline void list_replace(struct list_entry *old, struct list_entry *new)
{
    new->next = old->next;
    new->next->prev = new;
    new->prev = old->prev;
...
static inline void list_replace_init(struct list_entry *old, struct list_entry *new)
{
 ...
 */
static inline void list_move_tail(struct list_entry *list, struct list_entry *head)
{
    __list_del_entry(list);
 ...
 */
static inline int list_is_last(const struct list_entry *list, const struct list_entry *head)
{
 ...
 */
static inline int list_empty(const struct list_entry *head) { return head->next == head; }
 ...
 */
static inline int list_empty_careful(const struct list_entry *head)
{
 ...
 * @head: the head of the list
 */
static inline void list_rotate_left(struct list_entry *head)
{
 ...
 * @head: the list to test.
 */
static inline int list_is_singular(const struct list_entry *head)
{
    return !list_empty(head) && (head->next == head->prev);
}

static inline void __list_cut_position(struct list_entry *list, struct list_entry *head, struct list_entry *entry)
{
    struct list_entry *new_first = entry->next;
    list->next = head->next;
    list->next->prev = list;
    list->prev = entry;
    entry->next = list;
 ...
 *
 */
static inline void list_cut_position(struct list_entry *list, struct list_entry *head, struct list_entry *entry)
{
...
static inline void __list_splice(const struct list_entry *list, struct list_entry *prev, struct list_entry *next)
{
 ...
 */
static inline void list_splice(const struct list_entry *list, struct list_entry *head)
{
 ...
 * @head: the place to add it in the first list.
 */
static inline void list_splice_tail(struct list_entry *list, struct list_entry *head)
{
 ...
 */
static inline void list_splice_init(struct list_entry *list, struct list_entry *head)
{
    if (!list_empty(list)) {
 ...
 * The list at @list is reinitialised
 */
static inline void list_splice_tail_init(struct list_entry *list, struct list_entry *head)
{
    if (!list_empty(list)) {
...
#define HLIST_HEAD(name) struct hlist_entry name = {.first = NULL}
...
static inline int hlist_empty(const struct hlist_entry *h) { return !h->first; }
...
static inline void hlist_add_head(struct hlist_node *n, struct hlist_entry *h)
{
