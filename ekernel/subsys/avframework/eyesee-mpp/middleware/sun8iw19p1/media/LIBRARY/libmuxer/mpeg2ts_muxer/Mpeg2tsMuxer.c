...
static uint32_t *av_crc_get_table(AVCRCId crc_id)
{
		...
		if (!ts_crc_table[crc_id][FF_ARRAY_ELEMS(ts_crc_table[crc_id])-1]) {
			if (av_crc_init((uint32_t*)ts_crc_table[crc_id],
...
#endif
    return (uint32_t*)ts_crc_table[crc_id];
}
...
static void mpegts_write_pes(AVFormatContext *s, AVStream *st,
                             ...
                             int64_t pts, int64_t dts,int32_t idx)
{
    MpegTSWriteStream *ts_st = st->priv_data;
    MpegTSWrite *ts = s->priv_data;
    ...
    if(CODEC_TYPE_TEXT != idx)// fix bug that the play time shown when replay is not continuous.
    {
	...
	if(st->codec.codec_id == CODEC_ID_H264) {
		...
		if(s->output_buffer_mode == OUTPUT_M3U8_FILE) {
			...
			if(s->keyframe_num >= MAX_KEY_FRAME_NUM) {
				...
				s->current_segment++;
				sprintf((char*)s->filename, "%s%ld.ts", TS_FILE_SAVED_PATH, s->current_segment);
				...
				if(s->current_segment >= MAX_SEGMENT_IN_M3U8) {
					...
					if((int)s->first_segment - 3 >= 0) {
						sprintf((char*)s->filename, "%s%lu.ts", TS_FILE_SAVED_PATH, s->first_segment - 3);
						remove(s->filename);
					}
					s->first_segment ++;
				}
...
//		}
    ...
    mpegts_write_pes(s, st, s->pes_buffer, size, pkt->pts, 0, pkt->stream_index);
    return 0;
}
