...
struct media_entity_param *media_get_entity_by_name
				(struct media_device *media, const char *name)
{
	unsigned int i;

	for (i = 0; i < media->entities_count; ++i) {
		struct media_entity_param *entity = &media->entities[i];
...
struct media_entity_param *media_get_entity_by_id(struct media_device *media,
					    __u32 id)
{
	unsigned int i;
...
struct media_entity_param *media_pipeline_get_head(struct media_entity_param *me)
{
...
int media_video_to_isp_id(struct media_entity_param *me)
{
	struct media_entity_pad *pad = &me->pads[0];
	int id = -1;
...
int media_video_to_csi_id(struct media_entity_param *me)
{
	struct media_entity_pad *pad = &me->pads[0];
	int id = -1;
...
static struct media_entity_link *media_entity_add_link
						(struct media_entity_param *entity)
{
...
static int media_enum_links(struct media_device *media, int verbose)
{
	...
	for (id = 1; id <= media->entities_count; id++) {
		struct media_entity_param *entity = &media->entities[id - 1];
		...
		for (i = 0; i < entity->info.links; ++i) {
			...
			struct media_entity_param *source;
			struct media_entity_param *sink;
			...
			if (source == NULL || sink == NULL) {
				ISP_WARN("WARNING entity %lu link %u from %lu/%u "
					"to %lu/%u is invalid!\n", id, i,
...
static int media_enum_entities(struct media_device *media)
{
	...
	for (id = 0; ; id = entity->info.id) {
		...
		memset(entity->pads, 0x00, entity->info.pads * sizeof(*entity->pads));
		memset(entity->links, 0x00, entity->max_links * sizeof(*entity->links));
		...
		strcpy(sysname, "vin_video");
		if (!strncmp(sysname, entity->info.name, strlen(sysname)))
		{
#if 1
			p = strrchr(entity->info.name, '_');
			sprintf(entity->devname, "/dev/%s", p + 1);
#else
			sprintf(devname, "/dev/video%ld", entity->info.v4l.major - VIDEO_MAJOR);
			strcpy(entity->devname, devname);
#endif
			ISP_DEV_LOG(ISP_LOG_SUBDEV, "entity name is %s, node is video%ld\n",
					entity->info.name, entity->info.v4l.major - VIDEO_MAJOR);
			/*
			 *printf("[DBG]: %s line %d, entity name %s, devname %s, pads %d, maxlinks %d.\n", \
			 *        __func__, __LINE__, entity->info.name, entity->devname, entity->info.pads, entity->max_links);
			 */
			continue;
		}

		ISP_DEV_LOG(ISP_LOG_SUBDEV, "entity name is %s, node is v4l-subdev%ld\n",
				entity->info.name, entity->info.v4l.major - VIDEO_MAJOR);
		sprintf(devname, "/dev/v4l-subdev%ld", entity->info.v4l.major - VIDEO_MAJOR);
		strcpy(entity->devname, devname);
		/*
		 *printf("[DBG]: %s line %d, entity name %s, devname %s, pads %d, links %d.\n", \
		 *        __func__, __LINE__, entity->info.name, entity->devname, entity->info.pads, entity->max_links);
		 */
#if 0
		sprintf(sysname, "/sys/dev/char/%lu:%lu", entity->info.v4l.major,
			entity->info.v4l.minor);
		ret = readlink(entity->info.name, target, sizeof(target));
		if (ret < 0)
			continue;
...
struct media_device *media_open(const char *name, int verbose)
{
	...
	if (verbose) {
		...
		for(i = 0; i < media->entities_count; i++)
			ISP_PRINT("Entities %d is %s, type is %ld!\n", i+1,
