...
int Mpeg2tsMuxerWriteVos(void *handle, uint8_t *vosData, unsigned int vosLen, unsigned int idx)
{
...
int Mpeg2tsMuxerWriteHeader(void *handle)
{
	AVFormatContext *Mpeg2tsMuxerCtx = (AVFormatContext *)handle;
    int8_t *pCache = NULL;
...
int Mpeg2tsMuxerWriteTrailer(void *handle)
{
	AVFormatContext *Mpeg2tsMuxerCtx = (AVFormatContext *)handle;
...
int Mpeg2tsMuxerWritePacket(void *handle, void *pkt)
{
...
int Mpeg2tsMuxerIoctrl(void *handle, unsigned int uCmd, unsigned int uParam, void *pParam2)
{
	AVFormatContext *Mpeg2tsMuxerCtx = (AVFormatContext *)handle;
	...
	case SETAVPARA:	
		...
		if(pMediaInf->nWidth && pMediaInf->nWidth) {
			Mpeg2tsMuxerCtx->streams[Mpeg2tsMuxerCtx->nb_streams]->codec.height = pMediaInf->nHeight;
			Mpeg2tsMuxerCtx->streams[Mpeg2tsMuxerCtx->nb_streams]->codec.width  = pMediaInf->nWidth;
			Mpeg2tsMuxerCtx->streams[Mpeg2tsMuxerCtx->nb_streams]->codec.frame_rate = pMediaInf->uVideoFrmRate;
			...
			Mpeg2tsMuxerCtx->streams[Mpeg2tsMuxerCtx->nb_streams]->codec.time_base.den = pMediaInf->uVideoFrmRate;
	...
	case SETCACHEFD:            // path case
    {
		...
		if(Mpeg2tsMuxerCtx->output_buffer_mode == OUTPUT_M3U8_FILE) {
			sprintf((char*)Mpeg2tsMuxerCtx->filename, "%s%ld.ts", TS_FILE_SAVED_PATH, Mpeg2tsMuxerCtx->current_segment);
...
//			}
    ...
    case SETCACHEFD2:       // fd case, current case
    {
        ...
        if(Mpeg2tsMuxerCtx->mFallocateLen > 0)
        {
            ...
            {
                aloge("fatal error! Failed to fallocate size %ld, fd[%d](%s)", Mpeg2tsMuxerCtx->mFallocateLen, Mpeg2tsMuxerCtx->pb_cache->fd_desc.fd, strerror(errno));
            }
        }
        Mpeg2tsMuxerCtx->output_buffer_mode = OUTPUT_TS_FILE;
        break;
...
int Mpeg2tsMuxerClose(void *handle)
{
	AVFormatContext *Mpeg2tsMuxerCtx = (AVFormatContext *)handle;
	MpegTSWrite *ts = Mpeg2tsMuxerCtx->priv_data;
	AVStream *st;
