...
typedef struct CLOCKDATATYPE {
    ...
    int64_t                           WallTimeBase;   // record cedarx clock pts as cedarxClockTime base. start_to_play pts in cedarxClock.
    ...
    COMP_TIME_CONFIG_TIMESTAMPTYPE       sMinStartTime;  //min(audio, video), select the smallest time as stream start time.
    int     nDstRatio;      //CedarX Play speed vps value. default is 0, but can be set to other value to support variable speed play. 40~-100.
    ...
    int64_t port_start_pts[MAX_CLOCK_PORTS];  //video/audio start pts, unit:us
}CLOCKDATATYPE;
...
ERRORTYPE ClockComponentInit(PARAM_IN COMP_HANDLETYPE hComponent)
{
    ...
    alogd("create clock comp threadId:0x%lx", pClockData->thread_id);
    EXIT: return eError;
}
