...
//    pUvcInputData->sPortParam.nPorts++;
    ...
    if(pthread_create(&pUvcInputData->thread_id, NULL, Uvc_ComponentThread, pUvcInputData))
    {
        aloge("creat Uvc_ComponentThread fail!");
        eError = ERR_UVC_NOMEM;
        return eError;
    }
    alogw("UvcVirvi_Component had Init!!! threadId[0x%lx]", pUvcInputData->thread_id);
