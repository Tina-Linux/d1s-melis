...
//            }
	...
	if (pSinkInfo->pWriter == NULL) 
    {
        ...
        if (FALSE == pSinkInfo->nCallbackOutFlag) 
        {
            ...
            alogd("FileDurationPolicy[0x%x], use fsWriteMode[%d], simpleCacheSize[%d]KB", pSinkInfo->mFileDurationPolicy, pSinkInfo->mFsWriteMode, pSinkInfo->mFsSimpleCacheSize/1024);
...
//                    && (pRecSink->mLoopDuration + NOTIFY_NEEDNEXTFD_IN_ADVANCE) >= pRecSink->mCurFileEndTm) 
                ...
                {
                    alogd("Need set next fd. SinkInfo[%p], videosize[%dx%d], LoopDuration[%lld]ms, curFileEndTm[%lld]ms", 
...
//	}
    ...
    alogd("create RecRenderSink threadId[0x%lx]", pThiz->mThreadId);
    return SUCCESS;
_err5:
    if(!list_empty(&pThiz->mIdleRSPacketList))
    {
