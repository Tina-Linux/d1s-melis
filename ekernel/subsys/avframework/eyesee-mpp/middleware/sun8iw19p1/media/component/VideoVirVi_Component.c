 ...
 ******************************************************************************
  ...
  Created       : 2016/05/09
 *  Last Modified:  2020-06-02 15:38:06
...
ERRORTYPE VideoViSetViDevAttr(PARAM_IN COMP_HANDLETYPE hComponent, VI_ATTR_S *pstViAttr)
{
    ...
    alogd("fps %d nbufs %d", pVideoViData->mViAttr.fps, pVideoViData->mViAttr.nbufs);
...
ERRORTYPE VideoViEmptyThisBuffer(PARAM_IN COMP_HANDLETYPE hComponent, PARAM_IN COMP_BUFFERHEADERTYPE *pBuffer)
{
    ...
    */
    ...
    if(COMP_StateIdle == pVideoViData->state)
    {
        eError = FAILURE;
        goto err_state;
    }
...
ERRORTYPE VideoViComponentInit(PARAM_IN COMP_HANDLETYPE hComponent)
{
    ...
    alogd("VideoVirvi component Init! thread_id[0x%lx]", pVideoViData->thread_id);
EXIT:
    return eError;
}
...
static void *Vi_ComponentThread(void *pThreadData)
{
    ...
    prctl(PR_SET_NAME, (unsigned long)"ViComponentThread", 0, 0, 0);
                ...
                if (pVideoViData->state == (COMP_STATETYPE)(cmddata)) {
                    ...
                    switch ((COMP_STATETYPE)(cmddata)) {
                        ...
                        case COMP_StateLoaded: {
                            ...
                            alogd("vipp[%d]virChn[%d]:wait using frame return done", pVideoViData->mMppChnInfo.mDevId, pVideoViData->mMppChnInfo.mChnId);
                        ...
                        case COMP_StateIdle: {
                            ...
                            pVideoViData->state == COMP_StateExecuting) {
                                ...
                                if(!VideoBufMgrUsingEmpty(pVideoViData->mpCapMgr))
                                {
                                    ...
                                    alogw("Be careful! using frame is not empty! vipp[%d]virChn[%d]", pVideoViData->mMppChnInfo.mDevId, pVideoViData->mMppChnInfo.mChnId);
                                }
        ...
        if (pVideoViData->state == COMP_StateExecuting) {
            if (TRUE == pVideoViData->mOutputPortTunnelFlag) { /* Tunnel */
                ...
                if (pOutTunnelComp != NULL) {
                    ...
                    if (pFrm == NULL) {
                        pVideoViData->mWaitingCapDataFlag = TRUE;
                        pFrm = VideoBufMgrGetValidFrame(pVideoViData->mpCapMgr);
                        if (NULL == pFrm) {
                            TMessage_WaitQueueNotEmpty(&pVideoViData->cmd_queue, 2000);
                            pVideoViData->mWaitingCapDataFlag = FALSE;
                            goto PROCESS_MESSAGE;
                        } else
                            pVideoViData->mWaitingCapDataFlag = FALSE;
                    }
                    pFrm->VFrame.mOffsetTop = 0;
                    pFrm->VFrame.mOffsetBottom = pFrm->VFrame.mHeight;
                    pFrm->VFrame.mOffsetLeft = 0;
                    pFrm->VFrame.mOffsetRight = pFrm->VFrame.mWidth;
                    if(pVideoViData->mbStoreFrame)
                    {
                        char strPixFmt[16];
                        ...
                        for(int i=0; i<3; i++)
                        {
                            ...
                            {
                                hal_dcache_invalidate((unsigned long)pFrm->VFrame.mpVirAddr[i], yuvSize[i]);
                                fwrite(pFrm->VFrame.mpVirAddr[i], 1, yuvSize[i], dbgFp);
                                //hal_dcache_clean(pFrm->VFrame.mpVirAddr[i], yuvSize[i]);
