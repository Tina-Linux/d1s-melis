...
{
    char strPixFmt[16];
    if(MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420== SFrame->VFrame.mPixelFormat)
    ...
    if (iEnableFlag) {
        if (!pVideoEISData->mEISAttr.bSimuOffline) {
            ...
            if (pVideoEISData->bByPassMode) {
                ...
                {
                    alogd("create EIS Gyro threadId[0x%lx]", pVideoEISData->mGyroTrd);
                }
            }
        } else {
...
static void VideoEisSetHwCfgs(VIDEOEISDATATYPE *pVideoEISData)
{
    ...
    pVideoEISData->mEisCfg.in_yuv_type =
        (pVideoEISData->mEISAttr.eVideoFmt == MM_PIXEL_FORMAT_YUV_SEMIPLANAR_420) ? YUV420 : YVU420;
...
ERRORTYPE VideoEISComponentInit(PARAM_IN COMP_HANDLETYPE hComponent)
{
    ...
    */
    ...
    alogd("VideoEIS component Init success! threadId[0xlx]", pVideoEISData->mEISTrd);
    return SUCCESS;
...
//    printf("EIS:[%u]\n", pstVFrm->mExposureTime);
    ...
    iRet = sprintf(pTmp, "Get one video pst[%llu], fixed[%llu], exp_time %u",
