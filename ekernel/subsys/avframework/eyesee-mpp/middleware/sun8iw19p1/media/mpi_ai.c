    ...
    switch(eEvent)
    {
        ...
        case COMP_EventBufferPrefilled:
        {
            if(pChn->mCallbackInfo.callback)
            {
                ...
                stUserData.mLen = nData1;
                stUserData.mbIgnore = nData2;
                pChn->mCallbackInfo.callback(pChn->mCallbackInfo.cookie, &ChannelInfo, MPP_EVENT_CAPTURE_AUDIO_DATA, &stUserData);
            }
            break;
        }
        default:
