...
status_t SampleCameraContext::loadConfig()
{
    ...
    if(ConfigFilePath.empty())
    {
        ...
        mConfigPara.mJpegFolderPath = "/mnt/E/sample_Camera_Files";
...
status_t SampleCameraContext::CreateFolder(const std::string& strFolderPath)
{
    ...
    if (stat(pJpegFolderPath, &sb) == 0)
    {
        ...
        {
            aloge("fatal error! [%s] is exist, but mode[0x%lx] is not directory!", pJpegFolderPath, sb.st_mode);
            return UNKNOWN_ERROR;
        }
    }
...
void SampleCameraPictureRegionCallback::addPictureRegion(std::list<PictureRegionType> &rPictureRegionList)
{
    ...
    if(pSampleCameraContext->mConfigPara.jpeg_region_overlay_w && pSampleCameraContext->mConfigPara.jpeg_region_overlay_h)
    {
        ...
        Region1.mInfo.mOverlay.mbInvColEn = (BOOL)pSampleCameraContext->mConfigPara.jpeg_region_overlay_invert;
...
{
    SampleCameraContext* pContext = (SampleCameraContext*) pThreadData;
    int ret;
    ...
    prctl(PR_SET_NAME, (unsigned long)"demo_Camera_DetectInput", 0, 0, 0);
    while(1)
    {
...
int main_demo_Camera(int argc, char *argv[])
{
    int result = 0;
    void *pValue = NULL;
    VO_LAYER hlay = 0;
    ...
    captureSize={(unsigned int)pContext->mConfigPara.mCaptureWidth, (unsigned int)pContext->mConfigPara.mCaptureHeight};
...
FINSH_FUNCTION_EXPORT_ALIAS(main_demo_Camera, __cmd_demo_Camera, test mpp framework camera);
