        ...
        case MEDIA_NOTIFY_EOF:
        {
            ...
            if(MOD_ID_DEMUX == eModId)
            {
                ...
                if(!mPlayer->mDmxNotifyEof)
                {
                    ...
                    {
                        AW_MPI_VDEC_SetStreamEof(mPlayer->mVdecChn, TRUE);
                    }
                    if(mPlayer->mAdecChn >= 0)
                    {
                        AW_MPI_ADEC_SetStreamEof(mPlayer->mAdecChn, TRUE);
                    }
            ...
            else if(MOD_ID_VDEC == eModId)
            {
                ...
                if(!mPlayer->mVdecNotifyEof)
                {
                    ...
                    if(mPlayer->mVOChn >= 0)
                    {
                        AW_MPI_VO_SetStreamEof(mPlayer->mVOLayer, mPlayer->mVOChn, TRUE);
                    }
            ...
            else if(MOD_ID_ADEC == eModId)
            {
                ...
                if(!mPlayer->mAdecNotifyEof)
                {
                    ...
                    if(mPlayer->mAOChn >= 0)
                    {
                        AW_MPI_AO_SetStreamEof(mPlayer->mAODev, mPlayer->mAOChn, TRUE, FALSE);
                    }
...
status_t EyeseePlayer::prepare()
{
    ...
    if(mAdecChn >= 0)
    {
        ...
        //enable audio_hw_ao
        //embeded in AW_MPI_AO_EnableChn, not need call this api any more.
        ...
        {
            ret = AW_MPI_AO_EnableChn(mAODev, mAOChn);
            if(SUCCESS == ret)
            {
                bSuccessFlag = true;
...
status_t EyeseePlayer::stop_l()
{
    ...
    if(mAODev>=0 && mAOChn>=0)
    {
        AW_MPI_AO_DisableChn(mAODev, mAOChn);
        mAOChn = MM_INVALID_CHN;
    }
    if(mAdecChn >= 0)
    {
...
status_t EyeseePlayer::seekTo_l(int msec)
{
    ...
    if(mDmxChn >= 0)
    {
        ...
        if(mAdecChn >= 0)
        {
            AW_MPI_ADEC_SetStreamEof(mAdecChn, FALSE);
        }
        if(mAOChn >= 0)
        {
            AW_MPI_AO_SetStreamEof(mAODev, mAOChn, FALSE, FALSE);
        }
        if(mVdecChn >= 0)
        {
            AW_MPI_VDEC_SetStreamEof(mVdecChn, FALSE);
        }
        if(mVOChn >= 0)
        {
            AW_MPI_VO_SetStreamEof(mVOLayer, mVOChn, FALSE);
        }
        mDmxNotifyEof = false;
        mVdecNotifyEof = false;
        mAdecNotifyEof = false;
...
{
    if (true == mute)
        AW_MPI_AO_SetDevMute(mAODev, TRUE, NULL);
    else
        AW_MPI_AO_SetDevMute(mAODev, FALSE, NULL);
...
EyeseePlayer::TrackInfo::TrackInfo(int trackType, string language) :
...
void EyeseePlayer::getTrackInfo(vector<TrackInfo> &trackInfo)
{
    ...
    {
        pTrackInfo = new TrackInfo(TrackInfo::MEDIA_TRACK_TYPE_VIDEO, "video");
        trackInfo.push_back(*pTrackInfo);
        delete pTrackInfo;
    }
    for(i=0; i<DemuxMediaInfo.mAudioNum; i++)
    {
        pTrackInfo = new TrackInfo(TrackInfo::MEDIA_TRACK_TYPE_AUDIO, (const char*)DemuxMediaInfo.mAudioStreamInfo[i].strLang);
        trackInfo.push_back(*pTrackInfo);
        delete pTrackInfo;
    }
    for(i=0; i<DemuxMediaInfo.mSubtitleNum; i++)
    {
        pTrackInfo = new TrackInfo(TrackInfo::MEDIA_TRACK_TYPE_TIMEDTEXT, (const char*)DemuxMediaInfo.mSubtitleStreamInfo[i].strLang);
        trackInfo.push_back(*pTrackInfo);
        delete pTrackInfo;
    }
}
