...
{
    VIChannel* const mVich = (VIChannel*)dv;
        ...
        if(mbAdasDetectEnable)
        {
            ...
            if(mAdasDataCallBackOut == true)
            {
                if(NULL != mPtrImgBuffer)
                {
                    ...
                    //printf("\nmemcpy data to mPtrImgBuffer  w:%d h:%d-\n",m_width,m_height);
...
status_t VIChannel::prepare()
{
    ...
    alogd("set vipp[%d]attr:[%ldx%ld],fps[%d], pixFmt[0x%lx], nbufs[%d]", mChnId, attr.format.width, attr.format.height, attr.fps, attr.format.pixelformat, attr.nbufs);
    ...
    alogd("get vipp[%d]attr:[%ldx%ld],fps[%d], pixFmt[0x%lx], nbufs[%d]", mChnId, attr.format.width, attr.format.height, attr.fps, attr.format.pixelformat, attr.nbufs);
...
status_t VIChannel::releaseFrame(uint32_t index)
{
    ...
    {
        aloge("Fatal error! invalid buffer index %ld!", index);
        return BAD_VALUE;
    }
...
//    VIDEO_FRAME_BUFFER_S *pFrmbuf = mpVideoFrmBuffer + index;
    ...
    for(std::list<VIDEO_FRAME_BUFFER_S>::iterator it=mFrameBuffers.begin(); it!=mFrameBuffers.end(); ++it)
    {
        if(it->mFrameBuf.mId == index)
        {
            ...
            {
                aloge("fatal error! VI frame id[0x%lx] is repeated!", index);
            }
        }
    }
    if(NULL == pFrmbuf)
    {
        aloge("fatal error! not find frame index[0x%lx]", index);
        return UNKNOWN_ERROR;
    }
