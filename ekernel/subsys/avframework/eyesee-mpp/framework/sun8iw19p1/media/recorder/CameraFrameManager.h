...
{
public:
    CameraFrameManager(CameraRecordingProxy *pCameraProxy, int nChannel);
    ~CameraFrameManager();
    status_t addReleaseFrame(VIDEO_FRAME_INFO_S *pReleaseFrame);
private:
    ...
    std::list<VIDEO_FRAME_INFO_S> mIdleEncBufList;
    std::list<VIDEO_FRAME_INFO_S> mWaitReleaseEncBufList;
    std::list<VIDEO_FRAME_INFO_S> mReleasingEncBufList;    //only one element in it.
};
