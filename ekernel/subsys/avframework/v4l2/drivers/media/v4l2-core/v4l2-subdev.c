...
static int subdev_open(struct file *file)
{
	...
	subdev_fh  = rt_malloc(sizeof(*subdev_fh));
	if (subdev_fh == NULL)
		return -ENOMEM;

	memset(subdev_fh, 0x00, sizeof(*subdev_fh));
...
static unsigned int subdev_poll(struct file *file, struct rt_pollreq *req)
{
	struct video_device *vdev = video_devdata(file);
	struct v4l2_subdev *sd = vdev_to_v4l2_subdev(vdev);
...
v4l2_subdev_alloc_pad_config(struct v4l2_subdev *sd)
{
	...
	cfg = rt_calloc(sd->entity.num_pads, sizeof(*cfg));
	if (!cfg)
		return NULL;
