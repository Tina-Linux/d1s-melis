				     ...
				     struct v4l2_subdev_format *fmt)
{
	...
	rt_mutex_take(&mipi->subdev_lock, RT_WAITING_FOREVER);
	fmt->format = *mf;
	rt_mutex_release(&mipi->subdev_lock);
	return 0;
}
...
static int __mipi_init_subdev(struct mipi_dev *mipi)
{
	...
	rt_mutex_init(&mipi->subdev_lock, "mipisubmtx", RT_IPC_FLAG_FIFO);
...
static int mipi_probe(int id)
{
	struct mipi_dev *mipi = NULL;
	int ret = 0;

	mipi = rt_calloc(1, sizeof(struct mipi_dev));
	if (!mipi) {
		ret = -ENOMEM;
		goto ekzalloc;
	}
	...
	mipi->id = id;
	mipi->base = sunxi_vin_get_mipi_base(id);
	if (!mipi->base) {
		ret = -EIO;
		goto freedev;
	}
...
static int mipi_remove(int id)
{
	struct mipi_dev *mipi = glb_mipi[id];
	struct v4l2_subdev *sd = &mipi->subdev;
