...
static int read_frame(int mode)
{
	...
	if (ret = ioctl(fd, VIDIOC_DQBUF, &buf)) {
	...
	if (buf.index >= n_buffers) {
		printf("buf.index: %d, n_buffers: %d\n", buf.index, n_buffers);
		free(buf.m.planes);
		return -1;
	}
	...
	if (ret = ioctl(fd, VIDIOC_QBUF, &buf)) {
		printf("VIDIOC_QBUF buf.index %d failed : %d\n", buf.index, ret);
		free(buf.m.planes);
		return -1;
	}
...
static int req_frame_buffers(void)
{
	...
	if (ret = ioctl(fd, VIDIOC_REQBUFS, &req)) {
	...
	for (n_buffers = 0; n_buffers < req.count; ++n_buffers) {
		...
		for (i = 0; i < nplanes; i++) {
			...
			buffers[n_buffers].start[i] = calloc(1, buf.m.planes[i].length);
			if (!buffers[n_buffers].start[i]) {
...
static int camera_init(int sel, int mode)
{
	...
	inp.index = sel;
	if (ret = ioctl(fd, VIDIOC_S_INPUT, &inp)) {
	...
	if (ret = ioctl(fd, VIDIOC_S_PARM, &parms)) {
...
static int camera_fmt_set(int mode)
{
	...
	if (ret = ioctl(fd, VIDIOC_S_FMT, &fmt)) {
		printf("VIDIOC_S_FMT error!: %d\n", ret);
		return -1;
	}

	if (ret = ioctl(fd, VIDIOC_G_FMT, &fmt)) {
...
static int video_set_control(int cmd, int value)
{
	...
	control.id = cmd;
	control.value = value;
	if (ret = ioctl(fd, VIDIOC_S_CTRL, &control)) {
...
static int video_get_control(int cmd)
{
	...
	control.id = cmd;
	if (ret = ioctl(fd, VIDIOC_G_CTRL, &control)) {
...
static int main_test(int sel, int mode)
{
	...
	if (ret = ioctl(fd, VIDIOC_STREAMON, &type)) {
...
#ifdef TIMEOUT
				if (ret = ioctl(fd, VIDIOC_STREAMOFF, &type))
...
int cmd_csi_mp_usr(int argc, char *argv[])
{
	int i, test_cnt = 1;
	int sel = 0;
	int width = 640;
...
#define DEFAULT_PRIORITY 14
	...
	ret = pthread_create(&csi_mp_usr, &attr, csi_thread, (void *)&arg);
	...
	pthread_join(csi_mp_usr, NULL);

	return 0;
}

FINSH_FUNCTION_EXPORT_ALIAS(cmd_csi_mp_usr, __cmd_csi_mp_usr, csi mplane userptr);
