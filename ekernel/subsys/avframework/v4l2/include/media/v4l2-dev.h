...
struct v4l2_file_operations {
	...
	unsigned int (*poll) (struct file *, struct rt_pollreq *);
	long (*ioctl) (struct file *, unsigned int, unsigned long);
	unsigned long (*get_unmapped_area) (struct file *, unsigned long,
				unsigned long, unsigned long, unsigned long);
	int (*mmap) (struct file *, struct map_struct *);
...
struct video_device
{
	...
	int vfl_dir;    //VFL_DIR_TX

	int minor;
	u16 num;

	unsigned long flags;    //V4L2_FL_REGISTERED
	int index;

	/* V4L2 file handles */
	rt_ubase_t		fh_lock;
	struct list_head	fh_list;    //struct v4l2_fh*

	int dev_debug;  //V4L2_DEV_DEBUG_IOCTL
...
#define dfs_to_vdev(dfp)  container_of((dfp)->data, struct video_device, dev)


/**
 * vdev_file - return a pointer to the struct file
 *
 *
 * @dfp: pointer to struct &dfs_fd
 */
static inline struct file *video_file(struct dfs_fd *dfp)
{
	struct video_device *vdev = dfs_to_vdev(dfp);

	return vdev_files[vdev->minor];
}
