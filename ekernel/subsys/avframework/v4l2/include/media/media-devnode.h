 ...
 * @dev:	pointer to struct &rt_device containing the media controller device
...
struct media_devnode {
	...
	/* sysfs */
	struct rt_device dev;		/* media device */
 ...
 */
static inline struct media_devnode *media_devnode_data(struct file *file)
{
	return file->private_data;
}


#define dfs_to_mdev(dfp)  container_of((dfp)->data, struct media_devnode, dev)

/**
 * media_file - return a pointer to the struct file
 *
 *
 * @dfd: pointer to struct &dfs_fd
 */
static inline struct file *media_file(struct dfs_fd *dfp)
{
	struct media_devnode *devnode = dfs_to_mdev(dfp);

	return mdev_files[devnode->minor];
}
