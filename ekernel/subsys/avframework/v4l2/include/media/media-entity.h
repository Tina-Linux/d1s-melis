...
struct media_entity_graph {
	struct {
		...
		struct list_head *link; //struct media_link
...
struct media_link {
	...
	unsigned long flags;    //MEDIA_LNK_FL_ENABLED
	bool is_backlink;
};
...
struct media_pad {
	...
	struct media_entity *entity;
	u16 index;
	unsigned long flags;    //MEDIA_PAD_FL_SINK
};
...
struct media_entity {
	...
	u32 function;   //MEDIA_ENT_F_UNKNOWN
	unsigned long flags;
	...
	struct list_head links; //struct media_link
...
struct media_interface {
	...
	u32				type;   //MEDIA_INTF_T_V4L_VIDEO
	u32				flags;
};
 ...
 */
struct media_intf_devnode *
  media_devnode_create(struct media_device *mdev,
				  u32 type, u32 flags,
				  u32 major, u32 minor);
/**
