...
int df(const char *path)
{
    ...
    result = dfs_statfs(fullpath ? fullpath : NULL, &buffer);
    if (result != 0)
    {
        rt_free(fullpath);
...
static int cmd_show_mount_table(int argc, char **argv)
{
    struct dfs_filesystem *iter;
            ...
            iter < &filesystem_table[DFS_FILESYSTEMS_MAX]; iter++)
    {
        if ((iter->path == NULL) || (iter->ops == NULL))
            continue;
        rt_kprintf("%-08s", iter->path);
        rt_kprintf("%-08s", iter->ops->name);
        if (iter->dev_id)
        {
            object = (struct rt_object *)(iter->dev_id);
            rt_kprintf("%-08s", object->name);
        }
        else
        {
            rt_kprintf("%-08s", "NULL");
        }
        rt_kprintf("%d\n", iter->rwflag & 0xffffffff);
    }
...
FINSH_FUNCTION_EXPORT_ALIAS(cmd_show_mount_table, __cmd_mount_table, show mount table);
#endif
