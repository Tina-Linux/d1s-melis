...
static rt_mutex_t lock_env = RT_NULL;
static int env_lock(void)
{
    if (lock_env == RT_NULL) {

	lock_env = rt_mutex_create("dmntex", RT_IPC_FLAG_FIFO);
	if(lock_env == RT_NULL)
		return -1;
    }

    if(rt_mutex_take(lock_env, RT_WAITING_FOREVER) == RT_EOK)
	return 0;
...
{
    if (lock_env != NULL) {
	if(rt_mutex_release(lock_env) == RT_EOK)
		return 0;
	else
		return -1;
    }
