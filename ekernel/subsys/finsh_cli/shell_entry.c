...
{
    int ch;
    struct finsh_shell *shell = NULL;
    shell = rt_malloc(sizeof(struct finsh_shell));
    ...
    cli_task_set_console(rt_thread_self(), (cli_console *)parameter);
	
    memset(shell->line, 0, FINSH_CMD_SIZE);
    /* normal is echo mode */
...
#ifdef CONFIG_FINSH_EXTERN_SCRIPT
    extern int g_cli_direct_read;
    if (g_cli_direct_read == 0)
    {
        int ret = 0;
        ret = msh_exec_script(CONFIG_FINSH_EXTERN_SCRIPT_PATH"startup.sh", \
                strlen(CONFIG_FINSH_EXTERN_SCRIPT_PATH"startup.sh"));
        if(-1 == ret)
        {
            rt_kprintf("open external script file %s failed!\r\n",
                    CONFIG_FINSH_EXTERN_SCRIPT_PATH"startup.sh");
        }
    }
#endif
    while (1)
    {
        if (cli_console_task_check_exit_flag())
        {
            break;
        }
        ch = finsh_getchar();
