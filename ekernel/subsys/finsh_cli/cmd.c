...
int helloworld(int argc, const char **argv)
{
    (void)argc;
    (void)argv;
...
FINSH_FUNCTION_EXPORT(helloworld, say hello world);
MSH_CMD_EXPORT(helloworld, say hello to RT-Thread);
        ...
        next = list_get_next(next, &find_arg);
        {
            ...
            for (i = 0; i < find_arg.nr_out; i++)
            {
                ...
                thread = (struct rt_thread*)obj;
                {
                    ...
                    rt_kprintf(" 0x%08lx 0x%08lx    %02ld%%   0x%08lx %03d\n",
                    ...
                    rt_kprintf(" 0x%08lx 0x%08x    %02ld%%   0x%08lx %03ld\n",
...
//cmd eg:  query_thread disp2
int query_thread(int argc, const char **argv)
{
    (void)argc;
    (void)argv;
    rt_ubase_t level;
    ...
    if(argv[1])
    {
        findname = (char *)argv[1];
    }
    else
    {
        rt_kprintf("cmd :  query_thread name\n");
        return 0;
    }
        ...
        next = list_get_next(next, &find_arg);
        {
            ...
            for (i = 0; i < find_arg.nr_out; i++)
            {
                ...
                thread = (struct rt_thread*)obj;
                {
                    rt_uint8_t stat;
                    rt_uint8_t *ptr;
                    if(strcmp(findname, thread->name) != 0)
                    {
                        continue;
                    }
                    stat = (thread->stat & RT_THREAD_STAT_MASK);
    ...
    while (next != (rt_list_t*)RT_NULL);
    rt_kprintf("not_exist\n");
    return 0;
}
FINSH_FUNCTION_EXPORT(query_thread, query thread);
MSH_CMD_EXPORT(query_thread, query thread);


#ifdef RT_USING_SEMAPHORE
int list_sem(int argc, const char **argv)
{
    (void)argc;
    (void)argv;
    rt_ubase_t level;
...
int list_event(int argc, const char **argv)
{
    (void)argc;
    (void)argv;
    rt_ubase_t level;
...
int list_mutex(int argc, const char **argv)
{
    (void)argc;
    (void)argv;
    rt_ubase_t level;
...
int list_mailbox(int argc, const char **argv)
{
    (void)argc;
    (void)argv;
    rt_ubase_t level;
...
int list_msgqueue(int argc, const char **argv)
{
    (void)argc;
    (void)argv;
    rt_ubase_t level;
...
int list_mempool(int argc, const char **argv)
{
    (void)argc;
    (void)argv;
    rt_ubase_t level;
...
int list_timer(int argc, const char **argv)
{
    (void)argc;
    (void)argv;
    rt_ubase_t level;
    ...
    rt_kprintf("current tick:0x%08lx\n", rt_tick_get());
...
int list_device(int argc, const char **argv)
{
    (void)argc;
    (void)argv;
    rt_ubase_t level;
