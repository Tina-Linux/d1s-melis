...
static void ntp_sync_thread_enrty(void *param)
{
    ...
    rt_thread_delay(30 * RT_TICK_PER_SECOND);

    while (1)
    {
        ntp_sync_to_rtc(NULL);
        rt_thread_delay(3600 * RT_TICK_PER_SECOND);
    }
}
...
MSH_CMD_EXPORT_ALIAS(rt_rtc_ntp_sync_init, ntp_init, Iinit NTP(Network Time Protocol): ntp_sync [host_name]);
...
time_t ntp_sync_to_rtc(const char *host_name)
{
    ...
    if (cur_time)
    {
	/*use settimeofday update the local time :by ranchao*/
	sys_time.tv_sec = cur_time;
...
static int cmd_ntp_sync(int argc, char **argv)
{
    ...
    rt_kprintf("===================================\n");
    rt_kprintf("Default NTP Server: ntp1.aliyun.com\nYou can use ntp_sync argv[1] replace\n");
    rt_kprintf("===================================\n");
