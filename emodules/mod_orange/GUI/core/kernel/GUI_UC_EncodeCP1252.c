...
static U16 _GetCharCode(const char GUI_UNI_PTR *s)
{
    ...
    if (esCHS_Char2Uni(EPDK_CHARSET_ENM_CP1252, (__u8 *)s, 8, &r) <= 0)
    {
        r = 1;
    }
...
static int _Encode(char *s, U16 Char)
{
    ...
    r = esCHS_Uni2Char(EPDK_CHARSET_ENM_CP1252, Char, (__u8 *)s, 3);
    if (r <= 0)
    {
        return 1;
    }
...
*/
void GUI_UC_SetEncodeCP1252(void)
{
    GUI_LOCK();
    GUI_Context.pUC_API = &_API_Table;
    GUI_Context.charset_enum = EPDK_CHARSET_ENM_CP1252;
    GUI_UNLOCK();
}
