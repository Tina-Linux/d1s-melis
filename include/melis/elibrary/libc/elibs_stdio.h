...
{
    uint32_t    d_ino;              /* to be POSIX conform                                  */
    uint8_t     fatdirattr;         /* FAT only. Contains the "DIR_Attr" field of an entry. */
    uint64_t    d_size;             /* file size, if directory it will be 0                 */
    uint8_t     d_name[FSYS_DIRNAME_MAX];
} ES_DIRENT;
...
typedef struct
{
    uint32_t    f_type;               /* fs type */
    uint32_t    f_bsize;              /* block size */
    uint32_t    f_blocks;             /* blocks number */
    uint32_t    f_bfree;              /* free blocks number */
    uint32_t    f_files;              /* exist files number */
    uint32_t    f_namelen;            /* max name lenght */

    uint32_t    f_attribute;          /* fs attribute like readonly */
    uint32_t    f_mediaattr;          /* media attribute like movable */
    uint8_t     f_volletter;          /* like 'c' or 'd' etc */
    uint8_t     f_volname[MAX_VOLUME_NAME_LEN];  /* volume name */
    uint8_t     f_fsname[MAX_FS_NAME_LEN];       /* the bind file system name */
} __ES_FSSTAT;
...
*******************************************************************************************************************
*/
extern int32_t eLIBs_setfs(char *partname, uint32_t cmd, int32_t aux, char *para);
