...
#define CONFIG_PROCESS_THREAD_PRIO (18)

#define SBM_SHOW_DETAIL_LOG (0)

#define dy_log(fmt, arg...) do{ \
    if(pSbm->bShowLogFlag == 1) \
        logd(fmt, arg); \
    else \
        logi(fmt, arg); \
}while(0)
...
extern SbmInterface* GetSbmInterfaceStream(void);
...
int SbmFrameInit(SbmInterface* pSelf, SbmConfig* pSbmConfig)
{
    ...
    param.sched_priority = CONFIG_PROCESS_THREAD_PRIO;
    pthread_attr_setschedparam(&attr, &param);

    int err = pthread_create(&pSbm->mThreadId, &attr, ProcessThread, pSbm);
    #else
...
static VideoStreamDataInfo* SbmFrameRequestFramePic(SbmInterface* pSelf) //* requestFramePic
{
    ...
    dy_log("sbm request stream, pos = %d, pFrame = %p, pts = %lld",
          pSbm->mFramePicFifo.nFPReadPos,pFramePic, (long long int)pFramePic->nPts);
    unlock(pSbm);
                                                ...
                                                VideoStreamDataInfo *pDataInfo)//* returnFramePic
{
    ...
    dy_log("sbm return stream, pDataInfo = %p",pDataInfo);
                                               ...
                                               VideoStreamDataInfo *pDataInfo) //* flushFramePic
{
    ...
    dy_log("sbm flush stream , pos = %d, pFrame = %p, %p",nFlushPos,
        ...
        if(CdcMessageQueueTryGetMessage(pSbm->mq, &msg, 20) == 0)
        {
            ...
            else if(msg.messageId == SBM_THREAD_CMD_RESET)
            {
                ...
                if(pSbm->sbmInterface.bUseNewVeMemoryProgram == 1)
                {
                    ...
                    while(1)
                    {
                        SbmFrameBufferNode* pSbmFrameBufferNode = (SbmFrameBufferNode*)FIFODequeue(
                            (FiFoQueueInst**)&pSbm->pSbmFrameBufferValidQueue);
                        if(pSbmFrameBufferNode != NULL)
                        {
                            FIFOEnqueue((FiFoQueueInst**)&pSbm->pSbmFrameBufferEmptyQueue,
                             (FiFoQueueInst*)pSbmFrameBufferNode);
